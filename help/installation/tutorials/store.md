...
description: Découvrez comment configurer votre magasin Adobe Commerce à partir de la ligne de commande après la configuration du déploiement et la configuration du schéma de base de données, y compris les options d’installation sécurisées.
...
| `--language` | Code de langue à utiliser dans Admin et storefront.<br><br>(Si vous ne l’avez pas déjà fait, vous pouvez afficher la liste des codes de langue en saisissant `magento info:language:list` dans le répertoire `bin`.) | Non |
