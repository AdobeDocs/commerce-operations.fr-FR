---
title: Installation et configuration de Redis
description: Découvrez comment installer et configurer Redis pour la mise en cache et le stockage de session avec Adobe Commerce. Découvrez les options d’optimisation et d’optimisation des performances.
...
Redis est un magasin de données en mémoire qui peut être utilisé comme serveur principal du cache et pour le stockage des sessions. Les principales fonctionnalités sont les suivantes :
...
- [Océan numérique](https://www.digitalocean.com/community/tutorials/how-to-install-and-use-redis)
...
Pour optimiser l’instance Redis en fonction de vos besoins, vous obtenez de meilleurs résultats en utilisant une instance dédiée pour chaque session, le cache de Commerce et le cache de pleine page (FPC).
...
Vous pouvez également activer simultanément les options RDB et AOF. Pour plus d’informations, y compris les avantages et les inconvénients des options de persistance, consultez la [documentation sur la persistance Redis](https://redis.io/docs/latest/operate/rs/databases/configure/database-persistence/).

Pour l’instance de cache, configurez-la de sorte qu’elle soit suffisamment grande pour stocker l’intégralité de votre cache Commerce.

- Les exigences de taille dépendent de différents facteurs, tels que le nombre de produits et de vues de magasin.

  Pour commencer, utilisez la taille du dossier `var/cache` sur votre système de fichiers. Par exemple, si la `var/cache` est de 5 Go, configurez votre instance Redis avec au moins 5 Go pour démarrer.
