...
* Les composants Zend Framework (ZF1) qui ont atteint leur fin de vie ont été supprimés de la base de code. Voir [Modifications non rétrocompatibles mises en évidence](https://developer.adobe.com/commerce/php/development/backward-incompatible-changes/highlights/).<!-- AC-1512 -->
...
* [Passage en caisse rapide Bolt](https://experienceleague.adobe.com/docs/commerce/quick-checkout/overview.html)
...
* Des noms descriptifs accessibles ont été ajoutés aux boutons dans toute la vitrine.

<!-- AC-2488-->

* Des étiquettes descriptives verbales ont été ajoutées aux cases des **[!UICONTROL Has Video]** filtres de recherche.
...
* Le **[!UICONTROL Pay Later]** bouton et les bannières (messages) pour l’Italie et l’Espagne ont été activés pour Braintree.
...
**Amélioration du temps de réponse lors de l’interrogation de catégories avec les autorisations de** catégorie activées. Les temps de réponse se sont améliorés lors de l’interrogation de produits dans des catégories pour lesquelles les autorisations de catégorie sont activées et qui contiennent de nombreux groupes de clients ou catalogues partagés. Code modifications qui vont dans le sens de cette amélioration des performances, notamment : <!-- B2B-2179 2404 2329-->

* Le fonctionnement du résolveur GraphQL a été optimisé.
* Le fournisseur de données ne réinitialise plus les données mises en cache au moment de l’exécution pour chaque nouveau produit en cours de traitement, ce qui a amélioré les performances. <!-- B2B-2452-->
* Le traitement des autorisations de produit a été optimisé dans le résolveur GraphQL. Les autorisations de produit sont désormais appliquées aux collections de produits sur une base par produit.  <!-- B2B-2404-->
* Autorisations optimisées lues dans les catalogues partagés de la `addProductsToCart` mutation. <!-- B2B-2424-->
* Les autorisations de produit ont été optimisées avec des fragments de produit insérés. <!-- B2B-2459-->

**Optimisation des performances du rendu** de l’arborescence de catégories. La `categoryList` requête a été optimisée afin de prendre en charge les clients avec un grand nombre de catégories. Les marchands qui utilisent cette requête pour rechercher plus de 1500 catégories avec jusqu’à cinq niveaux de sous-catégories imbriquées remarqueront une amélioration des performances. Le chargement des enfants de catégories par refactorisation de code a été amélioré en supprimant les appels de méthode inutiles, en améliorant la mise en cache de l’arborescence des catégories et en chargeant les données de catégorie de manière récursive.  Les modifications de code qui prennent en charge cette amélioration des performances incluent : <!-- B2B-2484 2469 2472-->
...
PWA Studio v.13.0.x est compatible avec Adobe Commerce 2.4.6. Il comprend plusieurs améliorations pour améliorer l’accessibilité. Pour plus d’informations sur les correctifs, voir [Versions de PWA Studio](https://github.com/magento/pwa-studio/releases). Voir [Compatibilité](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) des versions pour obtenir la liste des versions de PWA Studio et leurs versions principales compatibles Adobe Commerce.
...
* Le déploiement de contenu statique n’expire plus ou n’échoue plus lors de déploiements standard ou compacts avec plusieurs paramètres régionaux et sites Web.

<!-- ACP2E-923-->

* La stratégie compacte n’entraîne plus une référence non valide à un domaine externe lors de l’exécution du déploiement de contenu statique (`bin/magento set:static-content:deploy -f --strategy compact`).

<!-- AC-5994-->

* L’exécution `bin/magento setup:di:compile -vvv` (ou avec les options ou`-vv`) renvoie désormais un message informatif en cas d’échec`-v`. [GitHub-35391](https://github.com/magento/magento2/issues/35391)
...
* Adobe Commerce renvoie désormais un message informatif lorsqu’une `addProductsToCart` mutation échoue lors de l’ajout d’un produit au panier avec des autorisations insuffisantes.
...
* Les listes de catégories sont désormais mises à jour comme prévu lorsqu’un produit est affecté à un nouveau site web ou magasin. Auparavant, l’indexeur de prix de produit n’était pas marqué comme non valide et il a ensuite été redéclenché lors d’une exécution de `cron` comme prévu. Par conséquent, l’entrée de prix du produit n’apparaissait pas dans le tableau de l’indice des prix et n’était pas affichée dans le nouveau magasin ou site Web.
...
* Les clients affectés à un groupe de vente en gros peuvent désormais ajouter un produit groupé à leur panier lorsque les autorisations de catégorie l’autorisent. Si cette autorisation n’est pas définie pour un groupe de clients, le paramètre de configuration prévaut.

<!-- ACP2E-1306-->

* Le tri des produits par attributs booléens fonctionne maintenant comme prévu. [GitHub-36390](https://github.com/magento/magento2/issues/36390)
...
* La logique de validation a été ajoutée au flux de travail Gérer les codes de bon pour appliquer les paramètres de code de coupon lors de la génération de coupons. Auparavant, la génération de code de coupon ignorait le **[!UICONTROL Code Length]** paramètre lorsque la valeur Qté&#x200B;**de** coupon était supérieure à 2500.

<!-- ACP2E-1029-->

* Les points de récompense sont désormais ajoutés au compte d’un client comme prévu lorsqu’un coupon de récompense est appliqué lors du paiement avec le mode de paiement PayPal Express.
...
* Les montants des produits de cartes-cadeaux restent désormais identiques au montant spécifié dans un fichier CSV lors de l’importation. Des vérifications des options existantes ont été ajoutées et les anciennes valeurs sont maintenant mises à jour sur la base d’une clé unique. Auparavant, le montant des cartes-cadeaux était doublé lorsque les cartes-cadeaux étaient importées plusieurs fois.

### Cadeau

<!-- AC-3061-->

* Le prix de l’emballage cadeau est désormais affiché comme prévu dans le bloc récapitulatif de la commande lorsque l’emballage cadeau a été appliqué au niveau de la commande à une commande contenant à la fois des produits physiques et virtuels. Auparavant, Adobe Commerce n’affichait pas le prix de l’emballage du cadeau.
...
* Les permissions des catégories sont désormais respectées lors des requêtes GraphQL.

<!-- AC-1805-->

* La `customer` requête filtre désormais correctement les commandes avec une plage spécifiée `from` et `to` une plage. [GitHub-31537](https://github.com/magento/magento2/issues/31537)

<!-- AC-5984-->

* La `product` requête renvoie maintenant les données de catégorie correctes dans l’agrégation. Auparavant, `attribute_code.category_uid` renvoyait la valeur de `category_id` au lieu de `category_uid`.

<!-- AC-6084-->

* La `customerOrders` requête ne renvoie plus d’erreur `\"model\" value should be specified`. [GitHub-35852](https://github.com/magento/magento2/issues/35852)
...
* L’exportation complète fonctionne désormais correctement lorsqu’AWS S3 est utilisé comme stockage distant. Auparavant, l’exportation échouait en raison de la taille du fichier téléchargé.

<!-- ACP2E-982-->

* L’état du produit est désormais correctement importé lorsque les colonnes de données d’inventaire du fichier CSV sont manquantes. Auparavant, après l’importation du produit, la source par défaut était automatiquement affectée au produit, sa quantité était définie sur zéro et son état était répertorié comme étant en rupture de stock lorsque les colonnes de données d’inventaire du fichier CSV étaient manquantes.

<!-- ACP2E-841-->

* L’importation de valeurs de prix préférentiels fournit désormais le nombre correct d’enregistrements nouveaux et mis à jour lorsque l’importation comprend plus de 100 enregistrements. Auparavant, le message de réussite de l’importation fournissait un décompte incorrect.
...
* Le `Fixer.io` service d’importation fonctionne maintenant comme prévu. Les taux de change sont maintenant importés avec succès. [GitHub-36010](https://github.com/magento/partners-magento2ee/issues/36010)
...
* Les commandes en double ne résultent plus de requêtes GraphQL identiques simultanées. Les guillemets sont maintenant verrouillés pendant 10 secondes au maximum pour empêcher la création de commandes en double. La deuxième demande échoue avec les erreurs suivantes : `The cart isn't active` ou `The order has already been placed and is currently processing`. Cette valeur de verrouillage peut être configurée via di pour le paramètre de `lockWaitTimeout` de classe `Magento\Quote\Model\PlaceOrderMutex`. [GitHub-36274](https://github.com/magento/magento2/issues/36274)
...
* Adobe Commerce ne renvoie plus l’erreur suivante lorsqu’un marchand clique sur **[!UICONTROL Reports]** > **[!UICONTROL Braintree Settlement]**: `Error: Undefined constant Braintree\PaymentInstrumentType::ANDROID_PAY_CARD in /var/www/html/vendor/paypal/module-braintree-core/Ui/Component/Report/Listing/Column/PaymentType.php:49`. [GitHub-35434](https://github.com/magento/magento2/issues/35434)

<!-- BUNDLE-3043-->

* La `customerPaymentTokens` requête ne renvoie plus Braintree jetons lorsque le coffre-fort est désactivé sur l’administrateur.

<!-- BUNDLE-3107-->

* Le **[!UICONTROL Pay Later]** bouton et les bannières ont été activés pour l’Italie et l’Espagne.
...
* Les caractères japonais peuvent désormais être saisis directement dans le jeu d’attributs de la page de produits (zone de liste). Auparavant, la saisie directe de caractères japonais dans cette zone de liste modifiable entraînait la modification ou la suppression de caractères lors de l’exécution du navigateur Firefox sous MacOS. L’attribut `textInput` HTML, qui peut mieux gérer la conversion des caractères japonais dans tous les navigateurs, a remplacé les `Value` attributs et `valueUpdate` .
...
* La fonctionnalité glisser-déposer des options personnalisées du produit fonctionne désormais comme prévu. Vous pouvez maintenant déplacer une option personnalisable d’une page à une autre lorsque plusieurs pages sont disponibles dans la grille d’options personnalisables.

<!-- ACP2E-866-->

* Les graphiques du tableau de bord d’administration affichent désormais les intervalles de temps cumulatif depuis le début de l’année civile en cours jusqu’à la date actuelle, comme prévu. Auparavant, les plages de dates s’incluaient sur 12 mois.

<!-- AC-3088-->

* Un favicon par défaut manquant a été ajouté sur la page d’accueil de la vitrine. [GitHub-35493](https://github.com/magento/magento2/issues/35493)
...
* Ajout d’un texte informatif sous le champ d’entrée `Storefront Properties` de l’attribut `Search Weight` dans Admin **[!UICONTROL Stores]**  > [!UICONTROL Attributes] > **[!UICONTROL Product]**.
...
* Le paramètre `Welcome Text` En-tête de thème peut désormais contenir un guillemet simple. Auparavant, Adobe Commerce a généré cette erreur JavaScript : `Uncaught SyntaxError: Unable to process binding "ifnot: function(){return customer().fullname }"`.

### Réécritures d’URL

<!-- ACP2E-808-->

* La modification de la visibilité du produit sur **[!UICONTROL Not Visible Individually]** l’étendue globale Toutes les vues de magasin supprime désormais uniquement les réécritures d’URL pour les vues de magasin qui n’ont pas remplacé l’attribut de visibilité. La modification de la visibilité du produit de cette manière génère également des réécritures d’URL pour toutes les vues de magasin, y compris celles qui ont remplacé l’attribut de clé URL (mais pas le paramètre de visibilité). Auparavant, les règles de réécriture d’URL n’étaient pas appliquées lorsque les paramètres de visibilité changeaient.
