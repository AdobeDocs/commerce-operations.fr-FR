...
>[!NOTE]
>
>Les notes de mise à jour fournies pour les versions bêta sont incomplètes et peuvent être modifiées par la disponibilité générale.
...
## Environnement de test 2.4.6-beta2

Adobe Commerce 2.4.6-beta2 a été testé par rapport aux versions de composants suivantes :
...
Cette version comprend huit correctifs de sécurité et améliorations de la sécurité de la plateforme. Ces correctifs de sécurité ont été rétroportés à Adobe Commerce 2.4.5-p2 et Adobe Commerce 2.4.4-p3.
...
Adobe Commerce 2.4.6-beta2 introduit la prise en charge de PHP 8.2. PHP 8.1 reste entièrement pris en charge.  <!-- AC-6598 -->

La prise en charge de PHP 7.4 a été supprimée de cette version. Vous ne pouvez pas exécuter Adobe Commerce sur PHP 7.4.

Adobe Commerce 2.4.6-beta2 prend désormais en charge :

* Compositeur 2.2.x <!-- AC-2839 -->

* Redis 7.0.x Bien que la compatibilité avec Redis 6.2 soit maintenue, nous vous recommandons de déployer cette version avec Redis 7.0.x, car Redis 6.2 aura atteint sa fin de vie en avril 2024. <!-- AC-6606 -->

* OpenSearch est désormais pris en charge en tant que moteur de recherche par défaut pour les déploiements sur site et cloud d’Adobe Commerce. Il est pris en charge en tant que moteur de recherche indépendant pour Magento Open Source. Cette version prend en charge OpenSearch v2.x et a été testée avec OpenSearch 2.4. Bien que la compatibilité avec OpenSearch 1.x soit maintenue, nous vous recommandons d’utiliser cette version avec OpenSearch 2.x. Vous pouvez installer et exécuter Adobe Commerce et Magento Open Source 2.4.6 avec OpenSearch 2.x sur le cloud et sur site. [Prérequis du moteur de recherche](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/prerequisites/search-engine/overview.html). <!-- AC-449 3555 -->
...
* `jquery-migrate` a été supprimé du code base de commerce. [GitHub-21406](https://github.com/magento/magento2/issues/21406) <!-- AC-1199 -->
...
* **Amélioration des performances pour les opérations affectant de nombreux segments de clients**. Les vendeurs peuvent améliorer les performances des opérations impliquant plus de 100 segments de clients en activant un nouveau paramètre de configuration. Le **Vérification en temps réel de la correspondance du client avec le segment** peut être désactivé si nécessaire afin de réduire le temps de traitement en réduisant la validation de chaque segment client par rapport aux clients. Voir [Validation des segments client](https://experienceleague.adobe.com/docs/commerce-operations/performance-best-practices/configuration.html#customer-segments-validation). <!-- AC-6832 ACPT-835-->

* **Nouveau paramètre de configuration système pour la limitation du nombre de produits affichés dans la grille de produits**. Les vendeurs peuvent utiliser la nouvelle **Limiter le nombre de produits dans la grille** paramètre de configuration pour améliorer les performances de la grille de produits. Cette limite est configurée par défaut sur 20 000 et affecte uniquement les collections de produits utilisées par les composants de l’interface utilisateur. Limiter le nombre de produits affichés peut améliorer les performances des opérations pour les grilles contenant plus de 200 000 produits. Voir [Limiter le nombre de produits dans la grille](https://experienceleague.adobe.com/docs/commerce-operations/performance-best-practices/configuration.html#limit-number-of-products-in-grid)<!-- ACPT-42 -->
...
La nouvelle `adobe-ims-metapackage` La version 2.2.0 est fournie avec Adobe Commerce 2.4.6. Aucune fonctionnalité n’a été modifiée dans cette version.<!-- CABPI-505-->


### B2B

La fonction Bon de commande pour les entreprises est désormais entièrement exposée par l’API GraphQL. Cette version introduit deux schémas GraphQL :

* Le schéma GraphQL des commandes d’achat couvre les fonctionnalités de gestion des commandes d’achat de l’utilisateur de la société B2B, telles que la création, la modification et la suppression de commandes d’achat.
* Le schéma GraphQL des règles de bon de commande couvre les fonctionnalités des règles d’approbation de bon de commande, telles que l’ajout de règles d’approbation. Selon le rôle et les autorisations de l’entreprise, les utilisateurs de l’entreprise peuvent créer, modifier, supprimer ou afficher des règles d’approbation.

Cette version comprend plusieurs correctifs de bogues. Les notes de mise à jour bêta pour B2B sont incluses dans cette rubrique. Vous trouverez les notes de mise à jour de la version 2.4.6 de la version GA d’Adobe Commerce pour B2B dans [Notes de mise à jour B2B](https://experienceleague.adobe.com/docs/commerce-admin/b2b/release-notes.html).
...
Nous corrigeons des centaines de problèmes dans le code principal Adobe Commerce 2.4.6. Vous trouverez ci-dessous un sous-ensemble des problèmes résolus inclus dans Adobe Commerce 2.4.6-beta2.
...
Cette version comprend plusieurs correctifs de bogues. Les notes de mise à jour bêta pour B2B sont incluses ici. Vous trouverez les notes de mise à jour de la version 2.4.6 de la version GA d’Adobe Commerce pour B2B dans [Notes de mise à jour B2B](https://experienceleague.adobe.com/docs/commerce-admin/b2b/release-notes.html).
...
* Le filtre Afficher mes guillemets sur la page Mes guillemets fonctionne désormais comme prévu. Auparavant, les résultats du filtre comprenaient des guillemets d’autres utilisateurs administrateurs.
...
* Les réponses de requête GraphQL incluent désormais des informations sur les produits enfants des produits en bundle dans la variable `product` de la section `options` block.
...
* La valeur exportée de **Quantité maximale autorisée dans le panier** correspond désormais à la valeur par défaut définie dans le paramètre de configuration Admin .
...
* Les règles de panier qui incluent des exigences relatives à la quantité configurable de produits fonctionnent désormais comme prévu. La variable `attribute_scope` pour `quote_item_qty` La condition a été remplacée par `parent`. [GitHub-35981](https://github.com/magento/magento2/issues/35981)
...
* Le nombre de stocks de storefront est désormais exact lorsque la variable **Afficher les produits en rupture de stock** configuration est activé. [GitHub-35899](https://github.com/magento/magento2/issues/35899)

<!-- AC-6384-->

* La page de catégorie n’est plus vide lorsque la variable **Autoriser tous les produits par page** est activé. [GitHub-35900](https://github.com/magento/magento2/issues/35900)

<!-- AC-6330-->

* Le **Afficher les produits en rupture de stock** l’option de configuration fonctionne désormais comme prévu lorsqu’elle est activée. Auparavant, tous les produits étaient affichés en rupture de stock sur le storefront lorsque cette option était activée sur l’administrateur. [GitHub-35898](https://github.com/magento/magento2/issues/35898)
...
* Les produits simples en rupture de stock qui sont des options pour un produit configurable s’affichent désormais comme désactivés sur le storefront lorsque **Afficher les produits en rupture de stock** est défini sur **[!UICONTROL Yes]**. Auparavant, toutes les options s’affichaient comme disponibles, en dépit de leur état et de leur disponibilité réelle.
...
* Le nombre d’éléments de catégorie est désormais exact après le filtrage sur la page Catalogue lors de la **Afficher en rupture de stock** est activée. Auparavant, le nombre de storefront des produits en rupture de stock était incorrect.
...
* Règles de catalogue basées sur des SKU avec la variable **n’est pas l’un des** fonctionne désormais comme prévu.

<!-- ACP2E-1239-->

* L’image principale du produit sur la page des détails du produit ne se déplace plus visiblement vers le bas au cours du chargement de la page lorsque `product_image_white_borders` dans le thème `view.xml` est désactivée. Auparavant, l’image du produit était visiblement déplacée vers le bas au chargement de la page en raison de paramètres de hauteur incorrects dans le code JavaScript de Fotorama.

### Produits configurables

<!-- ACP2E-1148-->

* Les options de produit configurables s’affichent désormais comme prévu en rupture de stock sur le storefront si l’option a été créée avec une quantité définie sur 0 et **Afficher les produits en rupture de stock** est activée.
...
* Les échantillons ne s’affichent plus comme disponibles sur le storefront lorsque la quantité vendable du produit configurable est 0. [GitHub-33697](https://github.com/magento/magento2/issues/33697)
...
* Une logique de validation a été ajoutée au workflow Gestion des codes de bon afin d’appliquer les paramètres de code de bon lors de la génération des bons. Auparavant, la génération du code de bon ignorait la variable **Longueur du code** lorsque la variable **Qualité de coupon** était supérieure à 2 500.
...
* Les emails de l’ordre des administrateurs ne sont plus envoyés lorsque la variable **Confirmation de commande par e-mail** est désactivé.  [GitHub-36256](https://github.com/magento/magento2/issues/36256)
...
* L’utilisation de la variable `mixed` Le type en tant que paramètre ou type de retour est désormais pris en charge. Précédemment. Adobe Commerce a généré cette erreur PHP lorsque ce type était utilisé : `Type "mixed" cannot be nullable`. [GitHub-35579](https://github.com/magento/magento2/issues/35579)

<!-- AC-6223-->

* Le `NewRelic` la classe wrapper capture désormais toutes les variables jetables `TypeError` exceptions. [GitHub-35854](https://github.com/magento/magento2/issues/35854)
...
* Erreurs internes du serveur dans la variable `/V1/products/{sku}/links` Les points de terminaison d’API ont été résolus. [GitHub-35923](https://github.com/magento/magento2/issues/35923)

<!-- AC-6421-->

* Erreurs internes du serveur dans la variable `/V1/creditmemo/refund` Les points de terminaison d’API ont été résolus. [GitHub-35925](https://github.com/magento/magento2/issues/35925)

<!-- AC-7497-->

* `get.php` ne génère plus cet avertissement lorsque `resource_config.json` est corrompu : `Trying to access array offset on value of type null`. [GitHub-36640](https://github.com/magento/magento2/issues/36640)
...
* Les chemins de navigation de la hiérarchie de pages CMS ne s’affichent plus lorsque **Activation de la fonctionnalité de hiérarchie** est désactivée. Auparavant, les chemins de navigation s’affichaient. Lorsque les acheteurs cliquaient dessus, une page 404 s’affichait. [GitHub-35513](https://github.com/magento/magento2/issues/35513)
...
* `base64`Les URL codées ne contiennent plus de caractères réservés, tels que définis par la norme RFC 3986.  [GitHub-36042](https://github.com/magento/magento2/issues/36042)
...
* Le module de création de rapports New Relic est désormais conforme à la stratégie de sécurité du contenu et les domaines New Relic spécifiés pour la création de rapports de navigateur sont inclus dans les en-têtes CSP lors de l’activation du suivi des données de site web. [GitHub-34707](https://github.com/magento/magento2/issues/34707)
...
* Adobe Commerce ne duplique plus les messages d’erreur sur les entrées de formulaire non valides. [GitHub-36201](https://github.com/magento/magento2/issues/36201)

<!-- AC-7508-->

* Les identifications des utilisateurs administrateurs sont désormais correctement enregistrées dans la variable `admin_user` une fois le compte utilisateur admin mis à jour. [GitHub-36642](https://github.com/magento/magento2/issues/36642)

<!-- AC-7507-->

* Les informations relatives à la création de la table ont été corrigées dans la section `Authorization/README.md`. [GitHub-36644](https://github.com/magento/magento2/issues/36644)
...
* Le client `order` query renvoie désormais les commandes de tous les sites web, magasins et vues de magasin spécifiés comme prévu. Ce correctif inclut la création d’un nouvel argument de requête.
...
* Vous pouvez maintenant définir le nombre d’erreurs autorisé sur 0 auprès de l’administrateur. **[!UICONTROL System]** >  **[!UICONTROL Import]**.  [GitHub-36252](https://github.com/magento/magento2/issues/36252)
...
* L&#39;interdit `@author` a été supprimé des modules dans toute la base de code. [GitHub-36338](https://github.com/magento/magento2/issues/36338)

### Inventory management

Cette version comprend plusieurs correctifs de bogues. Les notes de mise à jour bêta pour Inventory management sont incluses ici. Vous trouverez les notes de mise à jour de la version 2.4.6 de la version GA d’Adobe Commerce pour Inventory management dans [Notes de mise à jour](https://experienceleague.adobe.com/docs/commerce-admin/inventory/release-notes.html).
...
* Le nombre de produits de la liste de catégories est désormais correct lors de l’utilisation du stock en mode source unique avec la variable **Afficher les produits en rupture de stock** activée. Adobe Commerce vérifie désormais si un produit est vendable lors du comptage.
...
* Les produits configurables ne sont plus affichés en rupture de stock sur le storefront lorsque l’une de ses variantes a été manuellement remise en stock dans les déploiements mettant en oeuvre l’inventaire multi-source.
...
* La galerie multimédia héritée est désormais désactivée par défaut. Auparavant, **Activer l’ancienne galerie de médias** a été défini sur **[!UICONTROL Yes]**.
...
* Un état de **Fermé** est désormais affecté aux commandes virtuelles comme prévu après l’émission d’un avoir et le remboursement du client. Auparavant, les produits virtuels étaient affichés sous la forme **Terminer** dans la liste déroulante État , mais **Fermé** dans le pied de page des commentaires de commande lorsque le commentaire a été ajouté à partir de l’administrateur. [GitHub-35283](https://github.com/magento/magento2/issues/35283)
...
* Les acheteurs peuvent désormais réorganiser les produits configurables avec des options personnalisées facultatives, comme prévu. Auparavant, Adobe Commerce générait cette erreur : `Some of the selected options are not currently available`. [GitHub-35409](https://github.com/magento/magento2/issues/35409)

<!-- ACP2E-1313-->

* Les remboursements de livraison sont désormais correctement calculés lorsque la variable **Appliquer une remise sur les prix** l’option de configuration est définie sur **y compris les taxes**.
...
* Les éléments de ligne sont maintenant transmis pour les transactions PayPal lorsque la variable **Envoyer les éléments de ligne** le champ de configuration est activé à partir de l’administrateur.
...
* Les acheteurs peuvent désormais utiliser le mode de paiement du Braintree dans les vitrines où **Activer les paiements par carte** a été désactivé. Auparavant, Adobe Commerce générait cette erreur JavaScript : `_adapter.js:66 Uncaught TypeError: Cannot read properties of undefined (reading 'clientToken')at Object.getClientToken (adapter.js:66:66).`
...
* Les performances de chargement de page ont été améliorées lorsque la liste de produits du widget de catalogue est chargée. Auparavant, les attributs non globaux étaient chargés directement à partir de la base de données.

### Rapports

<!-- ACP2E-852-->

* Le rapport Journal d’administration affiche désormais une valeur exacte. **Valeur avant modification** lorsqu’une règle de catégorie est mise à jour.
...
* Les recherches de chaînes entre guillemets fonctionnent désormais comme prévu lorsque la variable **Termes minimum à faire correspondre** configuration est activé. Auparavant, Adobe Commerce générait cette erreur : `{"error":{"root_cause":[{"type":"parsing_exception","reason":"[match_phrase] query does not support [minimum_should_match]' ...`.
...
* Les résultats de recherche incluent désormais des sorties d’option en rupture de stock pour tous les produits disponibles lorsque **Afficher les produits en rupture de stock** est activé [GitHub-36055](https://github.com/magento/magento2/issues/36055)
...
* Adobe Commerce honore désormais la configuration client Admin **Afficher la société** paramètre de configuration lors de l’affichage de l’adresse de livraison ou du client. Lorsque ce paramètre est désactivé, la société de livraison n’est plus incluse dans l’enregistrement de commande de la base de données, ni dans l’adresse de livraison, ni dans l’adresse du client. Auparavant, les informations s’affichaient même si le paramètre était désactivé. [GitHub-33729](https://github.com/magento/magento2/issues/33729)
...
* Un manquant `scope` a été ajoutée à `isShippingCarrierAvailable`. [GitHub-36645](https://github.com/magento/magento2/issues/36645)
...
* La devise spécifique au site web est désormais utilisée comme prévu pour les colonnes Prix spécial, Coût et MSRP dans la grille Admin Produits . Auparavant, le symbole monétaire correct ne s’affichait pas dans la grille de produit du catalogue d’administration lors du filtrage par vue de magasin pour un prix spécial.
...
* La grille Client trie désormais comme prévu dans les déploiements avec des extensions personnalisées. `Shared` est désormais défini sur `false` pour `SortOrderBuilder`. Auparavant, ce créateur provoquait des effets secondaires indésirables et le fait de cliquer sur le titre de la grille Client n’avait aucun effet. [GitHub-35939](https://github.com/magento/magento2/issues/35939)
...
* Vous pouvez maintenant créer une expédition pour les commandes qui contiennent une quantité d’article supérieure à zéro et inférieure à 1 lorsque **[!UICONTROL Quantity uses decimals]** est activée. Auparavant, lorsqu’une quantité de produit inférieure à 1 était saisie, la facture était correctement créée, mais l’envoi n’était pas créé. [GitHub-35971](https://github.com/magento/magento2/issues/35971)

<!-- AC-7510-->

* Les modèles de problèmes GitHub ont été convertis en formulaires pour les problèmes d’expérience des développeurs et les demandes de fonctionnalités.[GitHub-36651](https://github.com/magento/magento2/issues/36651)

<!-- AC-7509-->

* Le redondant `getCacheTags` a été supprimée de la méthode `Topmenu` classe . [GitHub-36643](https://github.com/magento/magento2/issues/36643)

<!-- AC-7504-->

* Ajout d’une classe pour définir le style du modal pour l’expédition des adresses. Sans cette classe spécifique, les développeurs front-end ne peuvent pas terminer le style de tout le modal sans affecter d’autres modèles dans le workflow de passage en caisse. [GitHub-36647](https://github.com/magento/magento2/issues/36647)

### URL rewrites

<!-- ACP2E-808-->

* Modification de la visibilité du produit en **Non visible individuellement** dans la portée globale Toutes les vues de magasin , supprime désormais uniquement les réécritures d’URL pour les vues de magasin qui n’ont pas remplacé l’attribut de visibilité. La modification de la visibilité du produit de cette manière génère également des réécritures d’URL pour toutes les vues de magasin, y compris celles qui ont remplacé l’attribut de clé d’URL (mais pas le paramètre de visibilité). Auparavant, les règles de réécriture d’URL n’étaient pas appliquées lorsque les paramètres de visibilité étaient modifiés.
...
Vous pouvez installer Adobe Commerce 2.4.6 à l’aide de [Compositeur](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html).