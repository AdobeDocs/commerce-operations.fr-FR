...
* La variable `MaliciousCode` a été mis à niveau pour utiliser la variable `HtmlPurifier` bibliothèque . <!-- AC-1498-->

### Améliorations de la plateforme

* **Déploiements sur site d’Adobe Commerce**: Adobe Commerce 2.4.5 a été testé et sa compatibilité avec Elasticsearch 7.17 a été vérifiée (~7.17.0 avec contrainte). Les commerçants hébergeant Adobe Commerce sur site peuvent utiliser Elasticsearch ou OpenSearch 1.2. <!-- AC-2425-->
...
La variable `laminas/laminas-session`, `laminas/laminas-text`, et `laminas/laminas-view` les dépendances ont été supprimées. <!-- AC-2598-->
...
* La variable `Froogaloop` La bibliothèque a été remplacée par Vimeo `Player.js` bibliothèque (2.16.4). <!-- AC-2527 2262-->

* La variable `grunt-eslint` (NPM) La bibliothèque a été mise à niveau vers la dernière version. <!-- AC-2700-->

* La variable `jQuery Storage` Les bibliothèques ont été remplacées par `julien-maurel/js-storage`. <!-- AC-2627-->

* La variable `php-cs-fixer` et `phpcs` les outils d’analyse de code statique sont désormais compatibles avec PHP 8.x. <!-- AC-2259-->
...
* La variable `jarallax.js` et `jaralax-video.js` Les bibliothèques ont été mises à jour afin d’utiliser la dernière version de l’API REST Vimeo. <!-- AC-2590-->
...
Activer le nouveau **Activation de l’attribution du prix direct des produits du catalogue partagé** l’option de configuration améliore également les performances de l’indexeur de prix du produit. <!-- MCP-946-->
...
* La variable `products` query ne renvoie plus `price_range` pour les produits configurables affectés par les variantes désactivées. [GitHub-33629](https://github.com/magento/magento2/issues/33629)
...
* La variable `cart` query ne renvoie désormais qu’un seul mode de paiement pour les commandes gratuites. Auparavant, tous les modes de paiement principaux étaient renvoyés dans la réponse de requête. [GitHub-34036](https://github.com/magento/magento2/issues/34036)
...
* Adobe Commerce affiche désormais les enregistrements de la banque demandée sur la page de grille des notes de crédit dans les déploiements exécutant PHP 7.4. Auparavant, Adobe Commerce générait l’erreur suivante après avoir créé une note de crédit et tenté de l’afficher : `The store that was requested wasn't found. Verify the store and try again`.
...
* La variable `cart` ne renvoie plus tous les principaux modes de paiement pour les commandes gratuites. [GitHub-34036](https://github.com/magento/magento2/issues/34036)
...
* La variable **Heures utilisées** la valeur d’une remise n’est plus incrémentée deux fois lorsqu’un client invité passe une commande avec une remise et enregistre ensuite un compte.
...
* La variable `GET /V1/returnsAttributeMetadata` le point de fin fonctionne désormais correctement lorsque la variable `rma_item` l’identifiant du jeu d’attributs par défaut de l’entité diffère de l’identifiant d’installation par défaut. Auparavant, ce point de terminaison renvoyait un résultat vide si ces identifiants différaient.

<!-- ACP2E-247-->

* La variable **Utiliser la valeur par défaut** , qui est utilisée pour activer la RAM sur la page de modification du produit, fonctionne désormais comme prévu pour le magasin par défaut. Auparavant, la case à cocher était décochée immédiatement après l’enregistrement du produit.
...
* La variable **Magasin** >  **Configuration** > **Services** La page affiche désormais les informations de l’API Web Adobe Commerce comme prévu lorsque l’accès à la ressource est défini sur  **Personnalisé** dans l’onglet Ressources du rôle . [GitHub-35506](https://github.com/magento/magento2/issues/35506)
...
* Les requêtes Elasticsearch fonctionnent désormais comme prévu lorsque `int` est configuré comme serveur principal pouvant faire l’objet d’une recherche. `type` attribut. Auparavant, Adobe Commerce lançait une `Elasticsearch\Common\Exceptions\BadRequest400Exception` exception.
...
* La variable `custom_design_to` La valeur d’attribut est désormais mise à jour comme prévu lorsqu’une mise à jour planifiée est modifiée. Auparavant, deux mises à jour distinctes de conception de catégorie sans heure de fin étaient traitées comme une seule. Lorsque l’une des mises à jour planifiées a été supprimée alors que la première était en cours d’exécution, la variable `custom_design_from` la valeur de temps est devenue supérieure à la valeur `custom_design_to` valeurs d’heure dans la prochaine mise à jour planifiée.
...
* La variable `vendor/magento/module-catalog-staging/Setup/Patch/Data/MigrateCatalogProducts.php` le correctif de données n’échoue plus lorsque la base de données inclut un produit avec des options personnalisées et le `special_from_date` est défini.
...
Vous pouvez installer Adobe Commerce 2.4.5 en utilisant [Compositeur](../../../installation/composer.md).
