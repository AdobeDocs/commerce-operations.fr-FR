...
* `Braintree_disabled_partial_capture_for_applepay_googlepay.zip`. Ce correctif résout l’impossibilité pour les utilisateurs de créer des factures partielles lors de l’utilisation d’Apple Pay ou de Google Pay-through Braintree comme mode de paiement. Voir [Adobe Commerce 2.4.4 : Impossible de créer des factures partielles](https://support.magento.com/hc/en-us/articles/4487952754957-Adobe-Commerce-2-4-4-Unable-to-create-partial-invoices) Article de la base de connaissances.

* `AC-2655_2.4.4.patch.zip`. Ce correctif résout un problème en raison duquel les acheteurs ne peuvent pas ajouter un produit à leur panier lorsque l’option Autoriser les pays n’est pas sélectionnée. Voir [Les utilisateurs ne peuvent pas ajouter de produit au panier si rien n’est sélectionné dans Autoriser les pays](https://support.magento.com/hc/en-us/articles/5377681213325-Users-not-able-to-add-product-to-cart-if-nothing-selected-in-Allow-Countries) Article de la base de connaissances.
...
* **Déploiements sur site d’Adobe Commerce**: Adobe Commerce 2.4.5 a été testé et sa compatibilité avec Elasticsearch 7.17 a été confirmée (~7.17.0 avec contrainte). Les commerçants hébergeant Adobe Commerce sur site peuvent utiliser Elasticsearch ou OpenSearch 1.2. <!-- AC-2425-->

* **Déploiements hébergés sur le cloud Adobe Commerce**: Elasticsearch 7.11 ou version ultérieure n’est pas pris en charge sur les déploiements hébergés dans le cloud Adobe Commerce 2.4.5. OpenSearch est le moteur de recherche par défaut pour les déploiements cloud Adobe Commerce 2.4.5.
...
Les performances de l’index Principal ont été améliorées en réduisant le nombre d’index Principaux de 3 à 1 pour le `catalog_product_index_price_tmp` table. Cette amélioration a réduit le nombre d’enregistrements créés dans l’index de prix en réduisant la multiplication des eSKU résultant de catalogues partagés. Le temps d’indexation a été considérablement réduit. <!-- MCP-653 MC-43342-->
...
Activation de la nouvelle **Activation de l’attribution du prix direct des produits du catalogue partagé** l’option de configuration améliore également les performances de l’indexeur de prix du produit. <!-- MCP-946-->
...
Les améliorations de performances de GraphQL incluent :

* Les développeurs et les administrateurs expérimentent une reconstruction plus rapide du schéma GraphQL de storefront unifié lors du déploiement ou lors de la modification des attributs en production. Les acheteurs connaissent également des vitesses de chargement de page beaucoup plus rapides lorsque le schéma GraphQL doit être reconstruit pour n’importe quelle raison.

* Ajout d’une fonctionnalité permettant d’utiliser la date/l’heure d’expiration du jeton d’autorisation via l’utilisation de jetons web JSON (JWT) dans l’API GraphQL.

* Le `bin/magento config:set graphql/session/disable 1` La commande permet aux commerçants de désactiver complètement la création de cookies de session pour toutes les opérations GraphQL. Par défaut, Adobe Commerce crée ces cookies et s’appuie sur ces cookies pour les autoriser, ce qui affecte les performances. À l’avenir, nous vous recommandons d’utiliser des jetons comme seule forme d’autorisation pour les requêtes GraphQL. Il est déconseillé d’utiliser des cookies de session seuls ou conjointement avec des jetons d’autorisation. Voir [Autorisation GraphQL](https://devdocs.magento.com/guides/v2.4/graphql/authorization-tokens.html). <!-- B2B-2204 -->

* Les cookies de session sont désormais lancés dans les opérations GraphQL à l’aide de proxies de classe uniquement lorsque cela est nécessaire. <!-- B2B-2217 -->
...
* La validation de la signature de méthode identifie désormais les modifications incompatibles dans une méthode ou une signature de constructeur. <!-- AC-2632-->
...
* La validation de la configuration d’ID vérifie désormais les références aux classes principales non-API supprimées ou obsolètes dans `di.xml` et préférences pour les classes principales et les interfaces.  <!-- AC-438-->
...
* Le cache de la page entière n’est plus affiché comme désactivé dans l’administrateur lorsque le cache Adobe Commerce est vidé et que `use_stale_cache` est activée.

<!-- AC-2410-->

* Les nouveaux marqueurs de déploiement relatifs fonctionnent désormais comme prévu lorsque le cache est vidé. [GitHub-32649](https://github.com/magento/magento2/issues/32649)
...
* Les vendeurs ne peuvent plus modifier une principale mise à jour planifiée à partir de la page de modification de l’entité.
...
* Adobe Commerce affiche désormais les attributs configurables comme prévu lors de la création d’une `select` attributs via un script de correctif. Auparavant, les attributs globaux éligibles étaient masqués.
...
* Adobe Commerce affiche désormais correctement l’état du stock d’un produit configurable, les configurations disponibles et le prix affiché lorsque la disponibilité d’un produit enfant configurable est modifiée par une mise à jour planifiée. Auparavant, lorsqu’un produit configurable avait deux enfants et que la disponibilité d’un produit enfant était désactivée avec une mise à jour planifiée, l’état du stock du produit parent était incorrectement affiché comme **En rupture de stock** lorsque la mise à jour était principale, une fois la mise à jour terminée, le prix affiché et la disponibilité de ses configurations ont été mal calculés.
...
* Adobe Commerce n’affiche plus un `preg_replace()` sur l’administrateur. Le troisième argument (`$subject`) est désormais de type `array|string` au lieu de `bool`.
...
* Adobe Commerce ne déclenche plus une `trim(): Passing null to parameter #1 ($string) of type string is deprecated` lorsque la connexion AMPQ est configurée sans configuration SSL.
...
* Adobe Commerce ne renvoie plus d’erreur lorsque vous activez la variable  **Cliquez ici pour lier un flux RSS à votre liste de souhaits** avant de cliquer sur **Liste de souhaits de partage**. [GitHub-34998](https://github.com/magento/magento2/issues/34998)
...
* `products` les requêtes renvoient désormais correctement les données de produit qui contiennent des produits de carte-cadeau avec une `gift-card` Clé URL.
...
* Le `categories` ne renvoie plus d’exception lors de la récupération d’une liste de catégories dont l’une contient une image introuvable sur le système de fichiers. Auparavant, Adobe Commerce lançait cette exception : `Category image not found`. [GitHub-34266](https://github.com/magento/magento2/issues/34266)
...
* Les sessions Adobe Commerce ne se terminent plus une fois qu’une requête GraphQL a été effectuée. Auparavant, la variable `ClearCustomerSessionAfterRequest` module externe déconnecté de l’acheteur. [GitHub-34550](https://github.com/magento/magento2/issues/34550)

<!-- AC-818-->

* Le `products` query ne renvoie plus `price_range` pour les produits configurables affectés par les variantes désactivées. [GitHub-33629](https://github.com/magento/magento2/issues/33629)
...
* `setShippingAddressesOnCart` les demandes valident désormais les identifiants de région. Auparavant, Adobe Commerce générait une erreur lorsque vous utilisiez l’identifiant de région plutôt que le code de région.
...
* Adobe Commerce importe désormais correctement les images avec des noms de fichiers longs. Auparavant, Adobe Commerce n’importait pas l’image et générait cette erreur : `Imported resource (image) could not be downloaded from external resource due to timeout or access permissions in row(s):`.
...
* Les actions du client, de l’adresse du client et de la commande sont désormais correctement consignées dans le rapport des actions d’administration. Auparavant, Adobe Commerce ne consignait pas les actions si la variable `postDispatch` n’avait pas été spécifié dans les paramètres de configuration.
...
* Adobe Commerce affiche désormais les enregistrements de la banque demandée sur la page de grille des notes de crédit dans les déploiements exécutant PHP 7.4. Auparavant, Adobe Commerce envoyait l’erreur suivante après avoir créé une note de crédit et tenté de l’afficher : `The store that was requested wasn't found. Verify the store and try again`.
...
* Les vendeurs peuvent désormais soumettre un remboursement partiel pour les commandes payées avec Apple Pay par le biais du Braintree. Il s’agissait d’un problème connu dans Adobe Commerce 2.4.4.
...
* Le **Temps utilisé** la valeur d’une remise n’est plus incrémentée deux fois lorsqu’un client invité passe une commande avec une remise et enregistre ensuite un compte.
...
* Le `GET /V1/returnsAttributeMetadata` le point de fin fonctionne désormais correctement lorsque la variable `rma_item` l’identifiant du jeu d’attributs par défaut de l’entité diffère de l’identifiant d’installation par défaut. Auparavant, ce point de terminaison renvoyait un résultat vide si ces identifiants différaient.

<!-- ACP2E-247-->

* Le **Utiliser la valeur par défaut** , qui est utilisée pour activer la RAM sur la page de modification du produit, fonctionne désormais comme prévu pour le magasin par défaut. Auparavant, la case à cocher était décochée immédiatement après l’enregistrement du produit.
...
* Les performances de passage en caisse ont été améliorées lorsque des points de récompense sont appliqués. When `entity` est nul, la requête de base de données ne sera pas exécutée car l’interrogation de la variable `magento_reward_history` n’est pas obligatoire dans les circonstances.

<!-- ACP2E-674-->

* Les notes de crédit pour les commandes d’invités ne recherchent plus `customer_id`, qui ignore l’interrogation de la variable `magento_reward_history` table de base de données pour les points de récompense. Cette modification a amélioré les performances.

### Rôles

* Le **Magasin** >  **Configuration** > **Services** La page affiche désormais les informations de l’API Web Adobe Commerce comme prévu lorsque l’accès à la ressource est défini sur  **Personnalisé** dans l’onglet Ressources du rôle . [GitHub-35506](https://github.com/magento/magento2/issues/35506)
...
* Les requêtes Elasticsearch fonctionnent désormais comme prévu lorsque `int` est configuré en tant que serveur principal pouvant faire l’objet d’une recherche. `type` attribut. Auparavant, Adobe Commerce lançait une `Elasticsearch\Common\Exceptions\BadRequest400Exception` exception.
...
* Adobe Commerce affiche désormais un nombre précis de suggestions de résultats de recherche sur le storefront dans les déploiements où les suggestions de recherche et le **Afficher le nombre de résultats pour chaque suggestion** sont activés. Auparavant, le nombre affiché en regard des mots-clés était nul.
...
* Les principales mises à jour de planification pour une page CMS sont désormais visibles comme prévu dans la section Modifications planifiées de la page CMS.

<!-- ACP2E-81-->

* Le `custom_design_to` La valeur d’attribut est désormais mise à jour comme prévu lorsqu’une mise à jour planifiée est modifiée. Auparavant, deux mises à jour distinctes de conception de catégorie sans heure de fin étaient traitées comme une seule. Lorsque l’une des mises à jour planifiées a été supprimée alors que la première était en cours d’exécution, la variable `custom_design_from` la valeur de temps est devenue supérieure à `custom_design_to` valeurs d’heure dans la prochaine mise à jour planifiée.
...
* Le `vendor/magento/module-catalog-staging/Setup/Patch/Data/MigrateCatalogProducts.php` le correctif de données n’échoue plus lorsque la base de données inclut un produit avec des options personnalisées et le `special_from_date` est défini.
...
* Le journal des actions en bloc affiche désormais des informations sur l’échec. `to be moved` mises à jour planifiées. Auparavant, des informations sur ces échecs étaient disponibles dans la variable `cron.log` fichier uniquement.
...
* Vous pouvez maintenant configurer une vidéo Vimeo pour qu’elle s’exécute en arrière-plan dans une `banner` élément lorsque la stratégie de sécurité du contenu est définie sur `restrict mode`. Auparavant, Adobe Commerce renvoyait une erreur JavaScript.
...
* La mise à jour de la quantité d’un article à partir de la page de liste de souhaits met désormais à jour la quantité sur la page des détails du produit comme prévu. Adobe Commerce récupère désormais la valeur mise à jour de l’URL du produit et renseigne la variable `qty` champ de la page des détails du produit de la liste des souhaits elle-même.

## Problèmes connus

**Problème**: Les utilisateurs administrateurs ne peuvent pas créer de commande ou de réorganisation pour les clients à partir de l’administrateur lorsque Braintree est activé. Lorsque l’utilisateur administrateur clique sur l’une des options suivantes : **Commande** ou **Réorganiser**, Adobe Commerce n’envoie pas la commande et la variable  `system.log` affiche cette erreur : `report.CRITICAL: Error: Call to a member function getMethodInstance() on null in /app/vendor/paypal/module-braintree-core/Block/Form.php:174`.  **Solution**:  `BUNDLE-3137-composer.patch` est désormais disponible. Voir [L’administrateur ne peut pas créer de commande/réorganisation lorsque le paiement du Braintree est activé](https://support.magento.com/hc/en-us/articles/8322266352525) Article de la base de connaissances traitant du problème et de l’accès au correctif. Les commerçants cloud doivent télécharger les correctifs de cloud pour Commerce v1.0.18. Un correctif sera également inclus dans Adobe Commerce 2.4.5-p1. <!-- AC-6222-->

**Problème**: [!DNL RabbitMQ] les éléments d&#39;infrastructure (échanges, files d&#39;attente) peuvent ne pas être initialisés après une nouvelle installation (et non une mise à niveau) d&#39;Adobe Commerce 2.4.5. Cette configuration incorrecte d&#39; [!DNL RabbitMQ] peut entraîner une erreur fatale lors de l’exécution d’opérations asynchrones ou de la génération d’un profil de performance. Les déploiements dans le cloud ne sont pas affectés. **Solution**: Les commerçants effectuant une nouvelle installation d’Adobe Commerce 2.4.5 sur site doivent s’exécuter. `bin/magento setup:upgrade --keep-generated` après utilisation `bin/magento setup:config:set` pour configurer [!DNL RabbitMQ] ou installez Adobe Commerce en transmettant [!DNL RabbitMQ] aux options de configuration de `setup:install` . Voir [[!DNL RabbitMQ]](../../../installation/prerequisites/rabbitmq.md).
...
Vous pouvez installer Adobe Commerce 2.4.5 à l’aide de [Compositeur](../../../installation/composer.md).
