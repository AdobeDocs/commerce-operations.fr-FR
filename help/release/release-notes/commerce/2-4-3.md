...
* Les dépendances des bibliothèques Laminas ont été mises à niveau vers les versions compatibles avec PHP 8.x. Certaines dépendances redondantes ont été supprimées du fichier `composer.json`. **Adobe Commerce 2.4.3 utilise Laminas 3.4.0**. <!-- MC-39513-->
...
* **Enregistrements de cadeau**. Les tâches couvertes sont les suivantes : <!-- PWA-1290-->

   * Utilisez la [requête de registre des cadeaux](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/queries/gift-registry/) pour renvoyer le contenu des registres des cadeaux du client.

   * Recherchez des registres de cadeaux par [type](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/queries/type-search/), [email](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/queries/email-search/) ou [ID](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/queries/id-search/).

   * [Créez](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/create/), [mettez à jour](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/update/) ou [supprimez](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/remove/) des registres de cadeaux.

   * [Mettre à jour](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/update-items/) ou [supprimer](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/remove-items/) des éléments d’un registre des cadeaux.
   ...
   * [Ajoutez ](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/add-registrants/), [mettez à jour](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/update-registrants/) ou [supprimez](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/remove-registrants/) les inscrits d&#39;un registre de cadeaux.

   * [Partager](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/share/) un registre des cadeaux avec les invités.

* **Guillemets négociables**. Voir les requêtes [`negotiableQuote`](https://developer.adobe.com/commerce/webapi/graphql/schema/b2b/negotiable-quote/queries/quote/) et [`negotiableQuotes`](https://developer.adobe.com/commerce/webapi/graphql/schema/b2b/negotiable-quote/queries/quotes/) . <!-- PWA-1292-->
...
* **Chemins partagés**. La [ requête d’itinéraire ](https://developer.adobe.com/commerce/webapi/graphql/schema/products/queries/route/) et l’ [ interface livrable ](https://developer.adobe.com/commerce/webapi/graphql/schema/products/interfaces/routable/) prennent en charge les demandes de routage sur les pages de produit, de catégorie et de CMS. La requête `urlResolver` est obsolète et sa fonctionnalité a été remplacée par la requête `route`.
...
* Les segments client sont désormais automatiquement mis à jour après le passage d’une commande depuis l’administrateur. Auparavant, après avoir créé une commande Admin, l’utilisateur Admin devait actualiser manuellement les données de segment connexes en accédant aux segments de clients associés et en cliquant sur le bouton **Actualiser les données de segment** .
...
* Les problèmes de performances de la base de données qui résultent des règles de segmentation des clients utilisant la condition `Product was Ordered` ont été résolus.
...
* L’utilisation de `bin/magento setup:db-schema:split-sales` pour déplacer des tables de base de données fonctionne désormais comme prévu. Les tableaux sont désormais créés et remplis correctement avec des données. Auparavant, comme la vérification de l’existence de la table était incorrecte, l’application générait des erreurs `SQLSTATE` lors de l’exécution de `bin/magento setup:db-schema:split-sales`.
...
* Les modules externes Adobe Commerce globaux (par exemple, `webapi_rest` et `graphql` ) ne sont plus déclenchés pour un nouveau type de zone personnalisée lorsque le `di.xml` de ce fichier de zone ne contient aucun module externe enregistré.
...
* Les frais de paiement en attente sont désormais retirés d’une carte-cadeau comme prévu lorsqu’une carte-cadeau est annulée. Auparavant, l’application entraînait une erreur lorsque `cron` s’exécutait après la suppression de la carte-cadeau.

<!-- MC-37658-->

* Les acheteurs peuvent désormais supprimer des cartes-cadeaux d’une commande sur la page Commande de révision lors de l’extraction avec plusieurs adresses. Auparavant, l’application ne supprimait pas la carte-cadeau de la commande lorsque l’acheteur sélectionnait la carte, puis cliquait sur **Supprimer**.
...
* Les valeurs **Date de l’événement** sont désormais les mêmes sur le storefront et sur l’administrateur lors de la création ou de la modification du registre des cadeaux. Auparavant, chaque fois que vous modifiez et enregistrez le registre des cadeaux, l’application ajustait la **date de l’événement** un jour en arrière sur le storefront et l’administrateur. [GitHub-466](https://github.com/magento/partners-magento2ee/issues/466)
...
* La requête [`dynamicBlocks`](https://developer.adobe.com/commerce/webapi/graphql/schema/store/queries/dynamic-blocks/) renvoie le contenu des blocs dynamiques qui correspondent aux filtres spécifiés.
...
* L’application ne renvoie plus d’erreur lorsqu’un module externe est ajouté à une classe parent. (Le paramètre `optionsProvider` est désormais déclaré après `getContentIdentities` dans `Assest.php`.) Auparavant, l’application lançait cette erreur : `Error: Cannot instantiate interface Magento\Framework\Data\OptionSourceInterface`. [GitHub-31291](https://github.com/magento/magento2/issues/31291)
...
* Les administrateurs qui sont limités à un site web spécifique peuvent désormais créer une livraison pour une commande passée sur le même site web. Auparavant, l’application renvoyait cette exception lorsqu’un administrateur dépourvu d’autorisation d’accès à la vue de magasin par défaut tentait d’envoyer une commande qui était placée dans une vue de magasin à laquelle l’administrateur avait accès : `Notice: Undefined offset: 1 in /app/code/Magento/Catalog/Model/Product/Attribute/Backend/GroupPrice/AbstractGroupPrice.php on line 293`.
...
* Les crochets ajoutés aux chaînes ne sont plus ignorés lors de la configuration de la traduction intégrée. Auparavant, la méthode `escapeHtmlAttr` convertissait les crochets en codes d’entité HTML. [GitHub-32000](https://github.com/magento/magento2/issues/32000)
...
* Les chaînes de texte dans le modèle qui sont utilisées pour gérer les méthodes de paiement stockées (**Mon compte** > **Méthodes de paiement stockées** ) sont désormais traduisibles. [GitHub-31211](https://github.com/magento/magento2/issues/31211)
...
* Les problèmes d’alignement avec les onglets sur la page d’invitation de l’administrateur (Admin **Marketing** > **Ventes privées** > **Invitations**) ont été résolus. Auparavant, l’onglet des invitations qui avaient été acceptées par un client n’était pas aligné. [GitHub-333](https://github.com/magento/partners-magento2ee/issues/333)

<!-- MC-38226 magento/magento2#32008-->

* La page de révision des commandes affichée lors du passage en caisse avec PayPal Express Checkout se charge désormais correctement. Auparavant, le modèle contenait le bouton **Mettre à jour la méthode de diffusion** inutilisé, qui n’était que partiellement masqué par le script lors du rendu de la page. [GitHub-32006](https://github.com/magento/magento2/issues/32006)
...
* Le widget Lien de noeud de hiérarchie CMS dans TinyMCE effectue désormais le rendu des liens de hiérarchie comme prévu lorsque le champ **Texte personnalisé de l’ancre** est vide.
...
**Problème** : les requêtes GraphQL `category` et `categoryList` ignorent les autorisations de catégorie qui déterminent si les catégories d’un catalogue partagé sont masquées ou exposées. Le storefront affiche actuellement toutes les catégories qui ont été affectées ou non au catalogue partagé. Cela affecte les déploiements d’Adobe Commerce 2.4.3 qui implémentent une vitrine de PWA pour laquelle les catalogues partagés B2B ont été activés. <!-- MC-42528-->
