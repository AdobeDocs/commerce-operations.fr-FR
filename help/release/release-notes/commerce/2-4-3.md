...
Voir [Notes de mise à jour d’Adobe Commerce 2.4.2-p2](../security/2-4-2-p2.md) pour plus d’informations sur Adobe Commerce 2.4.2-p2.
...
* La bibliothèque TinyMCE v3 obsolète a été supprimée. Le `Magento_Tinymce3Banner` module et tests MFTF liés à TinyMCE v3.x ont été supprimés d’Adobe Commerce. <!-- MC-42199 42170 -->
...
* **Listes de souhaits**. Le [mutation addWishlistItemsToCart](https://devdocs.magento.com/guides/v2.4/graphql/mutations/add-wishlist-items-to-cart.html) déplace les éléments de la liste de souhaits spécifiée vers le panier du client. <!-- magento/partners-magento2ee#449-->
...
* **Enregistrements de cadeaux**. Les tâches abordées sont les suivantes : <!-- PWA-1290-->
   ...
   * [Partager](https://devdocs.magento.com/guides/v2.4/graphql/mutations/share-gift-registry.html) un registre des cadeaux avec invités.
...
* **Options partagées**. Le [requête itinéraire](https://devdocs.magento.com/guides/v2.4/graphql/queries/route.html) et [RoutableInterface](https://devdocs.magento.com/guides/v2.4/graphql/interfaces/routable-interface.html) prennent en charge les demandes de routage sur les pages de produit, de catégorie et de CMS. Le `urlResolver` est obsolète et sa fonctionnalité a été remplacée par la fonction `route` requête.

Voir [Guide du développeur GraphQL](https://devdocs.magento.com/guides/v2.4/graphql/) pour plus d’informations sur ces améliorations.
...
Le créateur de pages est désormais disponible en tant qu’extension regroupée dans Magento Open Source. Il s’agit désormais de l’outil d’édition de contenu par défaut d’Adobe Commerce 2.4.3 et de Magento Open Source 2.4.3. Il peut remplacer l’éditeur WYSIWG par tout module tiers.
...
* [Amazon Pay](https://docs.magento.com/user-guide/payment/amazon-pay.html). Amazon Pay a été abandonné et sera supprimé dans une version ultérieure de la version 2.4.x. La version 2.4.3 et ultérieure contiendra uniquement des mises à jour de compatibilité et des correctifs pour les bogues majeurs.
...
* Le `bin/magento setup:db:status` renvoie maintenant un message indiquant que tout est à jour après une mise à niveau réussie. Auparavant, l’application affichait cette erreur : `Declarative Schema is not up to date`.
...
* Mise à jour `sortOrder` load pour `AsyncCssPlugin`. L’application se charge désormais. `AsyncCssPlugin` before `JsFooterPlugin`. [GitHub-30882](https://github.com/magento/magento2/issues/30882)
...
* Vous pouvez désormais utiliser la variable `addProductsToCart` mutation pour ajouter un produit en regroupement avec plusieurs options de case à cocher à un panier.
...
* Le prix d’un article en lot peut désormais être défini sur 0,00. Auparavant, lorsque vous reveniez à la page de modification après avoir défini le prix sur 0,00, le prix renvoyait à sa valeur par défaut. [GitHub-32383](https://github.com/magento/magento2/issues/32383)
...
* L’application affiche désormais le même prix total que celui attendu sur la page Panier et dans l’étape d’expédition du workflow de passage en caisse une fois le prix d’une option de lot modifié.
...
* Le `varnish6.vcl` a été mis à jour afin de contourner la mise en cache de la page client.
...
* `_.isEmpty()` archiver la variable `defaultCaptcha.js` terminé. Auparavant, ces vérifications n’étaient pas terminées et, par conséquent, le chargement de la page de passage en caisse échouait après la mise à niveau. [GitHub-31641](https://github.com/magento/magento2/issues/31641)
...
* Produits avec une option personnalisable `(File)` incluez désormais des liens principaux comme prévu tout au long du processus de passage en caisse multi-expédition. Auparavant, ce lien était absent. [GitHub-31095](https://github.com/magento/magento2/issues/31095)
...
* Les acheteurs peuvent désormais ajouter un produit à leur panier dont le Prix minimum de publicité (MAP) dépasse le prix normal du produit.
...
* Ajout de la fonction `itemResolvers` argument vers le catalogue `di.xml` fichier . Par conséquent, le passage en caisse n’est plus interrompu si les modules de produits configurables et regroupés sont désactivés. [GitHub-30860](https://github.com/magento/magento2/issues/30860)
...
* Le **Ajouter au panier** dans la vue de liste de catégories, fonctionne désormais comme prévu. [GitHub-32232](https://github.com/magento/magento2/issues/32232)
...
* Les produits sont désormais affichés en rupture de stock sur le storefront lorsque la quantité vendable sur l’administrateur est 0. Auparavant, ces produits étaient répertoriés en stock sur le storefront et l’application affichait une principale variable **Ajouter au panier** bouton . [GitHub-31117](https://github.com/magento/magento2/issues/31117)
...
* Le `product` requête ne remplace plus les valeurs par défaut pour toutes les vues de magasin dans un déploiement multi-magasin lorsqu’un nom de produit est mis à jour pour une seule vue de magasin. [GitHub-31083](https://github.com/magento/magento2/issues/31083)
...
* Le `products` la requête renvoie désormais les valeurs actuelles lorsqu’une règle de prix de catalogue s’applique à un article. [GitHub-26738](https://github.com/magento/magento2/issues/26738)
...
* Vous ne pouvez plus modifier la portée de la variable `media_gallery` attribut. Auparavant, lorsque vous modifiez la portée de la variable `media_gallery` de retour à `global`, l’application a généré une erreur.
...
* Le `.editorconfig` a été restructuré afin de corriger le formatage automatique de la variable `db_schema_whitelist.json` fichiers . [GitHub-31171](https://github.com/magento/magento2/issues/31171)
...
* Les frais de paiement en attente sont désormais retirés d’une carte-cadeau comme prévu lorsqu’une carte-cadeau est annulée. Auparavant, l’application générait une erreur lors de la `cron` s’exécute après la suppression de la carte cadeau.
...
* Le `addRequisitionListItemsToCart` La requête renvoie maintenant le montant d’une carte-cadeau personnalisée comme prévu lorsque la requête ne contient pas de valeur pour `allow_open_amount`.
...
* Le `setGiftOptionsOnCart` la mutation crée désormais correctement des messages-cadeaux. [GitHub-388](https://github.com/magento/partners-magento2ee/issues/388)
...
* La réponse à la variable `{ category(id: 2){ children { name children { name } } } }` La requête GraphQL inclut désormais une arborescence de catégories correctement triée.

<!-- MC-41030-->

* Le `CartItemPrices` contient désormais le nouveau champ GraphQL. `fixed_product_taxes`, qui renvoie un tableau des taxes fixes sur les produits appliquées à un article de panier. Auparavant, les taxes sur les produits fixes appliquées à un article de panier n’étaient pas incluses dans la requête de panier.

<!-- MC-40920-->

* Les requêtes vides envoyées à GraphQL renvoient désormais le code de réponse 200 au lieu de 500. Auparavant, l’analyseur GraphQL envoyait une exception avant que le résultat de la requête ne soit généré.

<!-- MC-40867 magento/partners-magento2ee#532 -->

* Le `applyRewardPointsToCart` mutation met à jour maintenant le total général d&#39;un panier pour refléter précisément la valeur des points de récompense déduits. [GitHub-486](https://github.com/magento/partners-magento2ee/issues/486)
...
* Le `categoryList` ne renvoie plus d’exception lorsqu’elle contient plusieurs fragments sur la propriété `CategoryTree` . [GitHub-31086](https://github.com/magento/magento2/issues/31086)
...
* Le `CustomizableDateValue` contient désormais l’objet `type` attribut. Sa valeur est une énumération qui peut être définie sur DATE, DATE_TIME ou TIME.

<!--  magento/partners-magento2ee#421-->

* Le [`dynamicBlocks` query](https://devdocs.magento.com/guides/v2.4/graphql/queries/dynamic-blocks.html) renvoie le contenu des blocs dynamiques correspondant aux filtres spécifiés.
...
* Correction d’une erreur liée à la fonction `country_code` dans le `createGiftRegistry` mutation.
...
* Le `addConfigurableProductsToCart` mutation renvoie désormais la miniature correcte du produit spécifié. Auparavant, elle renvoyait la miniature du produit parent.

<!-- ENGCOM-8477-->

* Le `products` n’expose plus le prix spécial d’un produit lorsque la période de prix spécial est définie pour une date ultérieure. [GitHub-30210](https://github.com/magento/magento2/issues/30210), [GitHub-29631](https://github.com/magento/magento2/issues/29631)

<!-- magento-engcom/magento2ee#576-->

* Le `customer` La réponse de requête inclut désormais la variable `total_giftcard` dans le cadre de l’attribut `OrderTotal` .

<!-- MC-41084 ENGCOM-9042 -->

* Correction d’un problème en raison duquel la variable `products` requête pour renvoyer des informations erronées sur les niveaux de prix sur les articles qui n’ont pas de niveau de prix défini. [GitHub-32279](https://github.com/magento/magento2/issues/32279)
...
* Toutes les images de produit sont désormais validées pendant l’importation. Auparavant, l’application ne validait que la première image lorsqu’un produit contenait plusieurs images. [GitHub-28236](https://github.com/magento/magento2/issues/28236)
...
* La dépendance `pelago/emogrifier` a été mis à jour de la version 3.1.0 à la version 5.0.0. Cette mise à jour a entraîné l’introduction de modifications incompatibles à l’envers dans la `Magento\Email\Model\Template\Filter` classe . Le code modifié est exécuté lors du rendu des modèles d’email. Voir [Référence BIC](https://developer.adobe.com/commerce/php/development/backward-incompatible-changes/reference/).
...
* La bibliothèque TinyMCE v3 obsolète a été supprimée. Le `Magento_Tinymce3Banner` module et tests MFTF liés à TinyMCE v3.x ont été supprimés d’Adobe Commerce.
...
* Le `composer.lock` a été mis à jour vers la dernière version de Coding Standard. [GitHub-31152](https://github.com/magento/magento2/issues/31152)
...
* Process Manager gère désormais correctement les exceptions dans les processus dupliqués. Le processus principal se ferme maintenant et l’application n’affiche désormais un message d’erreur qu’une seule fois. En outre, les exceptions des processus dupliqués sont désormais gérées lorsqu’ils sont générés dans le processus principal. Auparavant, l’application consignait plusieurs échecs d’indexation et affichait plusieurs messages d’erreur.  [GitHub-30622](https://github.com/magento/magento2/issues/30622)
...
* Le `update` La méthode pour le rôle et les règles a été marquée comme obsolète dans `app/code/Magento/Authorization/Model/Role.php`. [GitHub-30756](https://github.com/magento/magento2/issues/30756)
...
* Le `ArrayIterator` L’objet PHP a été mis à jour pour fonctionner comme prévu avec PHP 7.4. [GitHub-32088](https://github.com/magento/magento2/issues/32088)
...
* L’application ne consigne plus chaque cookie comme contexte distinct. Le `$_COOKIE` a également été converti en chaîne. Auparavant, chaque cookie étant consigné comme contexte distinct, lorsque le nombre de cookies dépassait 50, l’application consignait ce message : `Unable to send the cookie. Maximum number of cookies would be exceeded`. [GitHub-31334](https://github.com/magento/magento2/issues/31334)
...
* Le `/V1/guest-carts/examplecartid/items` L’appel renvoie maintenant la vue de magasin demandée. Auparavant, il renvoyait la première vue de magasin dans le magasin, et non la vue demandée.
...
* Le **Soyez le premier à passer en revue ce produit** change désormais comme prévu pour un nombre de révisions après qu’un administrateur a approuvé une révision.
...
* Règles de prix du panier contenant la condition Catégorie **IS NOT** sont désormais appliquées comme prévu aux produits enfants/simples configurables qui ne sont pas affectés à une catégorie mais dont les produits parents sont affectés.
...
* Les résultats de la recherche incluent désormais la variable `weight` comme prévu lorsqu’il est configuré comme pouvant faire l’objet d’une recherche.
...
* La demande affiche désormais le prix de livraison ajusté correct lorsque certains articles du panier remplissent les conditions d’expédition gratuite. Auparavant, lorsqu’un sous-ensemble d’articles du panier était qualifié pour la livraison gratuite, la demande n’ajustait pas le prix de livraison et affichait le prix de livraison complet à l’acheteur.
...
* Le `Magento_SalesRuleStaging` ne déclenche plus d’erreurs lors de la mise à niveau à partir de la version 2.3.x.
...
* Le **Utiliser la valeur par défaut** la case à cocher correspondant à un produit reste cochée après la désélection d’une mise à jour planifiée avant la mise à jour.
...
* L’application affiche désormais une **Numéro TVA** sur la page d’enregistrement du client lors de la `customer/create_account/vat_frontend_visibility` est activée. [GitHub-31326](https://github.com/magento/magento2/issues/31326)
...
* Les crochets ajoutés aux chaînes ne sont plus ignorés lorsque la traduction intégrée est configurée. Auparavant, la variable `escapeHtmlAttr` convertit les crochets en codes d’entité de HTML. [GitHub-32000](https://github.com/magento/magento2/issues/32000)
...
* L’application affiche désormais une `DateRange` filtre sur la zone Connecté de la **Client**  >  **Connexion** page. Auparavant, l’application affichait une `Text` filtre. [GitHub-30328](https://github.com/magento/magento2/issues/30328)
...
* Le `PUT /V1/products/:sku` Le point de terminaison REST génère à nouveau les réécritures d’URL de produit comme prévu. Auparavant, le point de terminaison a généré à nouveau le produit. `url_key` mais pas les réécritures d’URL. [GitHub-30316](https://github.com/magento/magento2/issues/30316)
...
* Le widget de lien de noeud de hiérarchie CMS dans TinyMCE effectue désormais le rendu des liens de hiérarchie comme prévu lorsque la variable **Texte personnalisé de l’ancre** est vide.
...
Vous pouvez installer Adobe Commerce 2.4.3 à l’aide de [Compositeur](../../../installation/composer.md).
