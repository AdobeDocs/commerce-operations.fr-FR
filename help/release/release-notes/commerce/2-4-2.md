...
* Ajout de la prise en charge des fonctionnalités B2B suivantes :
   ...
   * Crédit de la société. La [requête d’entreprise](https://developer.adobe.com/commerce/webapi/graphql/schema/b2b/company/queries/company/) inclut des détails sur l’historique de crédit de l’entreprise. <!-- MC-33862-->
...
* [Amazon Pay](https://commercemarketplace.adobe.com//amzn-amazon-pay-magento-2-module.html). Amazon Pay est obsolète dans cette version et sera supprimé dans Magento 2.5.0. Les versions 2.4.3 et ultérieures contiennent uniquement des mises à jour pour la compatibilité et des correctifs pour les bugs majeurs.
...
* Cette version ne permet plus de configurer une installation pour utiliser une base de données fractionnée.
...
* Le titre de la page Echec de la commande a été révisé pour en garantir l’exactitude. Auparavant, cette page était intitulée : **Nous avons reçu votre commande !**.  [GitHub-29416](https://github.com/magento/magento2/issues/29416)
...
* Une valeur `NULL` n’est plus affectée à l’adresse du client `region_id` lorsque vous importez des adresses de clients à l’aide d’un fichier CSV (`entity type = "customer address"` et `import behavior = "add/update"`) à partir duquel certaines valeurs de champ ont été supprimées.
...
* Un problème lié à `StorefrontProductListWidgetWithSharedCatalogTest` en raison duquel le test était ignoré a été résolu. [GitHub-322](https://github.com/magento/partners-magento2b2b/issues/322)
...
* L’application n’affiche plus le message `Purchase Order number is a required field` au-dessus du champ du code de coupon avant que l’acheteur ne saisisse des informations dans les déploiements où la commande fournisseur a été configurée comme seule option de paiement disponible.
...
* L’application affiche désormais le message de bienvenue comme prévu lorsqu’un acheteur se connecte à un magasin où le panier persistant est activé. Auparavant, l’application affichait la mention **Pas vous ?** message après la connexion.

<!-- MC-38269-->

* L’application n’affiche plus le **Pas vous ?** lorsqu’un client sélectionne l’option **Mémoriser mon nom** lors de la connexion à un déploiement où le panier persistant est activé.
...
* Le nombre d’évaluations affichées dans **Magasins** > **Attribut** > **Évaluation** correspond désormais au nombre d’évaluations dans un déploiement multisite. [GitHub-30258](https://github.com/magento/magento2/issues/30258)
...
* L’envoi de 0 avec le point d’entrée de prix spécial `/rest/V1/products/special-price-information` enregistre désormais le prix et renvoie le planning comme prévu.
...
* Les modifications de produit replanifiées ne sont désormais plus effacées après une exécution de `cron`.

<!-- MC-38759-->

* L’application ne renvoie plus d’erreur lorsque vous essayez de planifier une mise à jour pour une catégorie. Auparavant, l’application générait cette erreur lorsque vous cliquiez sur **Enregistrer** sur la page Planifier une nouvelle mise à jour : `Something went wrong while saving the Magento\Catalog\Api\Data\CategoryInterface`.

<!-- MC-38455-->

* L’API `rest/V1/products/special-price` peut désormais être utilisée pour planifier des prix spéciaux pour plusieurs magasins. Auparavant, l’application générait une erreur lorsqu’un commerçant planifiait une mise à jour de prix pour plusieurs magasins à la même `from` et `to` fois qu’une restauration est disponible à l’heure de début du nouveau planning.
...
* Les règles de Target pour les produits associés fonctionnent désormais lorsque la condition `is one of` utilisée pour la fonction Produits à afficher contient plusieurs conditions. Auparavant, lorsque cette condition contenait plusieurs valeurs, elle n’était pas analysée.
...
* Les valeurs d’en-tête qui incluent des URL sont désormais correctement analysées en `ApiFunctional TestFramewor`k. [GitHub-26425](https://github.com/magento/magento2/issues/26425)
...
* Les appels de `rest/all/V1/categories?rootCategoryId=2` GET `rest/all/V1/categories` et GET renvoient désormais les champs `name` et `product_count` renseignés comme prévu pour toutes les catégories de l’arborescence. Auparavant, les valeurs des champs de catégorie étaient vides. Le plug-in de résolution de nom de table a renvoyé un nom de table incorrect pour récupérer le nombre de produits par catégorie.
...
Vous pouvez installer Adobe Commerce 2.4.2 à l’aide du [compositeur](../../../installation/composer.md).
