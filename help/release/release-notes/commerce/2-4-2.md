...
* Les appels GET `rest/all/V1/categories` et GET `rest/all/V1/categories?rootCategoryId=2` renvoient désormais les champs `name` et `product_count` renseignés comme prévu pour toutes les catégories de l’arborescence. Auparavant, les valeurs des champs de catégorie étaient vides. Le plug-in de résolution de nom de table a renvoyé un nom de table incorrect pour récupérer le nombre de produits par catégorie.
