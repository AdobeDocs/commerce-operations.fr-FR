...
* Ajout de la prise en charge de plusieurs listes de souhaits. Vous pouvez utiliser GraphQL pour [créer](https://developer.adobe.com/commerce/webapi/graphql/mutations/create-wishlist.html), [supprimer](https://developer.adobe.com/commerce/webapi/graphql/mutations/delete-wishlist.html) et [renommer](https://developer.adobe.com/commerce/webapi/graphql/mutations/update-wishlist.html) listes de souhaits, ainsi que pour [déplacer](https://developer.adobe.com/commerce/webapi/graphql/mutations/move-products-between-wishlists.html) ou [copier](https://developer.adobe.com/commerce/webapi/graphql/mutations/copy-products-between-wishlists.html) éléments entre eux. <!-- MC-32356-->

* Ajout de la prise en charge des autorisations de marchandisage renvoyées (RMA). Les acheteurs peuvent [demander un retour](https://developer.adobe.com/commerce/webapi/graphql/mutations/request-return.html). Si le commerçant accepte la demande, l&#39;acheteur peut effectuer des tâches telles que [ajouter un commentaire](https://developer.adobe.com/commerce/webapi/graphql/mutations/add-return-comment.html) et [ajouter des informations de suivi](https://developer.adobe.com/commerce/webapi/graphql/mutations/add-return-tracking.html). <!-- MC-34596-->

* Ajout de la prise en charge des fonctionnalités B2B suivantes :
   * Entreprises. Vous pouvez ajouter les [administrateurs](https://developer.adobe.com/commerce/webapi/graphql/mutations/create-company.html), [utilisateurs](https://developer.adobe.com/commerce/webapi/graphql/mutations/create-company-user.html), [rôles](https://developer.adobe.com/commerce/webapi/graphql/mutations/create-company-role.html) et [équipes](https://developer.adobe.com/commerce/webapi/graphql/mutations/create-company-team.html). <!-- MC-33861-->
   * Crédit de l’entreprise. La [requête de la société](https://developer.adobe.com/commerce/webapi/graphql/queries/company.html) comprend des détails sur l’historique de crédit de la société. <!-- MC-33862-->
   * Listes de demandes d’approvisionnement. Vous pouvez [créer](https://developer.adobe.com/commerce/webapi/graphql/mutations/create-requisition-list.html), [supprimer](https://developer.adobe.com/commerce/webapi/graphql/mutations/delete-requisition-list.html) et [mettre à jour](https://developer.adobe.com/commerce/webapi/graphql/mutations/update-requisition-list.html) des listes de demande. La prise en charge inclut également la possibilité de [ajouter](https://developer.adobe.com/commerce/webapi/graphql/mutations/add-products-to-requisition-list.html), [mettre à jour](https://developer.adobe.com/commerce/webapi/graphql/mutations/update-products-in-wishlist.html), [supprimer](https://developer.adobe.com/commerce/webapi/graphql/mutations/delete-requisition-list-items.html), [copier](https://developer.adobe.com/commerce/webapi/graphql/mutations/copy-items-between-requisition-lists.html) et [déplacer](https://developer.adobe.com/commerce/webapi/graphql/mutations/move-items-between-requisition-lists.html) éléments dans une liste de demandes et d’ajouter des éléments de liste de demandes [ dans le panier](https://developer.adobe.com/commerce/webapi/graphql/mutations/add-requisition-list-items-to-cart.html). <!-- MC-34312-->

* Prise en charge des unions dans GraphQL. [GitHub-29425](https://github.com/magento/magento2/issues/29425) <!-- ENGCOM-8291 -->

* Ajout de la prise en charge des [listes de comparaison](https://developer.adobe.com/commerce/webapi/graphql/queries/compare-list.html). Les acheteurs peuvent [créer](https://developer.adobe.com/commerce/webapi/graphql/mutations/create-compare-list.html) et [supprimer](https://developer.adobe.com/commerce/webapi/graphql/mutations/delete-compare-list.html) listes de comparaison, et [ajouter](https://developer.adobe.com/commerce/webapi/graphql/mutations/add-products-to-compare-list.html) et [supprimer](https://developer.adobe.com/commerce/webapi/graphql/mutations/remove-products-from-compare-list.html) éléments dans les listes de comparaison. En outre, les acheteurs qui créent une liste de comparaison en tant qu’invités peuvent se connecter en tant que client et [conserver](https://developer.adobe.com/commerce/webapi/graphql/mutations/assign-compare-list-to-customer.html) leurs listes de comparaison.
...
* L’administrateur affiche désormais uniquement les données provenant de sites Web pour lesquels il dispose d’autorisations. Auparavant, l’administrateur affichait les données de tous les sites Web du déploiement.

<!-- MC-35027-->

* Tous les sites Web sélectionnés sont désormais stockés dans `gws_websites` quelle que soit la taille de l’identifiant du site Web. Auparavant, les ID de site Web étaient tronqués et la liste des sites Web sélectionnés n’était pas stockée dans la base de données. Les rôles utilisateur n’étaient pas correctement enregistrés et l’application affichait cette erreur lorsqu’un marchand tentait de modifier et d’enregistrer un rôle : `The "X" store ID is incorrect. Verify the store ID and try again`.

<!-- MC-37223-->

* L’application affiche désormais le nombre d’éléments correct dans les grilles d’administration pour les utilisateurs administrateurs à accès limité. Auparavant, les comptages affichés pour ces grilles reflétaient les données de tous les pays, tandis que les grilles elles-mêmes affichaient des données provenant de pays spécifiques uniquement. (Ce correctif corrige un problème lié aux segments des clients, aux règles de prix du catalogue, aux règles de prix du panier, à tous les utilisateurs, aux utilisateurs verrouillés et aux grilles de rôles utilisateur.)
...
* Les rapports avancés fonctionnent désormais comme prévu pour plusieurs environnements dans le même domaine. Auparavant, les fichiers CSV générés par les rapports avancés échouaient car les guillemets doubles étaient précédés de barres obliques inverses plutôt que d’un autre guillemet double, comme l’exige la norme CSV RFC4180.

<!-- MC-35996-->

* Le `analytics_collect_data` travail s’exécute maintenant correctement lors de l’utilisation du port par défaut ou non par défaut pour se connecter à MySQL dans `env.php`. Auparavant, `analytics_collect_data` renvoyait une erreur lorsque vous utilisiez le port autre que le port par défaut.
...
* Les règles de prix du catalogue fonctionnent désormais comme prévu avec les options personnalisées sur les pages Détails du produit. Auparavant, les prix des produits sur cette page ne reflétaient pas les règles de prix catalogue applicables. [GitHub-22856](https://github.com/magento/magento2/issues/22856)
...
* Remplacement des actions répétitives par des groupes d’action dans `CheckStaticBlocksTest`. [GitHub-30561](https://github.com/magento/magento2/issues/30561)
...
* Un problème qui `StorefrontProductListWidgetWithSharedCatalogTest` entraînait l’échec du test a été résolu. [GitHub-322](https://github.com/magento/partners-magento2b2b/issues/322)

<!-- ENGCOM-8122 -->

* Mise en œuvre `ActionGroup` pour accéder à la page Passage en caisse. [GitHub-29843](https://github.com/magento/magento2/issues/29843)
...
* Les méthodes de paiement sont maintenant chargées dans l’administration pour tous les sites Web, comme prévu dans un déploiement multi-magasins.
...
* **Rapports** > **Marketing** > **Produits dans les paniers** affiche désormais uniquement l’enregistrement dans l’étendue autorisée de l’utilisateur qui génère le rapport. Auparavant, toutes les données étaient disponibles pour toutes les portées.

<!-- MC-38074-->

* L’application utilise désormais la devise spécifiée dans l’étendue de rôle du marchand qui génère le rapport lors de l’affichage de la devise du produit dans le rapport Produits dans le panier.

<!-- MC-37934-->

* Les fonctionnalités du produit Récemment consultées et Récemment comparées affichent désormais des statistiques pour différents magasins et sites Web dans l’administration, comme prévu dans un déploiement multi-magasins. Auparavant, ces fonctionnalités ne prenaient pas en compte tous les magasins ou sites Web.
...
* Les options sélectionnées s’appliquent désormais aux éléments de ligne fractionnés lorsque vous fractionnez une RAM. Auparavant, les valeurs Motif de retour, État de l’article et Résolution n’étaient pas enregistrées.

<!-- MC-35360-->

* Vous pouvez désormais supprimer les informations de suivi et d’expédition d’un RMA lorsque JavaScript regroupement est activé.
...
* La page Nouvelle révision se charge désormais comme prévu lorsque vous cliquez sur le bouton Nouvelle révision **dans Admin****Marketing** > **Toutes les révisions**. Cliquer sur un produit sur cette page ouvre le nouveau formulaire de révision attendu pour ce produit.
...
* Elasticsearch filtre désormais les prix des produits de la même manière que MySQL. Le type `price_*`  de champ a été modifié de `float` `double` à l’index Elasticsearch pour améliorer la précision et corriger une erreur en virgule flottante.
...
* Elasticsearch ne renvoie plus d’erreur, mais n’affiche aucun produit comme prévu lorsqu’une `string` valeur est utilisée pour filtrer les produits par attribut avec un `integer` type backend OR `decimal` . Auparavant, l’application affichait cette erreur dans `exception.log`: `number format exception`.
