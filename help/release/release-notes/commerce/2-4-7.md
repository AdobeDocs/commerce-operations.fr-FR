---
title: Notes de mise à jour d’Adobe Commerce 2.4.7-bêta3
description: Découvrez les nouvelles fonctionnalités, les améliorations, les correctifs de bogues et les problèmes connus de la version 2.4.7-beta3 d’Adobe Commerce.
...
# Notes de mise à jour d’Adobe Commerce 2.4.7-bêta3
...
## Environnement de test Adobe Commerce 2.4.7-bêta3

Adobe Commerce 2.4.7-beta3 a été testé par rapport aux versions de composants suivantes :

* Chrome 118
* Compositeur 2.6 ou dernière version compatible avec PHP 8.3
* Elasticsearch 8.11 ou la dernière version disponible
* MariaDB 10.6
* MySQL 8.0
* Nginx 1.24
* OpenSearch 2.12
* PHP 8.3 et 8.2
* PHPUnit 10.x
* RabbitMQ 3.13 ou la dernière version disponible
* Redis 7.2
* Varnish 7.4.1 ou la dernière version disponible
...
* Un nouveau paramètre de configuration du cache de la page entière peut aider à atténuer les risques associés au HTTP `{BASE-URL}/page_cache/block/esi` point de terminaison . Ce point de terminaison prend en charge les fragments de contenu sans restriction et chargés dynamiquement à partir des poignées de mise en page Commerce et des structures de bloc. La nouvelle **[!UICONTROL Handles params size]** paramètre de configuration définit la valeur de ce point de terminaison. `handles` qui détermine le nombre maximal autorisé de gestionnaires par API. La valeur par défaut de cette propriété est 100. Les vendeurs peuvent modifier cette valeur à partir de l’option Admin (**[!UICONTROL Stores]** > **[!UICONTROL Settings:Configuration]** > **[!UICONTROL System]** > **[!UICONTROL Full Page Cache]** > **[!UICONTROL Handles params size]**). Voir [Configuration de l’application Commerce pour l’utilisation du vernis](https://experienceleague.adobe.com/docs/commerce-operations/configuration-guide/cache/configure-varnish-commerce.html). <!-- AC-9113 -->
...
Adobe Commerce 2.4.7-beta3 comprend les mises à niveau de plateforme suivantes :
...
Adobe Commerce 2.4.7 est toujours compatible avec PHP 8.1 à des fins de mise à niveau uniquement. PHP 8.1 n’est pas pris en charge et n’est pas recommandé. Le code de base Adobe Commerce 2.4.7-beta3, toutes les extensions regroupées, ainsi que toutes les extensions détenues par l’Adobe et tous les services SaaS sont compatibles avec PHP 8.3. <!-- AC-9668 -->

* **Prise en charge de RabbitMQ 3.11**. Cette version est compatible avec la dernière version de RabbitMQ 3.11. La compatibilité reste avec RabbitMQ 3.9, qui est pris en charge jusqu’en août 2023, mais nous vous recommandons d’utiliser Adobe Commerce 2.4.7-beta3 uniquement avec RabbitMQ 3.11.

* **Compositeur 2.6.x**. La compatibilité avec le compositeur 2.2.x demeure. <!-- AC-9663 -->

* **Prise en charge du cache 7.4 de vernis**. Cette version est compatible avec la dernière version de Varnish Cache 7.4. La compatibilité reste avec les versions 6.0.x et 7.2.x, mais nous vous recommandons d’utiliser Adobe Commerce 2.4.7-beta3 uniquement avec Varnish Cache version 7.4 ou la version 6.0 LTS. <!-- AC-9662 -->
...
Toutes les bibliothèques JavaScript et les dépendances NPM du code principal Adobe Commerce ont été mises à jour vers les dernières versions disponibles. Toutes les dépendances des bibliothèques Laminas ont été mises à jour vers la dernière version compatible avec PHP 8.3. <!-- AC-9667 9665 -->

#### Mises à niveau supplémentaires

* **Prise en charge de plusieurs coupons par commande**. Les vendeurs peuvent désormais configurer le nombre maximal de bons à appliquer par commande avec la nouvelle **[!UICONTROL Maximum number of coupons per order]** l’option de configuration. Cette valeur est définie sur 1 par défaut. La prise en charge REST de cette fonctionnalité a été ajoutée à cette version et GraphQL sera bientôt disponible.
...
Commerce 2.4.7-beta3 comprend les améliorations suivantes des performances et de l’évolutivité de Commerce :
...
Le package Adobe Stock (`adobe-stock-integration`) est désormais inclus dans le métamodèle des extensions de commerce (`extensions-metapackage`) pour prendre en charge des mises à jour plus fréquentes. <!-- CABPI-630 -->

### B2B

Cette version est compilée avec B2B 1.4.2.
...
* Mises à jour de REST et GraphQL
   ...
   * Ajout d’un point de terminaison pour fournir une couverture d’API web pour la fonctionnalité de devis lancée par le vendeur introduite dans cette version : `POST /V1/negotiableQuote/draft`. Voir [Gestion des guillemets négociables](https://developer.adobe.com/commerce/webapi/rest/b2b/negotiable-manage/).
...
* Les données du périphérique pour 3DS sont désormais vérifiées et affichées dans le panneau de contrôle du Braintree. La variable `collectDeviceData` Le paramètre est maintenant transmis comme `true` dans le `verifyCard` appelez pour 3DS. <!-- BUNDLE-3327 -->

* La page de configuration des paiements d’administration comprend désormais un lien vers les notes de mise à jour et la page des extensions du Braintree. <!-- BUNDLE-3325 -->

* Le SDK Braintree a été mis à jour vers les versions les plus récentes. <!-- BUNDLE-3323 -->

### Intégration commerciale avec Adobe IMS

Le package d’intégration Adobe IMS (`adobe-commerce/adobe-ims-metapackage`) est désormais inclus dans le métamodèle des extensions de commerce (`extensions-metapackage`) pour prendre en charge des mises à jour plus fréquentes. <!-- CABPI-630 -->

### GraphQL

Commerce 2.4.7 inclut des fonctionnalités de mise en cache GraphQL améliorées, la prise en charge du schéma GraphQL pour les attributs personnalisés, la prise en charge de l’annulation de commande sans interface utilisateur graphique et l’amélioration de la mise en cache du résolveur.
...
Inventory management (v1.2.7-beta3) fournit des outils pour gérer l’inventaire des produits. Cette fonctionnalité développée par la communauté est fournie avec le code principal Adobe Commerce et Magento Open Source.

### Paiements

* La prise en charge de GraphQL a été ajoutée pour les opérations de base pour tous les modes de paiement, à l’exception de Venmo. Les nouveaux points de terminaison GraphQL pour les services de paiement sont décrits à la section [Passage en caisse](https://developer.adobe.com/commerce/webapi/graphql/schema/checkout/). <!-- BUNDLE-3326 -->
...
* Le workflow de passage en caisse comprend désormais une section de paiement express qui fournit les paiements PayPal, Google Pay et Apple **[!UICONTROL Express]** des boutons. <!-- BUNDLE-3322 -->
...
PWA Studio v.13.3.x est compatible avec Adobe Commerce 2.4.7-beta3. Plusieurs améliorations ont été apportées à l’accessibilité. Pour plus d’informations sur les correctifs, voir [Versions PWA Studio](https://github.com/magento/pwa-studio/releases). Voir [Compatibilité des versions](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) pour obtenir la liste des versions de PWA Studio et de leurs versions principales Adobe Commerce compatibles.

### Structure de l’API web

**Utilisation de plusieurs coupons par panier**

Les nouveaux points de fin REST prennent en charge l’affichage, l’ajout et la suppression de plusieurs bons associés à un panier. <!-- LYNX-208 -->
...
Supprimez un seul code de bon d’un panier :
...
Supprimez plusieurs codes de bon d’un panier :
...
Nous avons corrigé des centaines de problèmes dans le code de base Adobe Commerce 2.4.7-beta3. Vous trouverez ci-dessous un sous-ensemble des problèmes résolus inclus dans cette version. Les correctifs inclus dans la version précédente de Commerce 2.4.7-betas sont également décrits.

### Installation, mise à niveau, déploiement

<!-- AC-10396 -->

* La manipulation inutile du cache a été supprimée du processus de configuration. Auparavant, Commerce écrivit sa configuration sur le disque inutilement lorsque `bin/magento setup:db-data:upgrade` ou `bin/magento setup:upgrade` a été exécuté, ce qui entraînait des problèmes avec certains modules lors de la configuration. [GitHub-38124](https://github.com/magento/magento2/issues/38124)
...
* Le menu déroulant des actions de la grille d’administration peut désormais pointer comme prévu sur un lien sélectionné pour **[!UICONTROL Content]** > **[!UICONTROL Pages]** (et catalogues partagés dans le module B2B). Auparavant, ce menu était essentiellement masqué et les utilisateurs administrateurs devaient faire défiler l’écran pour afficher les options. [GitHub-36710](https://github.com/magento/magento2/issues/36710)

### Analytics

<!-- ACP2E-2266 -->

* Page Détails du produit `meta itemprop="image"` et `meta itemprop="description"` Les propriétés sont désormais disponibles pour l’image du produit principal. [GitHub-37497](https://github.com/magento/magento2/issues/37497)

<!-- ACP2E-2314 -->

* La section Rapport de la grille Produits en paniers affiche désormais le prix correct des produits lorsque la variable **[!UICONTROL Catalog Price Scope]** est défini sur **[!UICONTROL Website]**. Auparavant, le prix du produit s’affichait comme nul lorsque le produit était affecté à un site web autre que le site web par défaut.
...
* Le nombre de produits de catégorie tient désormais compte correctement des produits regroupés lors de la **[!UICONTROL Display Out Of Stock*]** est activée.
...
* Commerce affiche désormais des messages informatifs lorsqu’une action de module complémentaire au panier est déclenchée. Auparavant, la variable `The requested qty is not available` ne s’affichait pas toujours comme nécessaire sur le storefront.

<!-- ACP2E-2166 -->

* Les délais de chargement des pages de passage en caisse ont été améliorés pour les clients disposant de grands carnets d’adresses. Commerce traite désormais uniquement le nombre d’adresses client spécifié dans la variable **[!UICONTROL Customer Addresses Limit]** . Auparavant, Commerce chargeait l’intégralité du carnet d’adresses.
...
* Les vendeurs peuvent désormais utiliser le carrousel de produit pour créer des blocs CMS lors de la `catalog_product_price dimensions-mode` est défini sur `website`. Auparavant, Commerce n’enregistrait pas le bloc et générait cette erreur SLQ : `report.CRITICAL: PDOException: SQLSTATE[42S02]: Base table or view not found: 1146 Table`.

<!-- AC-6847 -->

* Commerce n’affiche plus d’images en double lorsque la couleur du produit est modifiée sur la page des détails d’un produit.  [GitHub-36243](https://github.com/magento/magento2/issues/36243)
...
* Commerce n’insère plus de texte de description de produit dans la page des détails du produit. **[!UICONTROL Meta Description]** lorsque ce champ a été laissé vide. Auparavant, lorsqu’une description de produit contenait du code de HTML généré par PageBuilder ou un autre éditeur de HTML, le HTML s’affichait dans les métadonnées. **[!UICONTROL Description]** et vous ne pouviez pas laisser ce champ vide.
...
* REST `V1/products/<sku>/media` endpoint peut désormais traiter le chargement simultané de plusieurs images multimédias. Auparavant, lors du traitement simultané de plusieurs requêtes, des données incohérentes étaient créées, et Commerce envoyait une erreur lorsque ces données étaient enregistrées dans la base de données.
...
* Refactorisation du code dans l’ensemble du code base créé lors de l’entraînement de codage qui imposait que toute section de cache incluse dans la variable `app/etc/env.php` inclut une section front-end. Cette refactorisation résout les problèmes potentiels lors de la mise à niveau des déploiements de Commerce antérieurs à la version 2.4.4. [GitHub-38363](https://github.com/magento/magento2/issues/38363)
...
* La variable `var/log/system.log` ne contient plus `main.INFO error` messages. Le journal des entrées de menu a été déplacé du niveau INFO vers DEBUG. Auparavant, la variable `var/log/system.log` a été inondé de messages comme celui-ci : `Add of item with id Magento_Theme::design_config was processed … `.
...
* Échec de lecture critique de la base de données lors de la `synchronous_replication` n’est plus activé pour les connexions esclaves sur Adobe Commerce Cloud.
...
* L&#39;interdit `@author` La balise a été supprimée de la base de code, y compris le module suivant : `Magento_user-variable-wee`, `Magento_Catalog`, et `Magento_Customer`, `Magento_Wishlist`, `Magento_Review` `Tax`, et dans l’ensemble de la structure. [GitHub-37245](https://github.com/magento/magento2/issues/37245), [GitHub-37246](https://github.com/magento/magento2/issues/37246), [GitHub-37249](https://github.com/magento/magento2/issues/37249), [GitHub-37263](https://github.com/magento/magento2/issues/37263)
...
* Configuration de la connexion à la file d’attente par rubrique dans `app/etc/env.php` fonctionne désormais correctement. Auparavant, la file d’attente des alertes de produit ne se générait pas correctement lorsqu’une nouvelle file d’attente de messages était créée avec un échange personnalisé. Si la connexion à la file d’attente d’une rubrique a été configurée dans `app/etc/env.php` avec un échange personnalisé, une nouvelle liaison a été créée pour toutes les rubriques comportant cet échange personnalisé. Les vendeurs ne pouvaient pas traiter les alertes de produits.
...
* Les acheteurs peuvent désormais accéder aux pages configurées en tant que noeuds de hiérarchie lorsque les restrictions d’accès sont activées. Auparavant, une boucle de redirection infinie se produisait, et le navigateur renvoyait cette erreur : `ERR_TOO_MANY_REDIRECTS`.
...
* La règle de produit associée **Est l’un des** fonctionne désormais correctement lorsqu’elle est configurée avec la fonction **Les produits correspondent** . Auparavant, cette règle n’identifiait aucun produit.
...
* Les vendeurs peuvent désormais informer les acheteurs qu’une commande est prête à être récupérée lorsque la variable **[!UICONTROL No Manage Stock]** est activé pour un produit. Auparavant, lorsque ce paramètre était activé, Commerce affichait ce message d’erreur : `Your order is not ready for pickup`.

<!-- ACP2E-2227 -->

* Les vendeurs peuvent désormais créer une note de crédit pour les commandes à partir desquelles un produit simple associé à un produit configurable a été supprimé. Auparavant, les commerçants ne pouvaient pas créer l’avoir et Commerce envoyait une `TypeError` exception.
...
* Les vendeurs peuvent désormais charger des images portant le même nom, mais avec différentes majuscules et minuscules (mélange de caractères majuscules et minuscules) dans la galerie de médias. Auparavant, les noms de fichiers image n’étaient pas sensibles à la casse dans le Créateur de pages et les fichiers portant les mêmes noms que la casse étaient remplacés dans le stockage de médias Commerce hébergés sur des systèmes de fichiers sensibles à la casse.
...
* REST `V1/orders` endpoint applique désormais les cartes-cadeaux comme prévu lors de l’importation de commandes. Auparavant, les modifications apportées aux attributs d’extension de carte cadeau dans ce point de terminaison n’étaient pas enregistrées.
...
* Commerce gère désormais correctement les erreurs 502 de PayPal en empêchant la création de commandes lorsque cette erreur se produit. Auparavant, les commandes étaient toujours créées malgré une erreur 502 de PayPal, ce qui entraînait des incohérences entre les enregistrements Commerce et ceux de PayPal.
...
* Les acheteurs peuvent désormais sélectionner un mode de livraison tout en appliquant le code de coupon lors du passage en caisse. Auparavant, dans ces conditions, Commerce lançait une erreur et attribuait à la commande une livraison à taux fixe.
...
* Les emails d’expiration de points de récompense ne sont plus envoyés une fois les points expirés. La variable `magento_reward_balance_warning_notification` et `magento_reward_expire_points` Les tâches cron ont été restructurées afin d’accroître la flexibilité et la capacité d’envoi des emails. Auparavant, les marchands ne pouvaient pas envoyer de nombreux emails générés par ces tâches cron en peu de temps. Cela aurait pu entraîner des retards dans l’envoi de notifications urgentes.
...
* REST `POST /V1/returns` crée désormais une RAM par produit. Auparavant, cette requête pouvait contourner la validation et renvoyer plusieurs RMA par produit.

<!-- ACP2E-2450 -->

* Les attributs RMA sont désormais automatiquement ajoutés au formulaire EAV par défaut lors de la **[!UICONTROL Show on Storefront]** L’attribut RMA est défini sur **non**. Auparavant, la fenêtre contextuelle des détails de l’élément RMA était vide lorsqu’un attribut de retour personnalisé était créé et que la variable **[!UICONTROL Show on Storefront]** L’attribut RMA a été défini sur **non**.

<!-- ACP2E-2211 -->

* La valeur de la variable **[!UICONTROL Enable RMA]** correspond désormais à la valeur spécifiée dans la configuration lorsque la variable **[!UICONTROL Use Config]** sur la page de modification du produit est activée.
...
* Commerce affiche désormais la valeur correcte pour la variable **[!UICONTROL Regular Price]** lorsqu’une option personnalisée est sélectionnée pour un produit doté d’une **[!UICONTROL Special Price]** valeur lorsque **[!UICONTROL Price Display Settings]** est défini pour inclure la taxe.
...
* Comportement correct du curseur dans le registre des cadeaux **[!UICONTROL Message]** Champ de texte.
...
* Amélioration de la messagerie des erreurs qui se produisent lors du rendu du modèle. Commerce affiche désormais un message informatif. Auparavant, Commerce affichait des fuites de HTML lorsqu’une erreur PHP se produisait.
...
* Commerce n’affiche plus une `Integration not secure` avertissement dans Admin **[!UICONTROL System]** > **[!UICONTROL Extensions]** > **[!UICONTROL Integrations]** lorsque la variable **[!UICONTROL Callback URL]** et **[!UICONTROL Identity link URL]** Les champs sont vides. [GitHub-36407](https://github.com/magento/magento2/issues/36407)
...
Les problèmes connus suivants seront corrigés dans Adobe Commerce 2.4.7 GA :

**Problème**: les services natifs AWS Cloud ne sont pas compatibles avec PHP 8.3. La compatibilité avec PHP 8.3 sera introduite dans Adobe Commerce 2.4.7 GA. <!-- AC-11395 -->

**Problème**: les catégories ne sont pas visibles par les acheteurs dans les magasins où le cache est défini sur un réseau de diffusion de contenu Fastly (**[!UICONTROL Caching Application=Fastly CDN]**). Cela affecte à la fois les clients invités et les clients enregistrés.  <!-- AC-11411 -->

**Problème**: un produit n’est pas supprimé comme prévu du mini panier une fois que le catalogue partagé qui lui est affecté a été supprimé du produit. <!-- AC-8935 -->
...
Vous pouvez installer Adobe Commerce 2.4.7-beta3 à l’aide de [Compositeur](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html).
