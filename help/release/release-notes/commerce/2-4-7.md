...
[GraphQL Application Server](../../../performance/application-server.md) permet à Adobe Commerce de maintenir l’état parmi les demandes d’API Commerce GraphQL et élimine le besoin d’amorçage, ce qui se traduit par un débit plus élevé, une latence plus faible et une utilisation efficace des ressources pour toutes les API GraphQL. En partageant l’état de l’application entre les processus, les demandes d’API GraphQL deviennent nettement plus efficaces et les réponses de l’API GraphQL sont en moyenne 30% plus rapides.<!-- ACPT-27 -->
...
>[!NOTE]
>
>Le serveur d’applications GraphQL n’est actuellement pas compatible avec [[!DNL Amazon Simple Storage Service (AWS S3)]](https://aws.amazon.com/s3/). Les clients Adobe Commerce sur les infrastructures cloud qui utilisent actuellement [!DNL AWS S3] pour le [stockage à distance](../../../configuration/remote-storage/cloud-support.md) ne peuvent pas utiliser GraphQL Application Server tant qu’Adobe n’aura pas publié un correctif plus tard en 2024.<!-- ACPT-1840 -->

### Métamodule Extension Adobe Commerce

Cette version introduit le métapackage Adobe Commerce Extension v2.0.0, qui regroupe automatiquement certaines extensions Adobe Commerce avec cette version principale. Les versions de ces extensions incluses dans ce métapackage sont installées lors `composer update` de son exécution, ce qui simplifie le processus de mise à niveau de ces extensions lors de la mise à niveau vers la dernière version principale. Ces extensions gèrent des calendriers de publication indépendants.

Le métapackage Extension Adobe Commerce pour Adobe Commerce 2.4.7 inclut les extensions suivantes :
...
* La condition de sous-sélection de produit de règle du prix du panier comprend désormais l’option `total (incl.tax)` comme prévu. [GitHub-34871](https://github.com/magento/magento2/issues/34871)
...
* Le préchargement Redis n’échoue plus lorsque `exec()` renvoie false. La vérification a été ajoutée au processus de préchargement pour empêcher `array_combine` de recevoir des `false` en tant que second argument. Par conséquent, le préchargement continuera à s’exécuter lorsque `exec()` .`false` [GitHub-37509](https://github.com/magento/magento2/issues/37509)
...
* Résolution d’erreurs d’arrondi du sous-total survenues dans des transactions pour des produits pour lesquels la quantité décimale a été activée. [GitHub-37817](https://github.com/magento/magento2/issues/37817)

<!-- ACP2E-2127 -->

* Le filtre de date de la grille client Admin fonctionne désormais comme prévu en fonction des paramètres régionaux spécifiés.
...
* Ajout d’une instruction manquante `return` au contrôleur d’activation de l’utilisation de l’administrateur. Auparavant, il n’y avait pas `return` de déclaration et le contrôleur en `FrontendController` classe était mal géré. [GitHub-31374](https://github.com/magento/magento2/issues/31374)

<!-- AC-9390 -->

* Remplacement de MySQL `CREATE TEMPORARY TABLE ... LIKE` par un `CREATE TEMPORARY TABLE` nouveau format pour assurer la compatibilité avec MySQL 8.x. [GitHub-37926](https://github.com/magento/magento2/issues/37926)

<!-- ACP2E-1462 -->

* Adobe Commerce n’inclut plus le chemin d’accès complet à la racine du serveur dans le message d’erreur qu’il affiche lorsqu’une requête HTTP incorrecte a été envoyée au serveur. Seules les chaînes sont désormais transmises à la `explode` méthode en tant que deuxième argument.

<!-- AC-8384 -->

* `restricted_classes.php` a remplacé `Zend_Validate_File_Upload`. Auparavant, le programme de validation se bloquait lors du téléchargement d’un fichier d’importation supérieur à la valeur configurée dans `php.ini `. [GitHub-37281](https://github.com/magento/magento2/issues/37281)
...
* Les produits repassent désormais automatiquement au statut En stock après un avoir qui inclut un retour de la quantité de produit. Auparavant, si un produit était en rupture de stock mais qu’une note de crédit renvoyait le stock de ce produit, les commerçants devaient modifier manuellement l’état des stocks de produits.

<!-- ACP2E-2123 -->

* Amélioration des performances de l’indexeur d’inventaire en mode asynchrone en supprimant les demandes redondantes pour réindexer les éléments sources par défaut.

<!-- AC-7023 -->

* Correction des problèmes de l’indexeur d’inventaire qui affectaient la précision du nombre de stocks de vitrine des produits configurables. [GitHub-36421](https://github.com/magento/magento2/issues/36421)

<!-- ACP2E-2082 -->

* La mise à jour des articles source de stock via REST `V1/inventory/source-items` fonctionne désormais plus rapidement. L’appel ne déclenche plus de réindexation ou d’opération de nettoyage du cache sur les produits en stock inchangés.
...
* Adobe Commerce renvoie désormais l’attribut (code d’image base64) dans `GET V1/products/sku/media` les `content` réponses de l’API des médias de produits. L’attribut `content` n’apparaissait pas auparavant dans la réponse.

### Liste de souhaits

<!-- ACP2E-1784 -->

* Commerce affiche désormais un message indiquant qu’un article a déjà été ajouté à une liste de souhaits lorsqu’un acheteur tente d’ajouter le même article à une liste de souhaits plusieurs fois. Auparavant, dans ces conditions, le Commerce augmentait la quantité de produits dans la liste de souhaits.

<!-- AC-8610 -->

* Adobe Commerce ajoute maintenant avec succès des produits avec des fichiers d’options personnalisables téléchargés requis à la liste de souhaits. Auparavant, Commerce ajoutait deux produits à la liste de souhaits, et non un seul. [GitHub-37437](https://github.com/magento/magento2/issues/37437)
...
Vous pouvez installer Adobe Commerce 2.4.7 à l’aide du [compositeur](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html).
