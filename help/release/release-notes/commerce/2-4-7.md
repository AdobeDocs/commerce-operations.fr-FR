...
* **Prise en charge du cache 7.3 de vernis**. Cette version est compatible avec la dernière version du Cache Varnish 7.3. La compatibilité reste avec les versions 6.0.x et 7.2.x, mais nous vous recommandons d’utiliser Adobe Commerce 2.4.7-beta1 uniquement avec le Cache Varnish version 7.3 ou la version 6.0 LTS.

* **Prise en charge de RabbitMQ 3.11**. Cette version est compatible avec la dernière version de RabbitMQ 3.11. La compatibilité reste avec RabbitMQ 3.9, qui est pris en charge jusqu’à août 2023, mais nous vous recommandons d’utiliser Adobe Commerce 2.4.7-beta1 uniquement avec RabbitMQ 3.11.
...
* Les produits groupés sont désormais importés avec succès sans dupliquer les SKU dans les options de produit. Auparavant, Adobe Commerce créait plusieurs SKU en double dans les options de produit lorsque vous importiez des produits en lot avec des entrées en double.
...
* Les performances des règles de segmentation des clients qui reposent sur des paniers principaux ont été améliorées, ce qui a réduit le temps nécessaire pour ajouter un produit au panier.
...
* Les points de récompense peuvent désormais être remboursés après la création d&#39;un avoir. Auparavant, les marchands ne pouvaient pas rembourser les points de récompense après la création d’une note de crédit, et l’état de la commande restait inchangé. `Completed`, not `Closed`.

### RMA

<!-- ACP2E-1230 -->

* Le **[!UICONTROL Enable RMA]** au niveau du produit individuel, respecte désormais la configuration des paramètres de la RAM du magasin. Auparavant, ce paramètre était désactivé par défaut lors de l’ajout d’un produit, quels que soient les paramètres de configuration du magasin.
...
* Adobe Commerce renvoie désormais avec succès la variable `content` (code image base64) dans `GET V1/products/sku/media` réponses de l’API de média de produit. Le `content` n’apparaissait pas auparavant dans la réponse.
