---
title: Notes de mise à jour d’Adobe Commerce 2.4.7-beta1
description: Découvrez les nouvelles fonctionnalités, les améliorations, les correctifs de bogues et les problèmes connus de la version 2.4.7-beta1 d’Adobe Commerce.
...
# Notes de mise à jour d’Adobe Commerce 2.4.7-beta1
...
Adobe Commerce 2.4.7-beta1 a été testé par rapport aux versions de composants suivantes :
...
Les mises à niveau de plateforme de cette version améliorent la conformité aux dernières bonnes pratiques en matière de sécurité. Adobe Commerce 2.4.7-beta1 introduit :

* Compositeur 2.5.x. La compatibilité avec le compositeur 2.2.x demeure.  <!-- AC-8129 -->

* **Prise en charge du cache 7.3 de vernis**. Cette version est compatible avec la dernière version de Varnish Cache 7.3. La compatibilité reste avec les versions 6.0.x et 7.2.x, mais nous vous recommandons d’utiliser Adobe Commerce 2.4.7-beta1 uniquement avec Varnish Cache version 7.3 ou la version 6.0 LTS.

* **Prise en charge de RabbitMQ 3.11**. Cette version est compatible avec la dernière version de RabbitMQ 3.11. La compatibilité reste avec RabbitMQ 3.9, qui est pris en charge jusqu’en août 2023, mais nous vous recommandons d’utiliser Adobe Commerce 2.4.7-beta1 uniquement avec RabbitMQ 3.11.
...
* **Opérations d’enregistrement plus rapides des configurations au niveau du magasin pour les déploiements avec de nombreux magasins**. L’enregistrement des paramètres de configuration dans les déploiements comportant plus de 500 boutiques peut prendre du temps. La nouvelle [`Async Config`](../../../performance/configuration.md#asynchronous-configuration-save-badge-247-beta1typeinformative-urlhelpreleaserelease-notescommerce2-4-7md-tooltipavailable-in-247-beta1-only) Le module active des opérations d’enregistrement de configuration asynchrones en exécutant une `cron` qui utilise un client pour traiter l’opération d’enregistrement dans une file d’attente de messages. `AsyncConfig` is **disabled** par défaut.  <!-- ACPT-831 -->
...
Cette version introduit un nouveau serveur d’applications PHP qui est implémenté sur une extension PHP Swoole. Ce serveur d’applications permet à Adobe Commerce de conserver l’état entre les demandes de l’API Commerce GraphQL et élimine la nécessité d’amorçage. En partageant l’état de l’application entre les processus, les demandes d’API deviennent beaucoup plus efficaces et les temps de réponse de l’API peuvent diminuer de 50 à 60 millisecondes.

La variable `ApplicationServer` met en oeuvre le serveur d’applications PHP, disponible uniquement pour les déploiements sur site. Les développeurs qui souhaitent mettre en oeuvre ce serveur doivent également installer et configurer Swoole et Nginx.

Cette version bêta de ce serveur d’applications est disponible uniquement pour les déploiements sur site ; elle ne prend pas en charge les déploiements basés sur le cloud. Il ne prend pas encore en charge la fonctionnalité de GraphQL B2B. Les requêtes GraphQL peuvent ne pas fonctionner comme prévu dans les déploiements sur site lorsque cette version du serveur d’applications PHP est configurée. Voir [Serveur d’applications pour les API GraphQL](https://experienceleague.adobe.com/docs/commerce-operations/performance-best-practices/performance-best-practices/application-server.html). <!-- ACPT-27 -->

### Métaphorage de l’extension Adobe Commerce

Cette version introduit le métapaquet d’extension Adobe Commerce v2.0.0-beta1, qui regroupe automatiquement les extensions Adobe Commerce sélectionnées avec cette version principale. Les versions de ces extensions qui sont incluses dans ce métappackage sont installées lorsque `composer update` est exécuté, ce qui simplifie le processus de mise à niveau de ces extensions lors de la mise à niveau vers la dernière version de base. Ces extensions maintiennent des calendriers de publication indépendants.

Le module Adobe Commerce Extension pour Adobe Commerce 2.4.7-beta1 comprend trois extensions :

* [Bouton d’extraction rapide](https://experienceleague.adobe.com/docs/commerce-merchant-services/quick-checkout/overview.html)
...
Cette version comprend de nouvelles fonctionnalités et améliorations pour les guillemets négociables B2B, les mises à jour de l’API REST, ainsi que plusieurs correctifs de bogues.

* **Guillemets lancés par le vendeur**. Les représentants commerciaux B2B peuvent désormais lancer une citation directement à partir des grilles de devis et de clients dans l’administrateur. Voir [Créer une citation](https://experienceleague.adobe.com/docs/commerce-admin/b2b/quotes/quote-request.html)
...
* Mises à jour de l’API REST
   ...
   * Ajout d’un point de terminaison afin de fournir une couverture d’API web pour la fonctionnalité de devis lancée par le vendeur introduite dans cette version. Voir [Gestion des guillemets négociables](https://developer.adobe.com/commerce/webapi/rest/b2b/negotiable-manage/).

     `POST /V1/negotiableQuote/draft`

   * La couverture API web améliorée des fonctionnalités de storefront pour le catalogue partagé inclut de nouveaux points de terminaison pour affecter et réinitialiser des actions de prix personnalisées :

     `POST /V1/sharedCatalog/:sharedCatalogId/assignCustomPrices`
     `POST /V1/sharedCatalog/:sharedCatalogId/resetCustomPrices`

  Une nouvelle `quickorder_active` pour le champ `storeConfig` et `availableStores` requêtes. Ce champ indique si la fonction d’ordre rapide est activée.
...
Page Builder v.1.7.4 est compatible avec Adobe Commerce 2.4.7-beta1.

### PWA Studio

PWA Studio v.13.2.x est compatible avec Adobe Commerce 2.4.7-beta1. Plusieurs améliorations ont été apportées à l’accessibilité. Pour plus d’informations sur les correctifs, voir [Versions PWA Studio](https://github.com/magento/pwa-studio/releases). Voir [Compatibilité des versions](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) pour obtenir la liste des versions de PWA Studio et de leurs versions principales Adobe Commerce compatibles.

## Problèmes résolus

Nous avons corrigé des centaines de problèmes dans le code principal Adobe Commerce 2.4.7-beta1. Vous trouverez ci-dessous un sous-ensemble des problèmes résolus inclus dans cette version.
...
* La variable `customer` la mutation renvoie désormais uniquement les révisions client liées à une vue de magasin spécifique, comme prévu lorsqu’elle est filtrée par identifiant de magasin.
...
* La variable `searchProducts` query renvoie désormais toutes les agrégations disponibles (filtres) et leurs libellés correctement traduits. Auparavant, les libellés d’agrégation de prix et de catégories n’étaient pas traduits comme prévu. [GitHub-36140](https://github.com/magento/magento2/issues/36140)
...
* Les configurations autres que les configurations par défaut (portée du site web ou du magasin) ajoutées en tant que variables d’environnement n’interrompent plus la variable `app:config:import` traiter avec une erreur de récursion.
...
Les problèmes connus suivants seront corrigés dans Adobe Commerce 2.4.7-beta2 :
...
**Problème**: les produits configurables sont incorrectement répertoriés comme en rupture de stock sur le storefront lorsqu’un ou plusieurs produits enfants sont désactivés par le biais d’une mise à jour planifiée. <!-- ACP2E-2003 -->
...
**Problème**: les prix des produits en regroupement fixe n’incluent pas les prix spéciaux attribués comme prévu. <!-- ACP2E-2034 -->
...
Vous pouvez installer Adobe Commerce 2.4.7-beta1 à l’aide de [Compositeur](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html).
