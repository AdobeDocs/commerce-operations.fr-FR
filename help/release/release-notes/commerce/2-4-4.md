...
* HTTPS est désormais activé par défaut pour la vitrine Adobe Commerce. Les paramètres **Utiliser des URL sécurisées sur Storefront** et **Utiliser des URL sécurisées dans Admin** sont activés par défaut et tous les cookies intégrés sont désormais définis comme sécurisés.  <!-- AC-1173-->
...
* Améliorations apportées au processus de validation des commandes affectées par une règle de prix de panier lors du placement de commande asynchrone. <!-- MCP-304-->
...
* Les messages de notification s’affichent désormais correctement lorsque Adobe Commerce a un sous-chemin configuré dans son URL de base.

<!-- MC-42469-->

* Ajout d’un `aria-label` élément au modèle de page de vitrine pour indiquer la page vers laquelle l’acheteur accédera lorsqu’il cliquera sur un lien. Auparavant, le même texte de lien était utilisé pour les liens vers des pages différentes. [GitHub-33075](https://github.com/magento/magento2/issues/33075)

<!-- ACP2E-73-->

* Adobe Commerce n’affiche plus de message d’erreur lors du chargement de la page d’accueil d’une boutique avec un thème personnalisé préexistant. Auparavant, Adobe Commerce affichait le message suivant : `The store will not work correctly in the case when cookies are disabled`.
...
* Adobe Commerce n’affiche plus les adresses en double lorsqu’un acheteur clique **sur Modifier l’adresse** sur la page Révision et paiements dans un déploiement dans lequel le **paramètre de configuration Adresse Search** est activé.

<!-- ACP2E-136-->

* Le **bouton Ajouter des produits** est maintenant activé comme prévu pour les administrateurs disposant d’autorisations restreintes lorsqu’ils travaillent dans l’étendue de l’affichage du magasin sur la page Catégorie d’administration.

<!-- ACP2E-169-->

* Les points d’arrêt de vue sont désormais les mêmes dans Adobe Commerce.

<!-- ACP2E-199-->

* Adobe Commerce affiche désormais correctement les attributs de date client personnalisés avec les paramètres régionaux personnalisés sur la page client Admin.

<!-- MC-41760-->

* Les images ajoutées à l’aide de Page Builder ne sont plus dupliquées sur la vitrine lorsqu’elles sont affichées avec une largeur de fenêtre de navigateur de 768 px. Auparavant, avec cette largeur de fenêtre de navigateur, Adobe Commerce dupliquait les images sur le storefront en affichant la version mobile et la version bureau des images.
...
L’équipe d’ingénierie de communauté [Magento Contributors](https://magento.biterg.io/app/kibana) tient à jour une liste des principaux contributeurs individuels et partenaires par mois, trimestre et année. À partir de cette page Contributeurs, vous pouvez suivre des liens vers leurs PR fusionnés sur GitHub.
