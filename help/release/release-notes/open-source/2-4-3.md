...
## Appliquez MC-__set_rate_limit__2.4.3.patch pour résoudre le problème de limitation de débit de l’API
...
* Les dépendances de la bibliothèque Laminas ont été mises à niveau vers des versions compatibles avec PHP 8.x. Certaines dépendances redondantes ont été supprimées du fichier `composer.json`. **Magento Open Source 2.4.3 utilise Laminas 3.4.0**. <!-- MC-39513-->
...
Pour plus d’informations sur les améliorations et les correctifs, voir [Versions de &#x200B;](https://github.com/magento/pwa-studio/releases). Consultez [compatibilité](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) pour obtenir la liste des versions de PWA Studio et de leurs versions compatibles.
...
* Les appels API REST GET Cart renvoient désormais des valeurs d’attribut personnalisé correctes pour les adresses de facturation et d’expédition. Auparavant, les attributs d’adresse personnalisés s’affichaient incorrectement dans la page des détails de la commande dans la page storefront de Mon compte et dans la page Admin.
...
* Les produits sont désormais disponibles comme prévu dans les résultats de recherche de storefront lors de la liaison de produits à l’aide d’une requête REST PUT `/V1/products/:sku/links` lorsque le mode indexeur est défini sur **Mise à jour lors de l’enregistrement**.
...
* L’application ne renvoie plus d’erreur lorsqu’un module externe est ajouté à une classe parente. (Le paramètre `optionsProvider` est maintenant déclaré après `getContentIdentities` dans `Assest.php`.) Auparavant, l’application générait cette erreur : `Error: Cannot instantiate interface Magento\Framework\Data\OptionSourceInterface`. [GitHub-31291](https://github.com/magento/magento2/issues/31291)
...
* Vous pouvez désormais ajouter un mappeur de champs Elasticsearch personnalisé à `Magento\Elasticsearch\Model\Adapter\FieldMapper\Product\FieldProvider\FieldName\Resolver\CompositeResolver`.
...
* Les champs de recherche (mini-recherche de formulaire) fonctionnent désormais comme prévu lorsque les suggestions de recherche sont désactivées.
...
* L’application ne décoche plus la case **Ajouter des commentaires** lorsqu’un acheteur clique sur **Obtenir les méthodes et les tarifs d’expédition** et sélectionne une méthode d’expédition lors de la création d’une commande auprès de l’administrateur.
...
* La quantité de produits reste désormais inchangée comme prévu après qu’un acheteur a modifié la quantité sur la page Adresse d’expédition multiple et cliqué sur le bouton Précédent du navigateur.
...
* L&#39;application affiche désormais un champ **Numéro de TVA** sur la page d&#39;enregistrement des clients lorsque l&#39;`customer/create_account/vat_frontend_visibility` est activée.
...
* Suppression de la commande `cache:flush` des tests afin d’améliorer le temps d’exécution pour les modules `Catalog`, `CatalogUrlRewrite` et `LoginAsCustomer`.

<!-- ENGCOM-8499-->

* Les paramètres redondants ont été supprimés et le POST a été modifié en GET là où nécessaire dans `\Magento\Logging\Model\ProcessorTest::testLoggingProcessorLogsActionShipping`.
...
* Les chaînes de texte du modèle utilisées pour gérer les modes de paiement stockés (**Mon compte** > **Modes de paiement stockés** ) peuvent désormais être traduites.

<!-- MC-30798 ENGCOM-8717-->

* Les e-mails de commande envoyés à partir de l’administrateur utilisent désormais les paramètres régionaux du magasin et non pas les paramètres régionaux associés au compte de l’administrateur.
...
* La page de révision de commande affichée lors du passage en caisse avec PayPal Express Checkout se charge désormais correctement. Auparavant, le modèle contenait le bouton inutilisé **Mettre à jour la méthode de diffusion**, qui n’était que partiellement masqué par le script lors du rendu de la page.

<!-- MC-37941-->

* Les étoiles d’évaluation et le texte d’évaluation dans la section Avis des clients de la page produit sont désormais correctement espacés lorsque de longs noms d’évaluation sont présents.
...
* L’application utilise désormais le modèle d’e-mail personnalisé correct lors de la génération d’e-mails pour les nouveaux utilisateurs administrateurs. Auparavant, l’application utilisait l’ancien modèle par défaut, qui omettait les nom et prénom de l’administrateur.
...
Vous pouvez installer Magento Open Source 2.4.3 à l’aide du [compositeur](../../../installation/composer.md).
