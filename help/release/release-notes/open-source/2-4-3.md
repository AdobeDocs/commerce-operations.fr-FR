...
* Magento Open Source 2.4.3 a été testé et confirmé comme compatible avec Redis 6.0.12. (La version 2.4.x reste compatible avec Redis 5.x.)

* Les dépendances de la bibliothèque Laminas ont été mises à niveau vers des versions compatibles PHP 8.x. Certaines dépendances redondantes ont été supprimées du fichier `composer.json`. **Magento Open Source 2.4.3 utilise Laminas 3.4.0**. <!-- MC-39513-->
...
Cette version ajoute la couverture GraphQL pour les itinéraires partagés. La [requête](https://developer.adobe.com/commerce/webapi/graphql/queries/route.html) d’itinéraire et [RoutableInterface](https://developer.adobe.com/commerce/webapi/graphql/interfaces/routable-interface.html) prennent en charge les demandes de routage sur les pages de produit, de catégorie et CMS. La requête `urlResolver` est obsolète et sa fonctionnalité a été remplacée par la requête `route`.
...
La portée de l’outil](https://experienceleague.adobe.com/docs/commerce-operations/upgrade-guide/upgrade-compatibility-tool/overview.html) de compatibilité de mise [à niveau a été élargie en fonction des commentaires de la communauté. Rejoignez notre [#upgrade-compatibilité-outil](https://magentocommeng.slack.com/archives/C019Y143U9F) Slack pour obtenir l’assistance de l’équipe produit Adobe et de la communauté, ainsi que pour aider à orienter l’orientation future de l’outil.
...
* Les administrateurs peuvent désormais se connecter à un déploiement lorsque l’application a été installée avec l’option ou avec `web/seo/use_rewrites` la `—use-rewrites=0` valeur 0 dans `core_config_data_table`. [GitHub-32100](https://github.com/magento/magento2/issues/32100)

<!-- ENGCOM-8451 -->

* Charge mise à jour `sortOrder` pour `AsyncCssPlugin`. L’application charge désormais `AsyncCssPlugin` avant `JsFooterPlugin`. [GitHub-30882](https://github.com/magento/magento2/issues/30882)
...
* Les redirections infinies ne se produisent plus lorsque l’URL d’administration diffère de l’URL de site Web par défaut dans les déploiements où l’application est configurée pour être accessible à partir de deux URL.

### Produits groupés

<!-- MC-41810-->

* Vous pouvez désormais utiliser la `addProductsToCart` mutation pour ajouter un produit groupé avec plusieurs options de case à cocher à un panier.

<!-- MC-41614-->

* L’indexation des prix des produits groupés est désormais exécutée à l’aide de tables temporaires, ce qui évite de verrouiller des tables de base de données. Auparavant, l’application utilisait des tables physiques, ce qui entraînait le verrouillage des tables.
...
* CAPTCHA valide désormais correctement les données fournies par un acheteur, et les champs CAPTCHA sont maintenant affichés comme prévu après plusieurs tentatives infructueuses d’un acheteur de passer à la caisse avec PayPal Payflow Pro.
...
* Les mises à jour de la disposition des thèmes personnalisés sont maintenant appliquées comme prévu. Auparavant, les mises à jour de la mise en page de thème personnalisée étaient ignorées.
...
* L’application supprime désormais une image multimédia de produit après la suppression d’un produit. Auparavant, l’image multimédia du produit restait dans le dossier après la suppression réussie du produit.
...
* L’application génère désormais correctement les factures pour les commandes qui ne contiennent qu’un seul produit configurable. [GitHub-31143](https://github.com/magento/magento2/issues/31143)
...
* L’application ne génère plus d’erreurs de journal système lorsqu’un acheteur invité utilise une adresse non valide. Auparavant, l’application affichait cette erreur par intermittence au lieu de restituer la page : `No such entity with addressId`. [GitHub-15115](https://github.com/magento/magento2/issues/15115)
...
* Les produits associés qui ont été ajoutés en tant que mise à jour planifiée ne sont plus affichés sur une page de produit de vitrine après la date de fin de la mise à jour. [GitHub-469](https://github.com/magento/partners-magento2ee/issues/469)
...
* La `applyRewardPointsToCart` mutation met désormais à jour le total général d’un panier pour refléter avec précision la valeur des points de récompense déduits. [GitHub-486](https://github.com/magento/partners-magento2ee/issues/486)
...
* La dépendance `pelago/emogrifier` a été mise à jour de la version 3.1.0 vers la version 5.0.0. Cette mise à jour a entraîné l’introduction de modifications rétrocompatibles avec la `Magento\Email\Model\Template\Filter` classe. Le code modifié est exécuté pendant le rendu des modèles d’email. Voir [la référence](https://developer.adobe.com/commerce/php/development/backward-incompatible-changes/reference/) BIC.

<!-- MC-42212-->

* Correction d’un problème qui `\Magento\CatalogInventory\Model\Indexer\Stock\CacheCleaner::getCategoryIdsByProductIds` empêchait l’enregistrement d’un nouveau produit.

<!-- MC-42199-->

* La bibliothèque TinyMCE v3 obsolète a été supprimée. Les `Magento_Tinymce3Banner` tests de module et MFTF liés à TinyMCE v3.x ont été supprimés de Adobe Commerce.
...
* L’application ne renvoie plus d’erreur fatale PHP lorsqu’un plugin est ajouté à une classe parent. [GitHub-31291](https://github.com/magento/magento2/issues/31291)

<!-- ENGCOM-9012-->

* Mise à jour des fichiers de README.md pour ces modules : `Magento_Msrp`, `Magento_MsrpConfigurableProduct`, `Magento_MsrpGroupedProduct`, `Magento_Multishipping`, . `Magento_MysqlMq` [GitHub-32577](https://github.com/magento/magento2/issues/32577)
...
* Ajout d’une dépendance manquante sur le `web-token/jwt-framework` package au `magento/module-jwt-framework-adapter`fichier . [GitHub-32578](https://github.com/magento/magento2/issues/32578)
...
* Les tests ont été remaniés avec `StorefrontCheckQuickSearchStringActionGroup` et `StorefrontAssertProductNameOnProductMainPageActionGroup` (groupes d’actions existants). [GitHub-31251](https://github.com/magento/magento2/issues/31251)
...
* L’application affiche désormais une valeur précise pour le crédit de boutique disponible sur la page Mode de paiement dans les déploiements qui prennent en charge plusieurs devises.

<!-- MC-37820-->

* La liste des modes de paiement est désormais mise à jour comme prévu lorsqu’un acheteur invité modifie l’adresse de livraison d’une commande dans un autre pays lors du paiement. Auparavant, le changement d’adresse de facturation ne déclenchait pas de mise à jour des modes de paiement possibles.

#### PayPal

<!-- MC-39895-->

* Les acheteurs peuvent désormais passer à la caisse une commande PayPal Payflow Pro avec une adresse de livraison contenant des caractères spéciaux. Auparavant, la demande refusait le paiement de ces commandes.

<!-- MC-39861-->

* Les acheteurs sont maintenant redirigés vers la page de réussite de la commande après un paiement réussi à l’aide de PayPal. Auparavant, les clients étaient redirigés vers une page vierge, car les données de session étaient perdues.
...
* Les performances du processus cron lors de l’exécution  `catalog_product_alert` sur des tables volumineuses (plusieurs millions de lignes) ont été améliorées. Auparavant, toutes `catalog_product_alert` les alertes de produit étaient chargées, ce qui entraînait une exception de mémoire insuffisante.
...
* Les révisions sont désormais enregistrées avec l’ID de magasin correct après qu’un administrateur a approuvé et enregistré la révision à partir d’un domaine différent de celui du magasin. [GitHub-17510](https://github.com/magento/magento2/issues/17510)
...
* Les factures sont maintenant créées avec le total général correct lorsqu’une règle de prix de panier attribuant une remise de 100% est appliquée à une commande qui est également soumise à des taxes sur les produits du catalogue et des remises et qui donne droit à la livraison gratuite. Auparavant, la commande avait le prix correct, mais pas la facture. [GitHub-30853](https://github.com/magento/magento2/issues/30853)
...
* Les développeurs peuvent désormais modifier Elasticsearch mappages. Auparavant, les modèles dynamiques envoyés au serveur Elasticsearch dans les mappages par défaut étaient codés en dur, ce qui empêchait les développeurs d’indexer dans Elasticsearch les données supplémentaires associées à un module personnalisé.
...
* Les acheteurs peuvent désormais utiliser le bouton Retour du navigateur pour revenir à la page Sélectionner la méthode d’expédition lors de l’extraction d’une commande avec plusieurs adresses. Auparavant, l’application affichait une page Sélectionner le mode d’expédition endommagée. [GitHub-30268](https://github.com/magento/magento2/issues/30268)

<!-- MC-40021 ENGCOM-8983-->

* La quantité de produits reste inchangée comme prévu après qu’un acheteur modifie la quantité sur la page Envoyer à plusieurs adresses et clique sur le bouton Précédent du navigateur.
...
* L’application ne traite plus une chaîne de `0` comme une valeur vide lors de l’affichage d’une page d’accueil de magasin. Auparavant, l’application traitait une valeur entière au début d’un chemin de requête comme un ID de magasin, ce qui avait des effets inattendus sur le référencement.

<!-- MC-40313-->

* Les plugins pour `\Magento\Framework\App\ActionInterface` under `lib/internal/Magento/Framework/App/Action/Plugin` ont été supprimés pour respecter la directive selon laquelle les plugins doivent être utilisés pour personnaliser le comportement d’un module à partir d’un autre module. [GitHub-28050](https://github.com/magento/magento2/issues/28050)
...
* L’application affiche désormais un champ Numéro **de** TVA sur la page d’enregistrement du client lorsqu’elle `customer/create_account/vat_frontend_visibility` est activée.
...
* Ajout d’un test pour ce scénario : Les utilisateurs administrateurs peuvent modifier un compte client lorsque le client est abonné à une newsletter en file d’attente.
...
* Les chaînes de texte du modèle utilisées pour gérer les modes de paiement stockés (**Mon compte**  >  **Modes** de paiement stockés) sont désormais traduisibles.

<!-- MC-30798 ENGCOM-8717-->

* Les e-mails de commande envoyés par l’administrateur utilisent maintenant les paramètres régionaux du magasin et non les paramètres régionaux associés au compte de l’administrateur.

### Interface utilisateur

<!-- MC-41784-->

* Le pied de page Admin affiche maintenant la version de produit correcte.
...
* L’API Catalogue met désormais correctement à jour les valeurs d’options personnalisées d’un produit en ajoutant de nouvelles valeurs et en supprimant les anciennes valeurs. Auparavant, l’API ne supprimait pas les anciennes valeurs.
...
* L’application affiche désormais le prix correct du produit lorsque vous mettez à jour un produit avec une option de fichier personnalisable dans la liste de souhaits. Auparavant, l’application affichait le mauvais prix du produit et n’affichait pas de lien vers le fichier téléchargé.

<!-- MC-39060-->

* Le nombre total de produits dans une liste de souhaits pour un client avec plusieurs listes de souhaits correspond désormais au nombre d’articles dans la liste de souhaits. Auparavant, les produits en rupture de stock étaient inclus dans le nombre total de produits.
