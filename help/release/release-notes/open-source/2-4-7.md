...
Magento Open Source 2.4.7 introduit la prise en charge de PHP 8.3. Elle comprend des centaines de correctifs et d’améliorations de qualité. Les dépendances du compositeur principal et les bibliothèques tierces ont été mises à niveau vers les dernières versions disponibles. Cette version augmente la couverture GraphQL pour les attributs personnalisés et les caches de résolution GraphQL. Il introduit également la prise en charge des services FedEx et UPS récemment mis à jour.
...
* **Limites du nombre de codes de bon générés automatiquement**. Magento Open Source limite désormais le nombre de codes promo générés automatiquement. La valeur maximale par défaut est de 250 000. Les commerçants peuvent utiliser la nouvelle **[!UICONTROL Code Quantity Limit]** option de configuration (**[!UICONTROL Stores]** > **[!UICONTROL Settings:Configuration]** > **[!UICONTROL Customers]** > **[!UICONTROL Promotions]**) pour éviter de surcharger potentiellement le système avec de nombreux coupons. <!-- AC-8753 -->
...
* **Modifications apportées à la stratégie de sécurité du contenu (CSP)** : mises à jour de configuration et améliorations apportées aux stratégies de sécurité du contenu (CSP) Adobe Commerce pour se conformer aux exigences PCI 4.0. Pour plus d’informations, voir [Stratégies](https://developer.adobe.com/commerce/php/development/security/content-security-policies/) de sécurité du contenu dans le Manuel _du_ développeur PHP de Commerce.<!--AC-11512-->

   * La configuration CSP par défaut pour les pages de paiement pour l’administration des commerces et les zones vitrine est maintenant `restrict` en mode. Pour toutes les autres pages, la configuration par défaut est `report-only` le mode.  Dans les versions antérieures à la version 2.4.7, la stratégie de sécurité du contenu était configurée en `report-only` mode pour toutes les pages.

   * Ajout d’un fournisseur de valeur à usage unique pour permettre l’exécution de scripts intégrés dans une stratégie de sécurité du contenu. Le fournisseur de valeur à usage unique facilite la génération de chaînes nonce uniques pour chaque requête. Les chaînes sont ensuite attachées à l’en-tête CSP.
...
Magento Open Source 2.4.7 inclut les mises à niveau suivantes :
...
* **Compatibilité Elasticsearch 8.11**<!-- AC-9661 -->

* Prise en charge d’OpenSearch 2.12 et OpenSearch 1.3 <!-- AC-9659 -->
...
Toutes les bibliothèques JavaScript et dépendances NPM dans le code principal de Magento Open Source ont été mises à jour vers les dernières versions disponibles. Toutes les dépendances des bibliothèques Laminas ont été mises à jour vers la dernière version compatible avec PHP 8.3. <!-- AC-9667 9665 -->
...
* **WebHooks de litige** : lorsqu’un client conteste une transaction en Braintree, le statut du litige est désormais transmis à Commerce. Il est consultable dans la grille de **[!UICONTROL Sales]** > **[!UICONTROL Order]** et attaché à chaque commande.
...
* **Les fonctionnalités de mise en cache améliorées de GraphQL améliorent la vitesse de chargement des pages**. La fonctionnalité de mise en cache a été ajoutée à ces requêtes, ce qui accélère le temps de chargement des pages pour la plupart des pages PWA :
   ...
   * [monnaie](https://developer.adobe.com/commerce/webapi/graphql/schema/store/queries/currency/)
   * [Métadonnées d’attribut personnalisé](https://developer.adobe.com/commerce/webapi/graphql-api/index.html#query-customAttributeMetadata)

* **Amélioration des performances** de l’analyseur GraphQL. Les performances de l’analyseur GraphQL ont été améliorées en réduisant le nombre de fois que la méthode d’analyse est appelée par demande. Il s’appelle maintenant une fois. Auparavant, l’analyseur était appelé au moins trois fois.

#### Nouveaux champs pour les mutations existantes

* Ajout du `quickorder_active` champ aux [requêtes storeConfig](https://developer.adobe.com/commerce/webapi/graphql/schema/store/queries/store-config/) et [availableStores](https://developer.adobe.com/commerce/webapi/graphql/schema/store/queries/available-stores/) . Ce champ indique si la fonction Commande rapide est activée.
...
Inventory management (v1.2.7) fournit des outils pour gérer l’inventaire des produits. Cette fonctionnalité développée par la communauté est fournie avec Adobe Commerce et Magento Open Source code de base.

### Métamodule Magento Open Source extension

Cette version inclut le métapackage d’extension Magento Open Source v1.0.0, qui regroupe automatiquement certaines extensions Magento Open Source avec cette version principale. La version de cette extension incluse dans ce métapackage est installée lors `composer update` de son exécution, ce qui simplifie le processus de mise à niveau de l’extension lors de la mise à niveau vers la dernière version principale. Cette extension maintient un calendrier de publication indépendant.

Le métapackage d’extension Magento Open Source pour Magento Open Source 2.4.7 inclut les extensions suivantes :
...
PWA Studio v14.0 est compatible avec Magento Open Source 2.4.7-beta1. Plusieurs améliorations ont été apportées à l’accessibilité. Pour plus d’informations sur les correctifs, voir [Versions de PWA Studio](https://github.com/magento/pwa-studio/releases). Voir [Compatibilité des versions](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) pour obtenir la liste des versions de PWA Studio et de leurs versions Magento Open Sources principales compatibles.

### Structure de l’API web

Cette version introduit deux nouveaux points de terminaison REST qui offrent une solution de contournement pour une limitation de l’API `GET` REST et `POST V1/products/attributes` des points de terminaison. Ces points de terminaison renvoient la même valeur pour l’attribut pour les `is_filterable` `Filterable(with results)` options et `Filterable(no results)` de l’option **[!UICONTROL Use in Layered Navigation]** . (La `is_filterable` propriété d’attribut est de type `Boolean`, ce qui ne permet pas de définir cette propriété sur `Filterable(no results)`.)

Deux nouveaux points de terminaison REST ont été mis en œuvre pour contourner ce problème :
...
* La manipulation inutile du cache a été supprimée du processus de configuration. Auparavant, Magento Open Source avait écrit sa configuration sur le disque inutilement lorsque `bin/magento setup:db-data:upgrade` ou `bin/magento setup:upgrade` était exécuté, ce qui provoquait des problèmes avec certains modules lors de la configuration. [GitHub-38124](https://github.com/magento/magento2/issues/38124)
...
* `bin/magento setup:install` se termine désormais correctement une fois `app/etc/config.php` supprimé. Auparavant, le fichier manquant n’était pas régénéré pendant l’installation et Magento Open Source renvoyait une erreur. [GitHub-37805](https://github.com/magento/magento2/issues/37805)
...
* Magento Open Source optimise désormais les performances en ne chargeant pas les données de devis spécifiques à l’utilisateur lors de la génération de pages entières pouvant être mises en cache telles que les pages de détails du produit (PDP) et les pages CMS lorsque le panier persistant est activé. Auparavant, le système chargeait inutilement des données de session et de devis spécifiques à l’utilisateur lors de la génération de ces pages, ce qui affectait les performances.

<!-- ACP2E-2684 -->

* Les attributs personnalisés d’adresse personnalisée sont maintenant enregistrés comme prévu dans la base de données. Auparavant, les valeurs d’attribut enregistrées dans la base de données étaient précédées d’un code attribut.

<!-- ACP2E-2517 -->

* La génération du texte des filtres des règles de panier pour les attributs de produit a été améliorée. Les produits individuels ne sont plus chargés plusieurs fois.
...
* Magento Open Source affiche désormais des messages informatifs lorsqu’une action d’ajout au panier est déclenchée. Auparavant, l’option n’était `The requested qty is not available` pas toujours affichée comme il convenait sur la devanture.

<!-- ACP2E-2166 -->

* Les temps de chargement de la page de paiement ont été améliorés pour les clients disposant de carnets d’adresses volumineux. Magento Open Source traite désormais uniquement le nombre d’adresses client spécifié dans le **[!UICONTROL Customer Addresses Limit]** paramètre. Auparavant, Magento Open Source chargeait le carnet d’adresses entier.
...
* La valeur `applied_rule_ids` de la table `quote_item` inclut désormais une liste correcte des ID de règle appliqués. Auparavant, cette valeur ne contenait que le dernier ID de règle appliqué.
...
* L’éligibilité à la livraison gratuite est maintenant calculée comme prévu lorsque la condition pour les règles du **[!UICONTROL Subtotal (Excl. Tax)]** panier est appliquée. [GitHub-36760](https://github.com/magento/magento2/issues/36760)

<!-- AC-6672 -->

* La `cart` requête renvoie désormais le niveau de prix correct pour un produit. [GitHub-29655](https://github.com/magento/magento2/issues/29655)

<!-- AC-1162 -->

* Des messages d’erreur informatifs ont été ajoutés à la page de paiement pour guider les acheteurs lorsque l’application d’un coupon sur la page d’expédition entre en conflit avec le mode d’expédition sélectionné. Auparavant, un message demandait aux acheteurs de revenir à la page précédente.
...
* La condition de règle de prix de catalogue `If an item is FOUND/NOT FOUND in the cart with ALL/ANY of these conditions true` fonctionne désormais correctement avec les attributs de catégorie et de SKU. Auparavant, cette condition n’était pas correctement appliquée dans la logique du coupon de prix du panier et des coupons non valides étaient appliqués aux commandes. [GitHub-37660](https://github.com/magento/magento2/issues/37660)

<!-- ACP2E-1932 -->

* Vous pouvez désormais enregistrer une nouvelle règle de prix de panier avec l’extension `Magento_OfflineShipping` désactivée. Auparavant, le Magento Open Source générait cette erreur : `report.ERROR: Warning: Undefined array key "simple_free_shipping"`. [GitHub-37580](https://github.com/magento/magento2/issues/37580)

<!-- ACP2E-1516 -->

* Les règles de prix du panier sont désormais appliquées comme prévu lorsqu’une règle de prix du panier associée à une méthode d’expédition est configurée pour le magasin et que cette méthode d’expédition est remplacée par une autre lors du passage en caisse. Auparavant, la valeur n’était `applied_rule_ids` jamais modifiée dans la `sales_order_item` table et la règle du prix du panier n’était pas appliquée à la commande.

<!-- AC-2044 -->

* Les coupons sont maintenant appliqués avec succès à une commande lorsqu’un acheteur applique le coupon après avoir sélectionné la méthode d’expédition forfaitaire où une règle de prix de panier avec la méthode d’expédition définie comme condition s’applique. Auparavant, la page du panier affichait l’erreur `Coupon is Not valid` et le bon n’était pas appliqué. [GitHub-34866](https://github.com/magento/magento2/issues/34866)

### Catalogue

<!-- AC-10746 -->

* Exécute `bin/magento cache:clean` ou `bin/magento cache:flush` ne vide plus l’intégralité du cache pleine page intégré ou Varni, sauf indication contraire. [GitHub-38301](https://github.com/magento/magento2/issues/38301)

<!-- ACP2E-2257 -->

* Les marchands peuvent désormais utiliser le carrousel de produits pour créer des blocs CMS lorsque `catalog_product_price dimensions-mode` la variable est définie sur `website`. Auparavant, Magento Open Source n’enregistriez pas le blocage et renvoyiez cette erreur SLQ : `report.CRITICAL: PDOException: SQLSTATE[42S02]: Base table or view not found: 1146 Table`.
...
* Magento Open Source n’insère plus de texte de description de produit dans le champ **[!UICONTROL Meta Description]** de la page Détails du produit lorsque ce champ a été laissé vide. Auparavant, lorsqu’une description de produit contenait du code d’HTML généré par Page Builder ou un autre éditeur d’HTML, l’HTML s’affichait dans le champ Meta **[!UICONTROL Description]** et vous ne pouviez pas laisser ce champ vide.
...
* Magento Open Source affiche désormais le nom correct après le tri des produits par nom dans Admin > **[!UICONTROL Category]** > **[!UICONTROL Products in category]** dans les déploiements multi-boutiques où les noms de produits varient selon les magasins. [GitHub-36208](https://github.com/magento/magento2/issues/36208)
...
* `routes` Les requêtes qui utilisent des fragments renvoient maintenant les informations sur les catégories comme prévu. Auparavant, une erreur de serveur interne se produisait sur la page de catégorie. [GitHub-35906](https://github.com/magento/magento2/issues/35906)
...
* Les règles de catalogue sont maintenant appliquées comme prévu dans un environnement multi-sites Web.

### Produits configurables

<!-- ACP2E-1881 -->

* Les marchands peuvent désormais sélectionner **[!UICONTROL Skip quantity at this time]** et **[!UICONTROL Skip image uploading at this]** lors de la configuration d’un produit configurable sans affecter la source des produits simples associés. Auparavant, la sélection **[!UICONTROL Skip quantity at this time]** entraînait la disparition des sources de produits.
...
* Les utilisateurs administrateurs disposant d’autorisations de site Web peuvent désormais enregistrer un produit enfant qui fait partie d’un produit configurable dans un contexte de magasin différent. Les processus de réindexation ignorent maintenant ce type de produit configurable.

<!-- ACP2E-2003 -->

* Les produits configurables avec deux produits enfants ne sont plus marqués comme en rupture de stock lorsqu’un produit enfant est désactivé par une mise à jour planifiée. Il s’agissait d’un problème connu pour la version 2.4.7-beta1.
...
* Les codes promo contenant des espaces ne sont plus invalidés. Auparavant, si un code promo contenait un espace (avant ou après le code réel), la validation échouait. [GitHub-38048](https://github.com/magento/magento2/issues/38048)
...
* Les e-mails contenant des signes diacritiques ou accents (c’est-à-dire les e-mails utf8) sont désormais pris en charge pour les clients et les entreprises, ainsi que pour les commandes passées par les invités et les clients enregistrés. Auparavant, Magento Open Source n’enregistrait pas les enregistrements de client contenant des signes diacritiques et affichait cette erreur : `Something went wrong while saving the customer`.

<!-- ACP2E-2503 -->

* Les noms des expéditeurs de courriel ne peuvent plus inclure de caractères de deux points. Une nouvelle règle de validation empêche désormais l’enregistrement des noms d’expéditeurs qui contiennent des caractères de deux points à partir de la section Stocker les adresses électroniques de l&#39;> **[!UICONTROL General]** d’administration **[!UICONTROL Stores]** . Auparavant, les marchands pouvaient enregistrer un nom d’expéditeur contenant un caractère deux-points, ce qui entraînait des erreurs de serveur de messagerie.
...
* Magento Open Source enregistre désormais un message d’exception comme prévu lorsque la validation du numéro de TVA par le service de validation VIES échoue. [GitHub-36065](https://github.com/magento/magento2/issues/36065)

<!-- AC-8146 -->

* Le nom `Administrators` a été supprimé de la requête utilisée pour récupérer le rôle d’administrateur lors de la création du rôle d’administrateur. [GitHub-36998](https://github.com/magento/magento2/issues/36998)
...
* `Param $options[`position`]` pour les symboles de devise fonctionne désormais comme prévu et prend en charge la personnalisation de la position du symbole de devise. Auparavant, après la migration à partir de Zend Framework 1, la méthode `toCurrency` ignorait le paramètre `position` . Ce problème se produisait dans toute l’interface d’administration.

<!-- ACP2E-1492 -->

* Mutex a été mis en œuvre pour les ordres visant à empêcher les conditions de concurrence lors de la mise à jour par des demandes simultanées. Auparavant, les demandes simultanées (conditions de concurrence) d’annulation de commande entraînaient des entrées en double dans le `inventory_reservation` tableau.

<!-- ACP2E-1381 -->

* La `populateWithArray` fonction transforme maintenant correctement les propriétés de l’objet en casse serpent, ce qui la rend compatible avec les `AbstractModel` getters et les setters. Auparavant, la mise en forme de la casse du serpent était incorrecte lorsque le nom d’attribut de données contenait plusieurs lettres majuscules à la suite.

<!-- ACP2E-1522 -->

* La valeur d’incrémentation automatique de la table s’incrémente `catalog_product_entity_varchar` désormais correctement après l’enregistrement d’un produit. Auparavant, cette valeur augmentait de dix.
...
* Le champ **Etat/Province** de l’adresse de l’administrateur conserve désormais la dernière valeur enregistrée comme prévu. Auparavant, lorsque vous supprimiez la valeur de ce champ, Magento Open Source continuait à afficher la valeur supprimée dans le champ **Etat/Province**. [GitHub-36846](https://github.com/magento/magento2/issues/36846)
...
* La requête `products` renvoie désormais tous les détails de tarification de storefront pertinents lorsque la tarification dynamique est définie sur non et que des remises sont appliquées. Auparavant, la fourchette de prix des produits n’était pas renvoyée. [GitHub-35649](https://github.com/magento/magento2/issues/35649)

<!-- ACP2E-1744 -->

* Les noms de transaction GraphQL incluent désormais des noms de requête de niveau supérieur uniquement dans les journaux New Relic. Auparavant, les noms de transaction comprenaient également des noms de requête secondaires.

<!-- AC-8623 -->

* La gestion des erreurs de résolution pour les composants de bibliothèque GraphQL fonctionne maintenant comme prévu.

<!-- AC-7430 -->

* Lorsque le paramètre Visibilité d’un produit est défini sur `Catalog`, `products` les requêtes sur ce produit résolvent désormais correctement le paramètre. Auparavant, tout filtre fourni était basculé vers le paramètre `Visibility: Catalog` . [GitHub-36591](https://github.com/magento/magento2/issues/36591)
...
* La `products` requête renvoie maintenant les étiquettes correctes. [GitHub-29635](https://github.com/magento/magento2/issues/29635)

<!-- ACP2E-1315 -->

* La `categoryList` requête renvoie maintenant une section remplie `product` du bloc pour les `options` produits enfants des produits groupés, comme prévu. Auparavant, aucune information sur le regroupement de produits enfants n’était renvoyée.
...
* Vous pouvez désormais utiliser un fragment `ProductCard` lors de l&#39;interrogation de `related_products`, `upsell_products` et `crossell_products` sur `ProductInterface`. [GitHub-29769](https://github.com/magento/magento2/issues/29769)
...
* Le formulaire de téléchargement de l’icône de favicon Admin prend désormais en charge les `.ico` types de fichiers. [GitHub-34858](https://github.com/magento/magento2/issues/34858)
...
* Les rapports sur les produits sont maintenant exportés vers l’annuaire `var` . Auparavant, ces rapports étaient exportés vers le répertoire racine du document, qui est un répertoire en lecture seule dans les instances cloud.
...
* Magento Open Source n’inclut plus le chemin d’accès complet à la racine du serveur dans le message d’erreur qu’il affiche lorsqu’une requête HTTP incorrecte a été envoyée au serveur. Seules les chaînes sont désormais transmises à la méthode `explode` comme second argument.
...
* Ajout de régions ukrainiennes `directory_country_region` au tableau. [GitHub-35187](https://github.com/magento/magento2/issues/35187)

<!-- https://github.com/magento/magento2/pull/29173 -->

* Les fichiers CSS ne sont plus corrompus lorsqu’ils sont fusionnés lorsque `var/tmp` et `pub/static` sont sur des systèmes de fichiers différents et **[!UICONTROL CSS Merging Enabled]** est activé. Les fichiers temporaires ont été créés dans le même répertoire statique que le fichier cible pour s’assurer que les deux fichiers se trouvent sur le même système de fichiers. Par conséquent, l’appel à le renommer est atomique. [GitHub-29172](https://github.com/magento/magento2/issues/29172)

<!-- https://github.com/magento/magento2/pull/36769 -->

* Correction d’une erreur potentielle lors de l’appel d’une fonction `getId()` membre sur `int`, qui déclenchait parfois une exception lorsqu’un client était affiché dans l’administration.

<!-- https://github.com/magento/magento2/pull/34092 -->

* Les contrôles de type sont maintenant appliqués pour le type de tableau dans le processeur de configuration du thème de conception. [GitHub-34440](https://github.com/magento/magento2/issues/34440)

### Gestion des stocks

<!-- ACP2E-1883 -->

* Les produits sont maintenant automatiquement retournés à l’état En stock après une note de crédit qui comprend un retour de la quantité de produit. Auparavant, si un produit était en rupture de stock mais qu’une note de crédit renvoyait le stock de ce produit, les commerçants devaient modifier manuellement l’état des stocks de produits.
...
* L’état configurable du stock de produits est maintenant mis à jour sur la vitrine comme prévu lorsque les produits enfants sont mis à jour par `V1/inventory/source-items`.

<!-- ACP2E-2114 -->

* L’appel `POST <store_code>/V1/inventory/source-items` REST valide désormais la charge utile pour les espaces blancs dans les articles sources de stock renvoyés et envoie un message d’erreur sur la validation si nécessaire.

<!-- ACP2E-2072 -->

* Les commerçants peuvent désormais informer les acheteurs qu’une commande est prête à être ramassée lorsque le **[!UICONTROL No Manage Stock]** paramètre est activé pour un produit. Auparavant, lorsque ce paramètre était activé, Magento Open Source affichait ce message d’erreur : `Your order is not ready for pickup`.

<!-- ACP2E-2227 -->

* Les marchands peuvent désormais créer une note de crédit pour les commandes dont un produit simple associé à un produit configurable a été supprimé. Auparavant, les commerçants ne pouvaient pas créer la note de crédit et Magento Open Source lançait une `TypeError` exception.

<!-- ACP2E-2310 -->

* Le nombre de sources de la grille d’administration renvoie désormais des enregistrements valides après l’application d’un filtre. Auparavant, la méthode `getSourcesCount` renvoyait toujours le nombre total de sources après l’application d’un filtre.
...
* L’état du stock de produits configurables est désormais correctement mis à jour lorsqu’un produit configurable et la valeur de ses produits `Stock Availability` enfants sont mis à jour par une mise à jour de stock en masse. Auparavant, vous ne pouviez pas retourner un produit configurable et ses produits enfants en stock par mise à jour en masse.

<!-- ACP2E-2096 -->

* Le type de données pour les sources autres que celles par défaut a été mis à jour pour prendre en charge jusqu’à huit chiffres entiers, ce qui correspond à `DECIMAL (12, 4)` la même limite appliquée pour le stock par défaut. Cette valeur est maintenant synchronisée avec les pages de produits Ajouter et modifier par l’administrateur, qui prennent en charge la validation des entrées jusqu’à huit chiffres pour tous les types de sources de stock. Auparavant, l’entrée du champ (pour les sources autres que celles **[!UICONTROL Quantity]** par défaut) était prise en charge jusqu’à six chiffres uniquement.
...
* Les commerçants peuvent maintenant créer des notes de crédit comme prévu pour le retour partiel des commandes qui ont été payées en partie par des points de récompense. Auparavant, lorsqu’un marchand créait un retour partiel, le statut de la commande était incorrectement identifié comme fermé et la page de commande Admin n’affichait pas les options de note de crédit.

<!-- ACP2E-2583 -->

* Magento Open Source applique désormais le montant correct de la remise aux commandes partiellement annulées. Auparavant, le montant de la remise tel que calculé dans la colonne était incorrectement calculé lors de la `base_discount_cancelled` modification d’une commande. La prise en charge des valeurs négatives a été ajoutée aux factures pour les commandes à prix réduit.
...
* La page de note de crédit ne se bloque plus lorsque la livraison gratuite est activée lorsque les prix d’expédition incluent des taxes et que les calculs de taxe sont appliqués après les remises. Auparavant, Magento Open Source lançait une exception `Division by zero`. [GitHub-36800](https://github.com/magento/magento2/issues/36800)
...
* Les problèmes d’affichage des colonnes dans le contenu du Créateur de pages ont été résolus. Auparavant, le contenu de type de contenu `column-line` n’était pas rendu. [GitHub-3949](https://github.com/magento/pwa-studio/issues/3949)
...
* Le **[!UICONTROL Venmo]** bouton peut désormais être masqué lorsque les boutons d’PayPal sont activés. Auparavant, le **[!UICONTROL Venmo]** bouton ne pouvait pas être supprimé du panier, du mini-panier, des détails du produit ou des pages de paiement.

<!-- BUNDLE-3313 -->

* Les acheteurs peuvent désormais sélectionner une méthode d’expédition tout en appliquant un code promo lors du paiement. Auparavant, dans ces conditions, Magento Open Source jetait une erreur et attribuait une expédition forfaitaire à la commande.

<!-- BUNDLE-3310 -->

* Les problèmes liés au traitement des modes de paiement par crédit PayPal et PayPal ont été résolus.

<!-- BUNDLE-3273 -->

* Résolution de problèmes liés aux URL de secours pour les commandes payées par Ideal payments lorsque le paiement local est activé.
...
* Les acheteurs peuvent désormais vérifier une commande expédiée à plusieurs adresses à l’aide d’un mode de paiement Braintree lorsque le coffre-fort pour les paiements par carte est désactivé. [GitHub-37615](https://github.com/magento/magento2/issues/37615)
...
* Magento Open Source ne génère plus d’erreur lors de la création d’une deuxième facture partielle pour une commande payée par Braintree lorsque l’option **[!UICONTROL Enable Vault for Card Payments]** est activée. Auparavant, lorsqu’un commerçant tentait de créer une seconde facture, le Magento Open Source envoyait cette erreur : `Your payment could not be taken. Please try again or use a different payment method. Payment method token is invalid`.

<!-- ACP2E-1229 -->

* L’option de mode de paiement Braintree Vault n’est plus automatiquement désélectionnée dans le processus de paiement lorsqu’un acheteur met à jour son adresse de facturation. Auparavant, le mode de paiement était désélectionné lorsque l’acheteur désélectionnait le bouton **[!UICONTROL My billing and shipping address are the same]**.
...
* La `bin/magento setup:perf:generate-fixtures setup/performance-toolkit/profiles/ee/small.xml` commande s’exécute désormais correctement lorsqu’elle est exécutée plusieurs fois. Auparavant, cette erreur était générée lors de l’exécution de plusieurs fois : `Coupon with the same code already exists`.

### Promotion

<!-- ACP2E-1290 -->

* Les effets de la **[!UICONTROL Fixed amount discount for whole cart]** règle des prix, lorsqu’ils sont appliqués avec la règle, sont maintenant répartis proportionnellement comme prévu entre les produits et les montants d’expédition **[!UICONTROL Apply to shipping amount]** . Auparavant, les calculs de remise étaient incorrects.

<!-- ACP2E-1300 -->

* Le **[!UICONTROL Discard Subsequent Rules]** paramètre est maintenant appliqué comme prévu. Lorsque ce paramètre est activé, d’autres règles de priorité basse ne peuvent être appliquées qu’à un autre produit. [GitHub-35707](https://github.com/magento/magento2/issues/35707)

### reCAPTCHA

<!-- ACP2E-2509 -->

* L’invisible reCAPTCHA v3 fonctionne désormais comme prévu pour le bouton de page de passage en caisse **[!UICONTROL Sign in]**. Auparavant, les clients ne pouvaient pas se connecter et Commerce générait des erreurs JavaScript.
...
* Les utilisateurs administrateurs peuvent désormais rechercher des enregistrements avec des e-mails contenant `+` (plus) des signes à partir de la page Rapport Paniers abandonnés. [GitHub-36560](https://github.com/magento/magento2/issues/36560)

### Rechercher {#search-heading}

<!-- ACP2E-2272 -->

* Les suggestions de recherche à saisie semi-automatique sur le mini-frontend de recherche de l’en-tête fonctionnent désormais comme prévu. Auparavant, la saisie automatique ne remplissait pas ce champ lorsque cette fonction était activée. [GitHub-37557](https://github.com/magento/magento2/issues/37557)

<!-- ACP2E-2101 -->

* Les performances des requêtes effectuées à partir de la barre de recherche Admin ont été améliorées par l’ajout d’un indexeur. Auparavant, plus la `search_query` table était longue, plus l’exécution de la requête prenait du temps.
...
* Magento Open Source affiche désormais la valeur correcte de la valeur **[!UICONTROL Regular Price]** lorsqu’une option personnalisée est sélectionnée pour un produit dont la valeur est **[!UICONTROL Special Price]** lorsque **[!UICONTROL Price Display Settings]** est défini pour inclure une taxe.
...
* Le téléchargeur d’images fonctionne maintenant comme prévu dans les lignes dynamiques. Les images sont désormais correctement téléchargées lorsqu’elles sont envoyées en tant que tableau multinational. [GitHub-37957](https://github.com/magento/magento2/issues/37957)

<!-- AC-10877 -->

* Les styles de chargement de fichiers ne sont plus dupliqués dans le thème Luma. [GitHub-32373](https://github.com/magento/magento2/issues/32373)
...
* Amélioration du message signalant les erreurs survenant lors du rendu des modèles. Magento Open Source affiche désormais un message informatif. Auparavant, Magento Open Source affichait des fuites d’HTML lorsqu’une erreur PHP se produisait.

<!-- ACP2E-2595 -->

* Les attributs personnalisés du client (type `File attached`) fonctionnent désormais correctement avec les images qui ont été supprimées, puis téléchargées. Auparavant, les attributs personnalisés transférés, puis supprimés, puis chargés à nouveau ne pouvaient pas être enregistrés à partir de la vitrine.

<!-- AC-9125 -->

* Correction d’une erreur de syntaxe dans le fichier du modèle de calendrier. [GitHub-37725](https://github.com/magento/magento2/issues/37725)

<!-- AC-6445 -->

* Les utilisateurs administrateurs peuvent désormais modifier le logo de la vitrine lorsque le mode magasin unique est activé. Auparavant, Magento Open Source affichait le logo de secours par défaut. [GitHub-35952](https://github.com/magento/magento2/issues/35952)
...
* Les messages d’erreur associés à la soumission de commentaires sur la page de détails de la commande sont désormais plus informatifs. Auparavant, lorsqu’un acheteur cliquait sur **[!UICONTROL Submit comment]** sans modifier l’état de l’ordre ni saisir du texte dans la zone de commentaire, le Magento Open Source affichait ce message : `The comment is missing. Enter and try again`.

<!-- ACP2E-1659 -->

* L’icône de calendrier s’affiche désormais comme prévu dans les champs de plage de dates sur Admin > **[!UICONTROL Cart Price Rules]** après la **[!UICONTROL Marketing]** sélection d’une date pour un champ. Auparavant, après avoir sélectionné une date pour un champ, l’icône de calendrier n’était plus affichée sur les autres champs et Magento Open Source affichait le texte d’espace réservé. [GitHub-36499](https://github.com/magento/magento2/issues/36499)

<!-- ACP2E-1717 -->

* Les formulaires d’enregistrement des clients qui incluent des champs à sélection multiple se chargent désormais. Auparavant, ce formulaire ne se chargeait pas lorsqu’il contenait des champs à sélection multiple et l’erreur suivante était déclenchée : `report.CRITICAL: Exception: Deprecated Functionality: explode(): Passing null to parameter #2 ($string) of type string is deprecated in /magento2ee/app/code/Magento/CustomAttributeManagement/Block/Form/Renderer/Multiselect.php on line 53 in /lib/internal/Magento/Framework/App/ErrorHandler.php:62`. [GitHub-37189](https://github.com/magento/magento2/issues/37189)
...
Vous pouvez installer Magento Open Source 2.4.7 à l’aide de [Composer](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html).
