...
Consultez le [Bulletin de sécurité d’](https://helpx.adobe.com/fr/security/products/magento/apsb22-13.html) pour connaître la dernière discussion sur ces problèmes résolus.
...
* Le protocole HTTPS est désormais activé par défaut pour le storefront Magento Open Source. Les paramètres **Utiliser des URL sécurisées sur Storefront** et **Utiliser des URL sécurisées dans Admin** sont activés par défaut et tous les cookies intégrés sont désormais définis comme sécurisés.  <!-- AC-1173-->
...
* L’équipe de développement peut désormais configurer la taille maximale des tableaux acceptés par les points d’entrée RESTful Magento Open Source par point d’entrée. Voir [Sécurité des API](https://developer.adobe.com/commerce/webapi/get-started/api-security/). <!-- AC-465-->
...
* Le prix minimum annoncé **qu&#39;est-ce que c&#39;est ?** Vous pouvez désormais cliquer sur le lien de la page du panier pour ouvrir la fenêtre contextuelle d’informations comme prévu. Auparavant, cliquer sur ce lien n’avait aucun effet.
...
* Magento Open Source génère désormais une `customertoken` par les requêtes d’API GraphQL ou REST comme prévu après plusieurs échecs de tentative de connexion consécutifs. Auparavant, applicationOpen Source ne vérifiait pas si la valeur de `lock_expires_at` dans `oauth_token_request_log` était supérieure à la date et à l’heure actuelles, et renvoyait toujours le nombre de tentatives ayant échoué, ce qui empêchait le client de se connecter un jour. [GitHub-34067](https://github.com/magento/magento2/issues/34067)
...
* L’appel PUT `/rest/all/V1/categories/<above_created_category_id>` respecte désormais les valeurs spécifiées par `available_sort_by`.  Auparavant, la liste déroulante Trier par liste de produits disponible était sélectionnée avec toutes les options possibles dans la page Catégorie et la case **Tout utiliser** était cochée. [GitHub-32596](https://github.com/magento/magento2/issues/32596)
...
* Magento Open Source n’affiche plus de liste de souhaits dans la barre latérale de catégorie lorsque l’option **Afficher dans la barre latérale** est désactivée. Auparavant, applicationOpen Source ignorait cette option.
...
**Problème** : les acheteurs ne peuvent pas ajouter un produit à leur panier lorsqu’aucune option n’est sélectionnée dans Admin **Store** > **Configuration** > **Général** > **Options du pays** > **Autoriser les pays**. L’application Open Source affiche à la place cette erreur de console : `Failed to load resource: the server responded with a status of 400 (Bad Request)`. <!-- AC-2564-->
...
Vous pouvez installer Magento Open Source 2.4.4 à l’aide du [compositeur](../../../installation/composer.md).
