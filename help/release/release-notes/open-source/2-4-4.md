...
## Mise en évidence de Magento Open Source 2.4.4
...
* Le protocole HTTPS est désormais activé par défaut pour le storefront du Magento Open Source. Le **Utilisation d’URL sécurisées sur Storefront** et **Utiliser des URL sécurisées dans Admin** sont activés par défaut et tous les cookies intégrés sont désormais définis comme étant sécurisés.  <!-- AC-1173-->
...
Cette version apporte une conformité améliorée aux directives d’accessibilité standard. Il comprend des info-bulles améliorées, un nommage et un balisage accessibles des éléments d’écran, ainsi que des icônes et des boutons reconçus. Plus de 80 % de ces correctifs permettent d’améliorer l’expérience d’achat pour les utilisateurs sans vision ou vision limitée.
...
* Correction de problèmes avec `sprintf(__())` et `sprintf(Magento\Framework\Phrase())` construction pour une compatibilité avec PHP 8.x dans tout le code base. Auparavant, l’applicationOpen Source générait ce type d’erreur : `Expected parameter of type 'string', 'Magento\Framework\Phrase' provided`. [GitHub-34085](https://github.com/magento/magento2/issues/34085)
...
* Magento Open Source génère désormais un `customertoken` par les demandes de l’API GraphQL ou REST, comme prévu après plusieurs tentatives de connexion ayant échoué. Auparavant, applicationOpen Source ne vérifiait pas si la valeur de `lock_expires_at` in `oauth_token_request_log` était supérieur à la date et l’heure actuelles et renvoyait toujours le nombre de tentatives ayant échoué, ce qui empêchait le client de se connecter un jour. [GitHub-34067](https://github.com/magento/magento2/issues/34067)
...
Vous pouvez installer Magento Open Source 2.4.4 à l’aide de [Compositeur](../../../installation/composer.md).
