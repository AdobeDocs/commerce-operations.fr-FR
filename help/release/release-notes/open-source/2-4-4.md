...
* Magento Open Source génère désormais un `customertoken` par les demandes de l’API GraphQL ou REST, comme prévu après plusieurs tentatives de connexion ayant échoué consécutivement. Auparavant, applicationOpen Source ne vérifiait pas si la valeur de `lock_expires_at` in `oauth_token_request_log` était supérieur à la date et l’heure actuelles et renvoyait toujours le nombre de tentatives ayant échoué, ce qui empêchait le client de se connecter un jour. [GitHub-34067](https://github.com/magento/magento2/issues/34067)
