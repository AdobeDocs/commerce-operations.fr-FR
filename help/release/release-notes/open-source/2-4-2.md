...
#### Plus de 35 améliorations de sécurité qui aident à fermer les vulnérabilités d’exécution de code à distance (RCE) et de cross-site scripting (XSS)
...
* La version 2.4.2 est désormais compatible avec **Composer 2.x**. Nous recommandons aux marchands de migrer vers Composer 2.x. Bien que vous puissiez installer cette version à l’aide de Composer 1.x, Composer 1.x atteindra bientôt sa fin de vie. Pour un aperçu des fonctionnalités de Composer 2.x, voir [Composer 2.0 est maintenant disponible !](https://blog.packagist.com/composer-2-0-is-now-available/)

La possibilité de configurer une installation pour utiliser une base de données fractionnée a été abandonnée dans cette version. Les commerçants qui utilisent actuellement une base de données partagée doivent commencer à planifier la restauration ou la migration vers une base de données unique ou utiliser une autre approche. Consultez la publication DevBlog [Abandon de la fonctionnalité de base de données partagée dans Magento Open Source](https://community.magento.com/t5/Magento-DevBlog/Deprecation-of-Split-Database-in-Magento-Commerce/ba-p/465187) pour obtenir un aperçu de ce problème.  Pour obtenir des instructions de migration, reportez-vous à la section [Rétablissement d’une base de données partagée vers une base de données unique](../../../configuration/storage/revert-split-database.md) .
...
* Ajout de la prise en charge des listes de [comparaison.](https://developer.adobe.com/commerce/webapi/graphql/queries/compare-list.html) Les acheteurs peuvent [créer](https://developer.adobe.com/commerce/webapi/graphql/mutations/create-compare-list.html) et [supprimer](https://developer.adobe.com/commerce/webapi/graphql/mutations/delete-compare-list.html) des listes de comparaison, et [ajouter](https://developer.adobe.com/commerce/webapi/graphql/mutations/add-products-to-compare-list.html) et [supprimer](https://developer.adobe.com/commerce/webapi/graphql/mutations/remove-products-from-compare-list.html) des éléments aux listes de comparaison. En outre, les acheteurs qui créent une liste de comparaison en tant qu’invité peuvent se connecter en tant que client et [conserver](https://developer.adobe.com/commerce/webapi/graphql/mutations/assign-compare-list-to-customer.html) leurs listes de comparaison.

* Ajout de la mutation [`generateCustomerTokenAsAdmin`](https://developer.adobe.com/commerce/webapi/graphql/mutations/generate-customer-token-as-admin.html) et mise à jour de l’objet `Customer` pour prendre en charge l’assistance à l’achat à distance.
...
* Les ID d’objet de type entier ont été abandonnés en faveur des `uid` attributs de type ID. <!-- MC-36346-->

* Ajout de l’attribut `staging` à l’événement [`ProductInterface`](https://developer.adobe.com/commerce/webapi/graphql/interfaces/product-interface.html) et [`CategoryInterface`](https://developer.adobe.com/commerce/webapi/graphql/interfaces/category-interface.html) pour déterminer si un produit est mis en scène et afficher les informations de campagne associées. <!-- MC-33897-->
...
Cette version de PWA Studio comprend :
...
Pour plus d’informations sur les améliorations et les correctifs, voir [Versions de PWA Studio](https://github.com/magento/pwa-studio/releases). Voir [compatibilité](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) pour obtenir la liste des versions de PWA Studio et leurs versions compatibles.

### Galerie multimédia

**Nouvelles ressources de rôle pour la Galerie multimédia**. Cette version offre aux marchands la possibilité de limiter l’accès administrateur à la seule galerie multimédia et de contrôler qui peut effectuer les actions suivantes :
...
**Images optimisées pour le Web dans le contenu**. Les marchands peuvent désormais utiliser [le rendu d’image](https://experienceleague.adobe.com/en/docs/commerce-admin/content-design/wysiwyg/gallery/media-gallery-image-optimization) optimisé pour le Web dans le contenu au lieu d’images haute résolution.L’image d’origine reste inchangée dans la Galerie multimédia et le rendu d’image est généré dynamiquement lorsque l’image est insérée dans le contenu.

### Cadre de test fonctionnel (MFTF)

MFTF 3.2.1 est maintenant disponible. Cette version introduit la tolérance aux erreurs dans les tests et la génération de suites de tests. D’autres améliorations et corrections de bogues sont décrites dans le journal des modifications de la structure des tests fonctionnels[](https://github.com/magento/magento2-functional-testing-framework/blob/develop/CHANGELOG.md).
...
* La possibilité de configurer une installation pour utiliser une base de données partagée est désormais obsolète dans cette version. Consultez la publication DevBlog [Abandon de la fonctionnalité de base de données partagée dans Magento Open Source](https://community.magento.com/t5/Magento-DevBlog/Deprecation-of-Split-Database-in-Magento-Commerce/ba-p/465187) .
...
* L’application ne renvoie plus d’erreur lorsque vous essayez de modifier `backend-frontname` à l’aide du conteneur `ssh` après l’installation du Magento Open Source. [GitHub-26762](https://github.com/magento/magento2/issues/26762)
...
* Le mode de paiement par défaut PayPal Express Checkout fonctionne désormais comme prévu avec le mode de paiement par carte de crédit Braintree. Auparavant, l’application n’affichait pas le **bouton PayPal** lorsque le mode de paiement par carte de crédit Braintree était activé.

### Produits groupés

<!-- MC-34063-->

* L’application ne fixe plus les prix des articles enfants de produit groupé à prix fixe entre guillemets.

<!-- MC-38050-->

* Le tri des prix fonctionne désormais comme prévu lorsque les produits groupés incluent des produits en rupture de stock. Auparavant, le tri des prix par ordre décroissant n’affichait pas les produits par ordre décroissant.
...
* La page de création de facture de commande affiche maintenant les produits enfants pour les produits groupés comme prévu. [GitHub-27350](https://github.com/magento/magento2/issues/27350)
...
* L’estimateur de mode d’expédition fonctionne désormais comme prévu lorsque des attributs d’adresse personnalisés sont présents à l’étape d’expédition du flux de travail de paiement. Auparavant, l’application affichait ce message d’erreur : `{"message":"Error occurred during \"custom_attributes\" processing. A custom attribute is specified with a missing attribute code. Verify the code and try again."}` [GitHub-27505](https://github.com/magento/magento2/issues/27505)

<!-- MC-35783-->

* L’application affiche désormais le `You have no items in your shopping cart` message dans le mini-panier comme prévu lorsqu’un acheteur supprime le seul article de son panier. Auparavant, les données du panier n’étaient pas mises à jour lorsqu’un `checkout/cart/removeFailed` acheteur supprimait l’article unique, et l’application affichait ce message : `1 product requires your attention`.

<!-- MC-36088-->

* L’application n’affiche plus les frais d’expédition d’une commande pour un mode d’expédition non applicable. Auparavant, l’application affichait un prix de 0 pour un mode de livraison non applicable dans le workflow de passage en caisse.
...
* L’application ne répète plus une adresse de livraison dans le processus de paiement de vitrine lorsqu’une commande est commandée à nouveau auprès de l’administrateur.
...
* Tous les nouveaux enregistrements de la `quote` table pour les invités se voient désormais attribuer la valeur 1 dans le `customer_is_guest field`. Auparavant, pour chaque nouvel invité qui ajoutait un produit dans le panier, l’enregistrement se voyait attribuer un 0 dans le`quote.customer_is_guest` champ.

<!-- ENGCOM-8091 -->

* Le **bouton Envoyer** de la section Révision et paiements du processus de paiement a été déplacé à l’intérieur `<form id="purchaseorder-form"...></form>`de , ce qui permet la soumission implicite de formulaires sans modifier aucune JavaScript. [GitHub-27925](https://github.com/magento/magento2/issues/27925)
...
* L’application affiche désormais correctement les noms de produits qui contiennent des caractères spéciaux dans le mini-panier. [GitHub-29075](https://github.com/magento/magento2/issues/29075)

<!-- ENGCOM-8308 -->

* L’application définit maintenant le forfait de remise correct pour le tarif de table. Auparavant, l’application n’appliquait pas de règle de panier qui définissait une remise sur le panier lors du calcul du tarif de table. [GitHub-30169](https://github.com/magento/magento2/issues/30169)
...
* Le prix d’une option personnalisée avec un prix en pourcentage est désormais converti, comme prévu, dans la devise de base du magasin actif dans les déploiements multi-magasins. Auparavant, le prix d’une option personnalisée avec un prix en pourcentage était converti incorrectement. [GitHub-26432](https://github.com/magento/magento2/issues/26432)
...
* Le `/V1/products/attributes/:attributeCode/options` point de terminaison REST prend désormais en charge la mise à jour d’une option d’attribut existante.
...
* Les grilles de produits sur les **pages Ventes** > **Commandes** et **Catalogue** > **Produits** répertorient désormais tous les magasins pertinents, et pas seulement le magasin le plus récemment créé. [GitHub-29267](https://github.com/magento/magento2/issues/29267)

<!-- ENGCOM-6698 -->

* Le référentiel de produits utilise `store_id` désormais, lorsqu’il est disponible, pour enregistrer les valeurs d’attribut d’un produit existant.  Auparavant, la classe de référentiel de produits remplaçait l’ID de magasin de produits et attribuait toujours l’ID de magasin par défaut. [GitHub-29933](https://github.com/magento/magento2/issues/29933)

### Règle de catalogue

<!-- MC-35065-->

* Les règles de prix catalogue fonctionnent désormais comme prévu avec des options personnalisées sur les pages de détails des produits. Auparavant, les prix des produits sur cette page ne reflétaient pas les règles de prix catalogue applicables. [GitHub-22856](https://github.com/magento/magento2/issues/22856)

<!-- MC-34265-->

* Les caches de produits et de catalogues expirent comme prévu. Auparavant, les caches expiraient quotidiennement car `сron` la `catalogrule_apply_all` tâche était exécutée une fois par jour, ce qui réindexait toutes les règles de catalogue et les indexeurs dépendants, et effaçait le cache pour tous les produits et catégories.
...
* Le titre de la page Échec de commande a été révisé pour plus d’exactitude. Auparavant, cette page était intitulée : **Nous avons reçu votre commande !**.  [GitHub-29416](https://github.com/magento/magento2/issues/29416)
...
* Suppression des espaces dans les éléments intégrés des filtres de navigation en couches de balisage. [GitHub-30448](https://github.com/magento/magento2/issues/30448)

<!-- ENGCOM-8056 -->

* `rl` a été corrigé en `url`  `\Magento\Framework\Filter\Template\Tokenizer\Parameter`. [GitHub-29185](https://github.com/magento/magento2/issues/29185)

#### Code nettoyage

<!-- MC-37922-->

* Une balise HTML `<br>` superflue a été supprimée de la zone de message Vérifier les données de la **page Importation** > **système**.
...
* Ajout de la classe block correcte pour l’exemple de référence front-end `viewModel` de la `\Magento\Framework\View\Element\Template` classe. [GitHub-30450](https://github.com/magento/magento2/issues/30450)
...
* Correction du commentaire de PHPdoc en `app/code/Magento/CatalogImportExport/Model/Export/Product.php`. [GitHub-30833](https://github.com/magento/magento2/issues/30833)

<!-- ENGCOM-8277 -->

* L’annotation docblock du `PublisherInterface` message a été corrigée. [GitHub-30190](https://github.com/magento/magento2/issues/30190)

<!-- ENGCOM-8264 -->

* Correction des arguments dans la `getStatusByState` méthode. [GitHub-30173](https://github.com/magento/magento2/issues/30173)
...
* Remplacé `@param string $attribute` par `@param AbstractAttribute|string[]|string $attribute` `EntityAbstract.php`. [GitHub-30191](https://github.com/magento/magento2/issues/30191)
...
* Un produit virtuel ne change plus de type lorsque son état passe d’activé à désactivé (ou vice versa). Auparavant, l’application changeait le type d’un produit de virtuel à simple lorsque son état changeait.

<!-- MC-38197-->

* La `getValue()` méthode ne déclenche plus d’erreur fatale lors de la mise à jour des paniers.

<!-- MC-34444-->

* Les produits configurables disponibles sur plusieurs sites Web n’affichent plus les produits enfants simples qui ne sont pas affectés au site Web spécifique. [GitHub-28291](https://github.com/magento/magento2/issues/28291)

<!-- ENGCOM-8202 -->

* Les pages de produits vitrines affichent désormais les prix par catégorie sans taxes, comme prévu. [GitHub-12225](https://github.com/magento/magento2/issues/12225)

<!-- ENGCOM-8134 8174-->

* Les prix PDSF des produits configurables sont désormais convertis correctement dans la devise choisie. Auparavant, l’application affichait les prix dans la mauvaise devise et lançait cette erreur :  `Call to a member function setAppliedTaxes() on null`. [GitHub-26526](https://github.com/magento/magento2/issues/26526), [GitHub-29603](https://github.com/magento/magento2/issues/29603)

<!-- ENGCOM-8234 -->

* Les marchands peuvent désormais enregistrer une étiquette d’option d’attribut d’échantillon de texte spécifique au magasin avec une valeur de 0 (zéro). Auparavant, la valeur 0 était remplacée par la valeur de l’option d’étiquette Admin. [GitHub-30073](https://github.com/magento/magento2/issues/30073)
...
* L’onglet Adresse client administrateur affiche désormais correctement le nombre d’adresses après qu’un administrateur a effectué une suppression massive d’adresses. Auparavant, l’application incluait des adresses supprimées dans son décompte.

<!-- MC-36718-->

* Le format DateTime du **champ Date de naissance** sur la page d’inscription des clients a été corrigé. Auparavant, le zéro de début qui précédait un jour à un chiffre était absent.
...
* Les améliorations suivantes ont été apportées aux `LoginAsCustomer` modules :

   * Remplacement du `around` module externe par`after`
...
* Le rechargement de toutes les sections de données client par caractère générique (*) n’entraîne plus l’envoi d’une `customer/section/load` erreur 400 aux demandes. [GitHub-28154](https://github.com/magento/magento2/issues/28154)

<!-- ENGCOM-7994 -->

* L’application transmet maintenant les exceptions qui se produisent lorsqu’une mise en page est rendue en mode de production à `var/report`. [GitHub-29606](https://github.com/magento/magento2/issues/29606)

<!-- MC-35058-->

* Les marchands peuvent désormais annuler l’attribution de produits des catégories comme prévu. Auparavant, l’application générait une erreur semblable à celle-ci : `Could not save product "4" with position 0 to category 3`.

<!-- ENGCOM-7527 -->

* L’application lève désormais une exception lorsqu’un client qui n’existe pas demande une réinitialisation du mot de passe. [GitHub-26288](https://github.com/magento/magento2/issues/26288)
...
* Le sélecteur `input[type=datetime]` d’attributs a été remplacé par un sélecteur de type d’entrée moins spécifique. [GitHub-30064](https://github.com/magento/magento2/issues/30064)

<!-- ENGCOM-8282 -->

* L’application affiche désormais des valeurs spécifiques au magasin qui sont associées aux attributs de catégorie personnalisés, comme prévu. [GitHub-13440](https://github.com/magento/magento2/issues/13440)
...
* La `\Magento\Catalog\Model\ImageUploader` classe a été restructurée afin d’utiliser la nouvelle `moveFileFromTmp` méthode. [GitHub-29598](https://github.com/magento/magento2/issues/29598)
...
* Les `framework/database/select` `where` fonctions se gèrent `type` maintenant correctement. [GitHub-29590](https://github.com/magento/magento2/issues/29590)

<!-- ENGCOM-8260 -->

* L’utilisation inefficace des boucles in a été améliorée dans l’ensemble de la base de code, ce qui a amélioré les performances à plusieurs endroits, y compris dans le processus de `array_merge` déploiement de contenu statique. [GitHub-30183](https://github.com/magento/magento2/issues/30183)

<!-- ENGCOM-8445 -->

* La durée de vie par défaut de la session Admin a été déplacée du module de chiffrement vers le module de sécurité. [GitHub-30859](https://github.com/magento/magento2/issues/30859)
...
* Ajout de la prise en charge de `root` en mode de maintenance et des pages erreurs liées par un lien symbolique. Auparavant, si les ou `root` `magento/pub` étaient reliés symboliquement dans un répertoire différent, le processeur d’erreur (`Magento\Framework\Error\Processor`) ne générait pas une URL de fichier d’affichage correcte. [GitHub-30296](https://github.com/magento/magento2/issues/30296)

<!-- ENGCOM-8087 -->

* Ajout du tableau manquant `order_data` aux `EmailSender` classes. Cela permet d’aligner ces classes sur la recommandation de Magento d’utiliser des variables scalaires au lieu d’objets dans les modèles de courrier électronique. [GitHub-29604](https://github.com/magento/magento2/issues/29604)
...
* Les requêtes de données d’un produit ne renvoient plus de données de chemin de `categories.breadcrumbs` navigation pour les catégories dont la catégorie parent est désactivée. [GitHub-30468](https://github.com/magento/magento2/issues/30468)
...
* L’application renvoie maintenant un message plus informatif lorsqu’une erreur se produit lors d’une opération de réinitialisation de mot de passe GraphQL. [GitHub-30179](https://github.com/magento/magento2/issues/30179)

<!-- MC-38078-->

* Les positions des produits sont maintenant mises à jour comme prévu dans les résultats de requête GraphQL lorsque les résultats de la requête sont mis en cache.

<!-- MC-35355-->

* La `products` requête renvoie désormais les résultats attendus lorsque le `category_id` filtre est spécifié avec le `in` mot-clé. [GitHub-30349](https://github.com/magento/magento2/issues/30349)
...
* La classe `FlushCacheByTags` a été mise à jour afin d’utiliser des plug-ins au lieu des plug-ins environnants. [GitHub-29558](https://github.com/magento/magento2/issues/29558)

<!-- ENGCOM-8370 -->

* Code dans le `checkout_index_index.xml` fichier a été améliorée par la suppression de `messages``sortOrder` , `authentication`, `progressBar`, `estimation`et `sidebar checkout` des composants. [GitHub-30550](https://github.com/magento/magento2/issues/30550)
...
* Le hachage redondant de `req.http.host/client.ip` a été supprimé et le hachage est géré par la VCL intégrée à Varnish. [GitHub-29988](https://github.com/magento/magento2/issues/29988)

<!-- ENGCOM-8302 -->

* Ajout d’un rappel d’échec au téléchargeur de `Magento_Ui` fichiers. [GitHub-29557](https://github.com/magento/magento2/issues/29557)
...
* GraphQL honore désormais les autorisations de catalogue. L’application limite les articles renvoyés pour une `products` requête en fonction du groupe de clients du consommateur.

<!-- MC-39690-->

* Ajout de l’attribut `customizable_option` à plusieurs types de données qui implémentent le `CartItemInterface`fichier .

<!-- MC-39744-->

* Ajout d’attributs pour la requête afin de renvoyer des informations sur le magasin, le `storeConfig` groupe de magasins et le site Web.
...
* La `products` requête renvoie la devise correcte pour les vues de magasin comportant plusieurs devises.

<!-- MC-37727-->

* Les étiquettes des `products` filtres de requête renvoient désormais les valeurs traduites, le cas échéant pour les vues de magasin autres que celles par défaut.

<!-- MC-37728-->

* Montant fixe des devises de la boutique sur les cartes-cadeaux du panier.

<!-- MC-38443-->

* Ajout du type de données, qui contient les `ConfigurableProductOptionsSelection` métadonnées correspondant aux options configurables sélectionnables pour un produit. Utilisez cet objet dans une `products` requête pour réduire le nombre d’éléments de la galerie multimédia affichés lorsque l’acheteur sélectionne les options de produit configurables.
...
* Les couleurs des images CMJN sont désormais correctement rendues. Lorsque l’adaptateur Imagick détecte désormais qu’une image ouverte possède un espace colorimétrique CMJN, l’espace colorimétrique est converti en SRVB. Auparavant, ces images étaient rendues avec des couleurs inversées. [GitHub-22375](https://github.com/magento/magento2/issues/22375)

<!-- ENGCOM-8251 -->

* Les valeurs booléennes ne sont plus définies pour `image_with_border.phtml`. Les paramètres et redondants `max-width` `height` ont été supprimés de `.product-image-photo`. (Ces valeurs sont déjà définies globalement sur l’image par le biais des styles de réinitialisation.) [GitHub-30186](https://github.com/magento/magento2/issues/30186)

### Importer/exporter

<!-- MC-35081-->

* Une valeur n’est plus attribuée `NULL` à l’adresse `region_id` du client lorsque vous importez des adresses client à l’aide d’un fichier CSV (`entity type = "customer address"` et `import behavior = "add/update"`) dont certaines valeurs de champ ont été supprimées.

<!-- MC-35295-->

* Vous pouvez désormais masquer les images des produits sur la vitrine pendant l’importation.
...
* Le fichier CSV d’exportation de produit apparaît maintenant dans la liste d’administration des fichiers CSV d’exportation comme prévu. Auparavant, l’application générait une erreur lorsque vous exécutiez cette commande : `bin/magento queue:consumers:start exportProcessor --single-thread --max-messages=10000`.

<!-- MC-37672-->

* L’application ne génère plus d’erreur lors de l’importation de données CSV contenant des caractères alphabétiques en bengali (bengali).
...
* La limite de 90 caractères pour les noms de fichiers image a `vendor/magento/framework/File/Uploader.php` été supprimée.  Auparavant, la longueur du répertoire et des noms de fichier ne pouvait pas dépasser 255 caractères. L’application a importé des produits avec des images dont le nom de fichier dépassait cette limite, mais n’a pas importé le fichier image. [GitHub-29377](https://github.com/magento/magento2/issues/29377)
...
* Nous avons amélioré les performances de l’indexeur partiel. L’application efface désormais les ID de cache qui ont été modifiés pour chaque itération de lot de 1000 ID. Le contexte de cache n’accumule plus d’ID. Auparavant, le contexte du cache accumulait des identifiants de cache pour chaque itération de lot de 1000 ID et effaçait le même ensemble d’identifiants à chaque itération.

<!-- ENGCOM-7929 -->

* Les incohérences entre le système de réindexation partielle et le système de réindexation complet ont été corrigées. Les deux systèmes d’indexation utilisent désormais le même code, qui fonctionne correctement. Auparavant, les deux systèmes d’indexation utilisaient une logique différente en ce qui concerne les indexeurs partagés, ce qui entraînait potentiellement l’annulation des indexeurs après chaque exécution du système de réindexation partielle. [GitHub -29478](https://github.com/magento/magento2/issues/29478), [GitHub-29297](https://github.com/magento/magento2/issues/29297)
...
* Nous avons résolu des problèmes liés à l’indexeur manuel qui entraînaient des pages de catégorie vides ou une réduction du nombre de produits sur les pages de catégorie. Des problèmes avec des pages de catégorie incomplètes ou un nombre de produits se sont produits lorsque :
   ...
   * L’indexeur partiel a été exécuté sur l’un ou l’autre `catalog_category_product` des indices ou `catalog_product_category` , et en même temps un réindexation complet a été exécuté sur l’un de ces indices. Ces deux processus d’indexation pouvaient entrer en conflit, ce qui entraînait l’absence de produits dans les pages de catégorie.

   * `bin/magento indexer:reindex inventory` a été exécutée sur un catalogue volumineux. Les produits étaient absents des pages de catégories jusqu’à ce que l’indexeur partiel termine l’exécution `catalogsearch_fulltext` .

<!-- MC-39700-->

* Un indexeur a été ajouté à la `Magento_Bulk` table MySQL, ce qui a amélioré les performances des opérations en masse.
...
* Vous pouvez désormais utiliser `app/etc/env.php` pour modifier le courtier de messages MYSQL en AMQP.

<!-- MC-38242-->

* Les problèmes liés au calcul des reliquats pour les valeurs décimales pour les quantités de produits ont été corrigés dans la bibliothèque de JavaScript pertinente. L’application calculait auparavant de manière incorrecte les fractions décimales de plus de deux chiffres et affichait par la suite un message incorrect.

<!-- MC-38993 ENGCOM-8473-->

* La `\Magento\Authorization\Model\Rules::update` méthode est obsolète.  [GitHub-29128](https://github.com/magento/magento2/issues/29128)
...
* Image uploader affiche désormais la taille d’image correcte lorsqu’un rendu est inséré à partir de Media Gallery. [GitHub-1806](https://github.com/magento/adobe-stock-integration/issues/1806)
...
* Ajout de groupes d’actions `Analytics` au module. [GitHub-29500](https://github.com/magento/magento2/issues/29500)

<!-- ENGCOM-7334 -->

* Mise à jour `CheckCheckoutSuccessPageAsRegisterCustomerTest` et `CheckCheckoutSuccessPageAsGuestTest` annulation des commandes créées après l’exécution des tests. [GitHub-28324](https://github.com/magento/magento2/issues/28324)
...
* Implémenté `AdminOpenCreateNewCMSPageActionGroup` pour naviguer afin de créer une page CMS. [GitHub-29824](https://github.com/magento/magento2/issues/29824)

<!-- ENGCOM-8268 -->

* Correction de problèmes liés au saut dans les tests MFTF pour la page CMS de la galerie multimédia. [GitHub-30164](https://github.com/magento/magento2/issues/30164)

#### Nouveaux tests

<!-- ENGCOM-7804 -->

* Ajout d’un test d’activation de la messagerie électronique à un ami dans l’administration. [GitHub-29145](https://github.com/magento/magento2/issues/29145)
...
* `Magento\Framework\DB\Adapter\Pdo\Mysql->isTableExists` a été refactorisé pour améliorer les performances. Les appels redondants pour `SHOW TABLE STATUS` vérifier l’existence d’une table ont été réduits. [GitHub-29662](https://github.com/magento/magento2/issues/29662)

<!-- ENGCOM-8258 -->

* `getPageLayoutsConfig` est désormais enregistré dans le cache, ce qui a amélioré les performances de Config Builder. [GitHub-29546](https://github.com/magento/magento2/issues/29546)

### Persistant

<!-- MC-36647 -->

* Les acheteurs ne peuvent plus commander une fois que le cookie de session a expiré lorsque le panier persistant est activé. Au lieu de cela, l’application affiche la page de connexion afin que l’acheteur puisse se connecter. Auparavant, l’application passait la commande. L’application n’a pas correctement validé le client persistant à `PHPSESSID` son expiration et `customerSession` a été recréée à partir des informations persistantes.
...
* L’application n’affiche plus le contenu du panier d’un client après l’expiration de la session client. Auparavant, l’application affichait cette erreur lorsqu’un client invité tentait de passer à la caisse lorsque le panier persistant était activé : `No such entity with cartid = 0`.

### Prisée

<!-- MC-34394-->

* Les mises à jour de prix asynchrones consécutives n’interfèrent plus les unes avec les autres et le statut correct est attribué à chaque opération. Auparavant, si une mise à jour de prix asynchrone échouait, toutes les mises à jour de prix asynchrones suivantes échouaient jusqu’à ce que le consommateur de file d’attente correspondant soit redémarré. [GitHub-27865](https://github.com/magento/magento2/issues/27865)
...
* La recherche avancée ne renvoie plus les enfants des produits configurables en tant que résultats de recherche individuels lorsque les produits enfants ont été configurés avec la visibilité définie sur **Non visible individuellement**.
...
* Les champs qui décrivent le mode de livraison UPS sur les magasins d’administration **>** la configuration **>** les modes **de livraison des ventes** > **sont maintenant activés comme** prévu.
...
* Vous pouvez maintenant créer un libellé d’expédition comme prévu pour une commande partielle. Auparavant, lorsque vous tentiez de créer une étiquette d’expédition pour un seul article d’une commande, l’application ajoutait tous les produits au colis et la validation échouait. [GitHub-29552](https://github.com/magento/magento2/issues/29552)

<!-- ENGCOM-8154 -->

* La validation du mode d’expédition n’est plus ignorée lors de l’appel `/V1/guest-carts/:cartId/totals-information`du fichier . [GitHub-25147](https://github.com/magento/magento2/issues/25147)

### Plan du site

<!-- MC-37306-->

* Les plans de site générés par `cron` incluent désormais des URL d’image correctes. Auparavant, le chemin d’accès de l’image mise en cache générée était incorrect dans les déploiements multi-magasins.
...
* `\Magento\TestFramework\TestCase\WebapiAbstract` contient désormais une fonction qui prend en charge la comparaison de grands tableaux imbriqués de résultats attendus et réels dans les tests. Les clés non pertinentes du résultat peuvent désormais être ignorées. [GitHub-29498](https://github.com/magento/magento2/issues/29498)

<!-- ENGCOM-8024 -->

* `AdminMediaGalleryCatalogUiEditCategoryGridPageTest` n’échoue plus de façon aléatoire. [GitHub-1764](https://github.com/magento/adobe-stock-integration/issues/1764)

<!-- ENGCOM-8041 -->

* `dev/tests/integration/testsuite/Magento/Customer/Controller/AccountTest` a été améliorée. `assert`( est désormais basé sur `XPath` le sélecteur au lieu de `assertStringContainsString`.) [GitHub-29700](https://github.com/magento/magento2/issues/29700)
...
* Un nouvel `--no-parent` indicateur a été ajouté à la `bin/magento setup:static-content:deploy` commande qui empêche le ou les parents d’un thème d’être compilés. Grâce à ce nouvel indicateur, vous pouvez améliorer considérablement les performances du processus de déploiement de contenu statique en évitant la compilation inutile. Notez que ce nouvel indicateur ne fonctionne pas lors de l’utilisation de la `compact` stratégie. [GitHub-30184](https://github.com/magento/magento2/issues/30184)

<!-- MC-37630 -->

* La configuration de la conception ne génère plus d’opérations DDL. Auparavant, lorsque la configuration de conception était mise à jour, l’application générait des instructions DDL qui pouvaient déclencher des erreurs MySQL.

<!-- ENGCOM-8112 -->

* Le code redondant dans le `Magento_ConfigurableProduct` module du thème Admin a été supprimé.  [GitHub-29857](https://github.com/magento/magento2/issues/29857)
...
* Ajout d’une fonctionnalité de navigation réductible au thème Vide dans la vue mobile. [GitHub-30237](https://github.com/magento/magento2/issues/30237)
...
* Le sélecteur de date de la grille Commandes d’administration fonctionne désormais comme prévu lorsque les paramètres régionaux d’administration sont l’arabe (`ar_SA - Saudi Arabia`).

<!-- MC-38476 -->

* L’application n’affiche plus de message d’erreur lorsqu’un acheteur entre une adresse argentine avec un code postal valide lors de l’inscription ou de l’ajout d’une nouvelle adresse. Auparavant, l’application affichait cette erreur : `Provided Zip/Postal Code seems to be invalid. Example: 1234. If you believe it is the right one you can ignore this notice.`

<!-- MC-38663-->

* L’application ne renvoie plus d’erreur lorsqu’un acheteur entre un code postal à cinq chiffres pour une adresse coréenne.

<!-- ENGCOM-8244 -->

* Ajout de la prise en charge des régions de l’Uruguay définies dans [l’ISO 3166-2 :UY.](https://en.wikipedia.org/wiki/ISO_3166-2:UY) [GitHub-29729](https://github.com/magento/magento2/issues/29729)
...
* L’application affiche désormais la case à cocher Identique à l’adresse **** de facturation, comme prévu sur la page de commande, lorsque des produits ont été ajoutés au panier par SKU.
...
* Le **bouton Prévisualiser le modèle** fonctionne maintenant comme prévu sur la page Modifier la file d’attente.
...
* Ajout d’une variable lourde d’épaisseur de police à `lib/web/css/source/lib/variables/_typography.less`. [GitHub-29778](https://github.com/magento/magento2/issues/29778)

<!-- ENGCOM-8211 -->

* La visibilité des filtres fonctionne maintenant avec la visibilité des colonnes, comme prévu dans les grilles d’administration.  [GitHub-30345](https://github.com/magento/magento2/issues/30345)
...
* L’utilisation de POST `V1/invoices/:id/capture` pour capturer les informations de paiement fonctionne maintenant comme prévu. Auparavant, l’application autorisait la commande mais ne la capturait que sur le site de la passerelle de paiement.
...
**Problème** : le `[magento_root]/index.php` fichier a été supprimé et l’application s’exécute `/pub` désormais par défaut pour les configurations Apache. Les magasins servis à partir de sous-dossiers ne fonctionneront pas comme prévu et peuvent afficher des erreurs 404. **** Solution : utilisez des liens symboliques pour émuler l’installation dans un sous-dossier. L’exemple suivant utilise `https://shop01.com/shop/` pour illustrer comment utiliser un lien symbolique pour émuler une installation dans un sous-dossier.
...
1. Créez un `index.php` fichier dans le nouveau répertoire (`magento_root/pub/shop01/index.php`) et ajoutez le contenu suivant :
...
Vous pouvez installer Magento Open Source 2.4.2 à l’aide de [Composer](../../../installation/composer.md).
