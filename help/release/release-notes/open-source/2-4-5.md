...
Consultez le [Bulletin de sécurité d’Adobe](https://helpx.adobe.com/security/products/magento/apsb22-38.html) pour connaître la dernière discussion sur ces problèmes résolus.
...
* Les sous-totaux des mini-paniers sont désormais mis à jour correctement lorsqu’un acheteur navigue de la page d’expédition à la page du panier dans le processus de paiement pour une commande avec plusieurs adresses de livraison. Auparavant, le sous-total était doublé.
...
* La `addConfigurableProductsToCart` mutation fonctionne maintenant comme prévu avec plusieurs produits. Auparavant, des informations incorrectes sur le produit étaient renvoyées ou un message d’erreur non valide était renvoyé. [GitHub-30948](https://github.com/magento/magento2/issues/30948)
...
* Les avis d’obsolescence ne se produisent plus dans les tests unitaires en raison de `\DateTimeFormatter::formatObject()`. Cette méthode fonctionne maintenant comme prévu avec des valeurs numériques pour `$format`.
...
* Magento Open Source ne renvoie plus d’erreur lorsqu’un administrateur dont les rôles sont étendus à un seul site Web ajoute un produit aux éléments de contenu à l’aide de PageBuilder. Auparavant, l’applicationOpen Source générait une erreur SQL.

<!-- AC-197-->

* La validation a été ajoutée à la deuxième ligne de l’adresse sur les pages Modifier et Ajouter une adresse. Les longueurs de texte minimale et maximale spécifiées sur la seconde sont désormais appliquées.
...
* Mise à jour des libellés et descriptions des commentaires dans `app/code/Magento/NewRelicReporting/etc/adminhtml/system.xml`. [GitHub-31947](https://github.com/magento/magento2/issues/31947)

<!-- ENGCOM-8600-->

* Suppression des fichiers inutiles `csp_whitelist.xml` . [GitHub-30607](https://github.com/magento/magento2/issues/30607)
...
* Les `new_from_data` et `new_to_datefields` dans `ProductInterface` ne sont plus obsolètes. [GitHub-34783](https://github.com/magento/magento2/issues/34783)
...
* La `urlResolver` requête résout désormais correctement le délimiteur de chemin (/) lorsque plusieurs pages d’accueil ont le même identifiant. Auparavant, la requête ne résolvait pas le délimiteur et renvoyait la valeur null. [GitHub-33615](https://github.com/magento/magento2/issues/33615)
...
* Regrouper les produits contenant un point d’interrogation ( ?) dans le titre de l’option peut maintenant être importé avec succès en raison des améliorations apportées à la méthode Query Builder Inside `populateExistingOptions` . Le titre de l’option s’affiche également correctement. Auparavant, après l’importation initiale, les importations successives entraînaient un comportement corrompu et doublaient les options. Les acheteurs ne pouvaient pas non plus ajouter le produit au panier.

<!-- ACP2E-766-->

* Ajout d’une validation des noms de catégorie lors de l’importation. Auparavant, l’applicationOpen Source ne validait pas les noms de catégorie, ce qui entraînait des erreurs lorsque les noms de catégorie dépassaient 255 caractères.
...
* La logique liée à la barre d’outils de l’éditeur TinyMCE dans le module Page Builder a été mise à jour suite à l’introduction d’une `delayedRender` logique pour la barre d’outils dans TinyMCE.
...
* Les observateurs placés sur `sales_order_state_change_before` prennent désormais en charge la récupération des données de l’objet order. L’argument `event` `sales_order_state_change_before` a été mis à jour. [GitHub-26789](https://github.com/magento/magento2/issues/26789)
...
* Vous pouvez désormais utiliser le symbole deux-points dans une `htmlClass` valeur d’attribut, ce qui prend en charge l’utilisation de composants supplémentaires tels que l’interface utilisateur Tailwind. [GitHub-34430](https://github.com/magento/magento2/issues/34430)
...
* Les administrateurs peuvent désormais passer une commande auprès de l&#39;administrateur à l&#39;aide du mode de paiement PayPal PayflowPro. Auparavant, l’applicationOpen Source affichait cette erreur : `No such entity with cartId = 0`.
...
* La `cart` requête ne renvoie plus tous les modes de paiement actifs pour les commandes gratuites. [GitHub-34036](https://github.com/magento/magento2/issues/34036)
...
* Le sélecteur d’affichage du magasin ne bloque plus le volet de traduction lorsque vous modifiez le texte ou les étiquettes de l’administrateur. Vous pouvez désormais modifier ces fonctionnalités à partir du volet de traduction et l’interface affiche ces modifications lorsque vous cliquez sur **Envoyer**.
...
**Problème** : les utilisateurs administrateurs ne peuvent pas commander ni commander à nouveau des clients auprès de l’administrateur lorsque Braintree est activé. Lorsque l’utilisateur administrateur clique sur **Commander** ou **Réorganiser**, Adobe Commerce n’envoie pas la commande et le `system.log` affiche le message d’erreur suivant : `report.CRITICAL: Error: Call to a member function getMethodInstance() on null in /app/vendor/paypal/module-braintree-core/Block/Form.php:174`.  **Solution de contournement** :  `BUNDLE-3137-composer.patch` est désormais disponible. Consultez l’article de la [Base de connaissances L’administrateur ne peut pas créer de commande/réorganiser lorsque Braintree paiement activé](https://support.magento.com/hc/en-us/articles/8322266352525) pour une discussion sur ce problème et l’accès au correctif. Un correctif sera également inclus dans Adobe Commerce 2.4.5-p1. <!-- AC-6222-->
