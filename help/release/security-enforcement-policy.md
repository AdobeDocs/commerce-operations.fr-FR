...
badgePaas: label="Adobe Commerce on Cloud uniquement" type="Informative" url="https://experienceleague.adobe.com/fr/docs/commerce/user-guides/product-solutions" tooltip="S’applique uniquement à Adobe Commerce sur les projets cloud."
hide: true
...
# Politique d&#39;application de la sécurité : Mesures et échéances requises
...
La politique de sécurité d’entreprise d’Adobe exige que tous les environnements hébergés par Adobe pour Adobe Commerce on Cloud s’exécutent sur des logiciels sécurisés et conformes.
...
1. Adobe Commerce on Cloud (version 2.4.8, 2.4.9 ou version la plus récente)

Cela permet de réduire les risques de sécurité dans vos environnements d’e-commerce. Le trafic entrant sera suspendu pour les environnements qui ne respectent pas ces exigences dans les délais indiqués dans le [Tableau 1](#determine-your-required-actions), ce qui mettra le storefront hors ligne. Veuillez considérer cet avis comme une exigence de sécurité et de conformité avec des dates d&#39;application.
...
Dans le tableau suivant, recherchez votre version d’Adobe Commerce on Cloud ci-dessous pour voir ce que vous devez faire.
...
| 2.4.8 ou 2.4.9 | Action requise avant le 30 octobre 2026 ou le 31 mai 2027, selon le logiciel. | Aucune action requise pour le moment |

## Qui n’a pas besoin d’agir ?

Le présent avis ne s&#39;applique pas :

* Clients utilisant [!DNL Adobe Commerce as a Cloud Service]
* Clients utilisant Adobe Commerce sur le cloud version 2.4.8 ou 2.4.9 avec dépendances logicielles prises en charge dans tous les environnements

### Vérification des versions actuelles

Vous avez besoin de l’aide de votre administrateur eCommerce pour passer en revue les étapes suivantes afin de vérifier quelle version vous exécutez dans chacun de vos environnements Adobe Commerce sur Cloud.

#### Étape 1 : vérifier la version d’Adobe Commerce on Cloud
...
#### Étape 2 : vérifier les versions de dépendance logicielle
...
>[!WARNING]
>
>Si votre environnement ne répond pas aux exigences dans les délais détaillés dans le tableau [Actions requises et échéances](#determine-your-required-actions), Adobe sera forcé de suspendre le trafic entrant vers l&#39;environnement affecté. Votre storefront d’e-commerce sera déconnecté et ne servira pas les acheteurs.

## Ce que requiert chaque action

### Action 1 : mise à niveau des dépendances logicielles tierces

Selon le logiciel, toutes les dépendances de logiciel non prises en charge doivent être mises à niveau en fonction des délais partagés dans le tableau ci-dessous. Vous pouvez afficher vos environnements dans la [console cloud](https://console.adobecommerce.com/). Pour vérifier les versions dépendantes exécutées dans chaque environnement, consultez [Vérification des versions dépendantes des logiciels](#check-software-dependency-versions). Les mises à niveau des dépendances logicielles s’appliquent à toutes les versions d’Adobe Commerce on Cloud 2.4.4 à 2.4.9.
...
* **Prise en main :** ouvrez un ticket d’assistance répertoriant les environnements à mettre à niveau et les dépendances impliquées. Ouvrez votre ticket au moins 30 jours avant votre date d&#39;application afin que notre équipe puisse planifier le travail.
...
### Action 2 : mettre à niveau vers une version d’Adobe Commerce prise en charge

Si vous devez mettre à niveau votre version d’Adobe Commerce on Cloud, vous disposez de deux options :

1. [Mise à niveau vers une version d’Adobe Commerce on Cloud prise en charge](#upgrade-to-adobe-commerce-on-cloud-version-249)
1. [Migration vers Adobe Commerce as a Cloud Service (plateforme SaaS)](#migrate-to-adobe-commerce-as-a-cloud-service)

La date d’application de votre version actuelle s’applique quelle que soit l’option choisie.

**Tableau 3 : instructions et délais de mise à niveau vers une version d’Adobe Commerce on Cloud prise en charge**
...
Pour décider de l’option qui correspond à vos besoins, consultez le tableau ci-dessous qui compare Adobe Commerce on Cloud version 2.4.9 à Adobe Commerce as a Cloud Service.

**Tableau 4 : Adobe Commerce sur le cloud par rapport à Adobe Commerce as a Cloud Service**
...
## Que se passera-t-il si vous n&#39;agissez pas?

Si un environnement n’a pas satisfait à ces exigences aux dates d’application indiquées dans [Déterminer les actions requises](#determine-your-required-actions), Adobe prendra les mesures appropriées. Cela inclut la suspension du trafic vers l’infrastructure affectée, ce qui entraîne la déconnexion de votre storefront d’e-commerce.
...
### Mise à niveau vers Adobe Commerce sur Cloud version 2.4.9
...
* **Migration des storefronts et des extensions :** Adobe [migration assistée par l’IA et outils de développement](https://developer.adobe.com/commerce/extensibility/developer-agent/), notamment [!DNL Adobe Developer App Builder] et [!DNL Commerce Storefront powered by Edge Delivery Services], permet d’accélérer la modernisation des storefronts et la reconfiguration des extensions.
