...
1. [Configurer](https://experienceleague.adobe.com/docs/commerce-merchant-services/user-guides/saas.html) une nouvelle clé API.
...
1. [Configuration des clés d’API](https://experienceleague.adobe.com/docs/commerce-merchant-services/user-guides/saas.html) pour connecter l’extension à votre système.
...
1. Téléchargez le fichier binaire et décompressez-le.
   ...
   1. Incorporez l’archive du lanceur.
...
1. Création d’un fichier d’unité systemd `(/etc/systemd/system/scheduler.service)` avec la configuration suivante (remplacez `<filesystemowner>` avec l’utilisateur Unix propriétaire du répertoire dans lequel l’agent est installé).
