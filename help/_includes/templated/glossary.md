...
En savoir plus : [Guide de l’utilisateur d’Adobe Commerce](https://experienceleague.adobe.com/docs/commerce-admin/user-guides/home.html)
...
En savoir plus : [Devise](https://experienceleague.adobe.com/docs/commerce-admin/stores-sales/site-store/currency/currency.html)
...
[Blocs](https://experienceleague.adobe.com/docs/commerce-admin/content-design/elements/blocks/blocks.html) sont implémentés et fournis par les modules.
...
[Blocs dynamiques](https://experienceleague.adobe.com/docs/commerce-admin/content-design/elements/dynamic-blocks/dynamic-blocks.html) fournissent du contenu basé sur une logique, telle que des règles de prix.

Page Builder se développe sur l’interactivité et la création de [blocs](https://experienceleague.adobe.com/docs/commerce-admin/page-builder/add-content/block.html) et [blocs dynamiques](https://experienceleague.adobe.com/docs/commerce-admin/page-builder/add-content/dynamic-block.html).
...
Identité unique qui définit un produit ou un groupe de produits spécifique par le fabricant ou Designer.
...
Pour [Inventory management](https://experienceleague.adobe.com/docs/commerce-admin/inventory/sources/sources-manage.html) et [Gestion des commandes](https://omsdocs.magento.com/getting-started/terminology/), ce magasin est une source permettant de suivre les quantités de produits, les commandes d’expédition et la prise en charge de la récupération en magasin.
...
Les variables de cloud sont des variables d’environnement spécifiques à Adobe Commerce sur l’infrastructure cloud et utilisent la variable **`MAGENTO_CLOUD`** préfixe.
...
Une variante spéciale de [block](https://experienceleague.adobe.com/docs/commerce-admin/content-design/elements/blocks/blocks.html) qui ne peuvent être créés que dans l’administrateur et ne peuvent pas être référencés par le biais de fichiers de mise en page.
...
En savoir plus : [Obtention des clés d’authentification](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/prerequisites/authentication-keys.html)
...
[Configuration du balisage](https://experienceleague.adobe.com/docs/commerce-admin/catalog/products/settings/settings-advanced-custom-options.html), ou markdown, d’un produit par le biais d’options personnalisables de produit.
...
[OMS](https://omsdocs.magento.com) est l’offre Système de gestion des commandes d’Adobe.
...
En savoir plus : [Fermeture à l&#39;origine](https://experienceleague.adobe.com/docs/commerce-knowledge-base/kb/faq/fastly-origin-cloaking-enablement-faq.html)
...
En savoir plus : [Guide de l’utilisateur de Page Builder](https://experienceleague.adobe.com/docs/commerce-admin/page-builder/introduction.html), [Documentation de développement du créateur de pages](https://developer.adobe.com/commerce/frontend-core/page-builder/)
...
Court pour le modèle de HTML ou le modèle PHTML.
Un fichier PHTML contient un mélange de balisage de HTML et de code PHP pour injecter du contenu dynamique dans le HTML.
La plupart des blocs comportent au moins un fichier PHTML (template) contenant le HTML statique généré par le bloc.
Dans l’Admin, les modèles d’email et de newsletter combinent du texte, des images et des variables avec des balises de HTML pour produire du contenu personnalisé envoyé par le système.
...
[PWA Studio](https://github.com/magento/pwa-studio) uses [UPWARD](https://developer.adobe.com/commerce/pwa-studio/guides/packages/upward/) dans le développement.
...
Cela permet à un Progressive Web Application de s’exécuter sur un serveur conforme UPWARD dans n’importe quelle langue sur une pile technologique, car l’application n’est concernée que par le comportement du point de terminaison HTTP du serveur UPWARD.
...
Les types virtuels ne peuvent être référencés que par le remplacement d’arguments dans une `<type>` dans les fichiers di.xml, jamais dans le code source.
...
A [widget](https://experienceleague.adobe.com/docs/commerce-admin/content-design/elements/widgets/widgets.html) est un fragment de code préparé qui peut être utilisé pour placer des blocs, des liens et du contenu dynamique à des emplacements spécifiques sur les pages de magasin.
