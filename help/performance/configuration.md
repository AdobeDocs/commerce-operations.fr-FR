...
Toutes les opérations asynchrones dans Commerce sont effectuées à l’aide de la commande Linux `cron`. Voir [Configurer et exécuter cron](../configuration/cli/configure-cron-jobs.md) pour le configurer correctement.
...
>[!INFO]
>
>Voir [Optimiser les fichiers de ressources](https://experienceleague.adobe.com/en/docs/commerce-operations/implementation-playbook/best-practices/development/optimize-css-js-files) pour plus d’informations sur l’optimisation du code CSS et JavaScript.

### Conseils sur le regroupement {#bundling-tips}

* Nous vous recommandons d’utiliser des outils tiers pour la minimisation et le regroupement (comme [r.js](https://requirejs.org/)). Les mécanismes intégrés de Commerce ne sont pas optimaux et sont fournis en tant qu’alternatives de secours.
...
Ce paramètre améliore les performances de la validation des segments clients s’il y a de nombreux segments clients dans le système. Cependant, il ne fonctionne pas avec les implémentations [base de données partagée](../configuration/storage/multi-master.md) ou lorsqu’il n’y a aucun client enregistré.
