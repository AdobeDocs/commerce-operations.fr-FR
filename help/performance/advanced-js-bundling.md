...
Dans une application modulaire, le nombre de requêtes du serveur peut atteindre des centaines. Par exemple, la capture d’écran ci-dessous montre uniquement le début de la liste des modules JavaScript chargés sur la page d’accueil d’une installation nettoyée.

![Pas de groupement](../assets/performance/images/noBundling.png)

## Fusion et regroupement

Commerce prend en charge le regroupement pour réduire le nombre de requêtes serveur. Le regroupement est désactivé par défaut. Vous pouvez l’activer dans **[!UICONTROL Stores]** > **Paramètres** > **[!UICONTROL Configuration]** > **[!UICONTROL Advanced]** > **[!UICONTROL Developer]** > **[!UICONTROL JavaScript Settings]** ou à partir de la ligne de commande.

Consultez les [Conseils sur le regroupement](configuration.md#bundling-tips) dans *Bonnes pratiques de configuration* pour obtenir des outils tiers, le HTTP/2, ainsi que des conseils sur la fusion JS et CSS obsolète.
...
Le regroupement de Commerce réduit le nombre de connexions par page, mais pour chaque requête de page, il charge tous les lots, même si la page demandée ne peut dépendre que de fichiers d’un ou de deux des lots. Les performances s’améliorent une fois que le navigateur met les lots en cache. Cependant, comme le navigateur charge ces lots de manière synchrone, la première visite de l’utilisateur dans un storefront Commerce peut prendre un certain temps pour s’afficher et nuire à l’expérience de l’utilisateur.

### Fusion de base (non recommandée)

>[!NOTE]
>
>Nous déconseillons d’utiliser **[!UICONTROL Merge JavaScript Files]**. Ce paramètre a été conçu uniquement pour le JavaScript chargé de manière synchrone dans la section HEAD de la page. Il peut entraîner un fonctionnement incorrect de la logique de regroupement et de [!DNL RequireJS]. Il est conservé uniquement à des fins de rétrocompatibilité et ne présente aucun avantage en termes de performances lorsque le HTTP/2 est activé.
...
Cette commande fusionne tous les fichiers JavaScript synchrones en un seul fichier. Il n’est pas utile d’activer la fusion sans activer le regroupement, car Commerce utilise [!DNL RequireJS]. Si vous n’activez pas le regroupement, Commerce fusionne uniquement [!DNL RequireJS] et sa configuration. Lorsque vous activez le regroupement et la fusion, Commerce crée un seul fichier JavaScript :
...
Avec une connectivité 3G lente, il faut environ 44 secondes pour charger tous les lots pour la page d’accueil d’une installation Commerce propre.
...
Grâce à une approche plus avancée du regroupement JavaScript, nous pouvons améliorer ces temps de chargement.

## Groupement avancé

N’oubliez pas que l’objectif du regroupement JavaScript est de réduire le nombre et la taille des ressources demandées pour chaque page chargée dans le navigateur. Pour ce faire, nous voulons créer nos bundles afin que chaque page de notre magasin n’ait à télécharger qu’un bundle commun et un bundle spécifique à chaque page consultée.

Pour ce faire, définissez vos lots par types de page. Vous pouvez catégoriser les pages de Commerce en plusieurs types de page, y compris Catégorie, Produit, CMS, Client, Panier et Passage en caisse. Chaque page classée dans l’un de ces types de page comporte un ensemble différent de dépendances de module [!DNL RequireJS]. Lorsque vous regroupez vos modules [!DNL RequireJS] par type de page, vous n’obtenez qu’une poignée de lots qui couvrent les dépendances de n’importe quelle page de votre magasin.
...
Une installation Commerce épurée permet d’obtenir des performances suffisantes en divisant les lots par types de page, mais certaines personnalisations peuvent nécessiter une analyse plus approfondie et d’autres distributions de ressources.
...
![[!DNL RequireJS] la configuration](../assets/performance/images/RequireJSConfig.png)
...
1. [!DNL RequireJS] dans la console de votre navigateur.

#### Pour utiliser [!DNL PhantomJS] :

Dans le répertoire racine Commerce, créez un fichier appelé `deps.js` et copiez-le dans le code ci-dessous. Ce code utilise [!DNL [!DNL PhantomJS]] pour ouvrir une page et attendre que le navigateur charge toutes les ressources de la page. Il génère ensuite toutes les dépendances [!DNL RequireJS] pour une page donnée.
...
Ouvrez un terminal dans le répertoire racine Commerce et exécutez le script sur chaque page de votre magasin qui représente un type de page spécifique :
...
- `include` : tableau de ressources (chaînes) incluses en tant que dépendances pour la page. [!DNL RequireJS] trace toutes les dépendances et les inclut dans le lot, sauf si elles sont exclues.
...
Ouvrez `requirejs-config.js` dans le répertoire `pub/static/frontend/Magento/luma/en_US` pour vérifier que [!DNL RequireJS] avez ajouté le fichier avec les appels de configuration de regroupement :
...
Les temps de chargement sont désormais trois fois plus rapides qu’avec le regroupement Commerce natif.
